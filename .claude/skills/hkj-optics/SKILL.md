...
| `@ImportOptics` | `package-info.java` | Lenses for external types (JDK, Jackson, etc.) via auto-detection |
| `@OpticsSpec` | `interface extends OpticsSpec<S>` | Fine-grained optics for external types with custom copy strategies |
...
| `set(A, S)` | `S` | Set all to same value |
...
| `.instanceOf(Class)` | AffinePath/TraversalPath | AffinePath | Safe downcast |
...
```java
@OpticsSpec(JsonNode.class)
public interface JsonNodeSpec {
...
Generates `JsonNodeSpecOptics` with prisms and lenses based on your declarations.
