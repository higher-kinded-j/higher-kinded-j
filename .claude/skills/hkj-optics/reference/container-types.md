...
- `Set` and `Collection` are built in too, but each names the `Each` that rebuilds its own shape, so the rule applies to them as it does to an SPI container: `Set<?>`, `Set<? extends Leaf>` and a raw `Set` are rejected at the declaration.
