...
| Generated Class | Purpose |
|----------------|---------|
| `{Effects}Wiring` | `injectXxx()` methods, `functor()`, `boundSet()` |
...
Free<AppEffectsWiring.ComposedType, String> program =
...
public class TestConsoleInterpreter extends ConsoleOpInterpreter<IdKind.Witness> {
    ...
    protected <A> Kind<IdKind.Witness, A> handleReadLine(ConsoleOp.ReadLine<A> op) {
        return IdKindHelper.ID_OP.widen(Id.of(op.k().apply(inputs.poll())));
    }

    @Override
    protected <A> Kind<IdKind.Witness, A> handlePrintLine(ConsoleOp.PrintLine<A> op) {
        outputs.add(op.message());
        return IdKindHelper.ID_OP.widen(Id.of(op.k().apply(Unit.INSTANCE)));
    }
}
```
...
var interpreter = Interpreters.combine(
    AppEffectsWiring.injectConsole(), consoleInterp,
    AppEffectsWiring.injectDb(), dbInterp,
    AppEffectsWiring.injectLog(), logInterp);
...
var testInterpreter = Interpreters.combine(
    AppEffectsWiring.injectConsole(), testConsoleInterp,
    AppEffectsWiring.injectDb(), testDbInterp,
    AppEffectsWiring.injectLog(), testLogInterp);

Id<String> result = IdKindHelper.ID_OP.narrow(
...
FreePath<F, String> free = boundary.embed(program);    // Wrap as FreePath for composition
...
| `embed(program)` | `FreePath<F, A>` | Wrap for further composition |
| `embedPath(program)` | `GenericPath<IO, A>` | Embed as GenericPath |
...
Inspect programs before execution using `ProgramAnalyser`:

```java
ProgramAnalyser<AppEffectsWiring.ComposedType> analyser = new ProgramAnalyser<>();
ProgramAnalysis analysis = analyser.analyse(program);

// Count operations by type
long dbCalls = analysis.operationCount(DbOp.class);
long consoleCalls = analysis.operationCount(ConsoleOp.class);
```
...
```java
Free<F, A> programWithRecovery = Free.handleError(
    riskyProgram,
