...
public record Employee(String id, String name, Address address) {
  ...
  public static final class Lenses {
    ...
    public static Lens<Employee, String> id() {
      return Lens.of(Employee::id, (newId, emp) -> new Employee(newId, emp.name(), emp.address()));
    }

    public static Lens<Employee, String> name() {
      return Lens.of(
          Employee::name, (newName, emp) -> new Employee(emp.id(), newName, emp.address()));
    }

    public static Lens<Employee, Address> address() {
      return Lens.of(
          Employee::address, (newAddr, emp) -> new Employee(emp.id(), emp.name(), newAddr));
    }
  }
}
