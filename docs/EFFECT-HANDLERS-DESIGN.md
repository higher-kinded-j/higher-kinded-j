...
### Phase 5: Worked Example + Documentation — COMPLETE

| Deliverable | Status | Files | Location |
|---|---|---|---|
| Payment processing example (with @EffectAlgebra + CPS) | Done | 40+ files (model, effects, interpreters, service, example, wiring) | `hkj-examples/.../payment/` |
| ProgramAnalyser + ProgramAnalysis | Done | 2 files + test | `hkj-core/.../free/` |
| EitherFFunctor.of() factory | Done | Edit to `EitherFFunctor.java` | `hkj-core/.../eitherf/` |
| hkj-book chapters | Done | `payment_processing.md`, `eitherf.md`, `effect_handlers.md` | `hkj-book/src/` |
| Tutorials (6 exercises + 6 solutions) | Done | 12 files | `hkj-examples/src/test/.../tutorial/effecthandlers/` |
| Benchmarks | Done | `FreeEffectBenchmark.java` | `hkj-benchmarks/` |
| EXAMPLES-GUIDE.md + SUMMARY.md + module-info | Done | 3 edits | `hkj-examples/`, `hkj-book/`, `hkj-examples/` |

**Implementation notes:**

- Effect algebras use continuation-passing style (CPS) with `mapK` for type-safe
  Functor delegation. Each operation carries a `Function<NaturalResult, A>`
  continuation, enabling proper type inference at call sites with zero raw types
  or unchecked casts in the service layer.
- `PaymentEffectsWiring` provides manual composition wiring (typed Inject
  factories via `Validation.function().require()`, composed functor via
  `EitherFFunctor.of()`, BoundSet record, and `interpret()` bridge method)
  because `@ComposeEffects` does not yet generate a named witness type alias.
- `EitherFFunctor` constructor is private; `of(Functor<F>, Functor<G>)` factory
  distinguishes from `instance()` (cached singleton) used by stateless type
  class instances elsewhere in the codebase.
- Audit interpreters target `WriterT<Id, AuditLog>` but Free.foldMap's eager
  Suspend optimisation discards the monadic context for strict monads,
  preventing log accumulation. Tutorial tests use recording interpreters
  instead. The limitation is documented in Free.foldMap Javadoc and hkj-book.
- Replay interpreters target `ReaderT<Id, EventLog>` and work correctly
  (ReaderT's function-based structure is not affected by the eager optimisation).
- `ProgramAnalyser` provides static analysis at 100% test coverage. All counts
  are lower bounds due to opaque `FlatMapped` continuations.
...
| EitherFFunctor | `final class` with private constructor and `of()` factory | Cannot be singleton; `of()` distinguishes from `instance()` (cached) convention |
...
|---|---|---|---|
| 14.1 Structured concurrency (Scope) | Only effect system with `StructuredTaskScope` integration | Phases 1-2, VTask, stable API | Medium |
| 14.2 Program policy engine | Pre-execution governance; no other effect system offers this | Phase 5 (ProgramAnalyser) | Small-medium |
| 14.3 Automatic batching (Haxl) | Applicative batching with `Batchable` interface | Phases 1-2 (Free.Ap) | Medium |
| 14.4 Program visualisation | Dataflow graphs from program structure | Phase 5 (ProgramAnalyser) | Small |
| 14.5 Gradual adoption boundary | Clean imperative/effect boundary with Spring integration | Phases 1-2 | Small |
| 14.6 Spring Boot integration | Auto-configuration, transactions, observability, test slices | 14.5 (EffectBoundary) | Small-medium |
| 14.7 Java platform alignment | ScopedValue, Joiner API, Gatherers | 14.1 (Scope) | Ongoing |

**Recommended pickup order**: 14.5 (boundary) first — it is small, immediately
useful, and demonstrates enterprise viability. Then 14.1 (scope) for the
concurrency story. Then 14.2 (policy) and 14.3 (batching) in either order.
14.4 (visualisation) can be developed in parallel with any of the above.

---

### 14.6 Spring Boot Integration

#### Motivation

Java developers overwhelmingly use Spring Boot for application development.
Effect handlers are only viable in the enterprise Java ecosystem if they
integrate cleanly with Spring's dependency injection, transaction management,
web request handling, and observability infrastructure. The existing
`EffectBoundary` concept (14.5) provides the foundation; this section expands
it into a full Spring Boot integration story.

#### How Free Programs Fit into Spring Applications

The key insight: Free programs are pure data structures that describe what
to do. Spring beans become interpreters. The boundary between Spring's
imperative world and the effect world is thin and well-defined.
...
│           │                                                  │
│           ▼                                                  │
│   PaymentService.processPayment()  ← Pure Free program       │
│           │                                                  │
│           ▼                                                  │
│   EffectBoundary.run(program)      ← Interpretation point    │
│           │                                                  │
│           ├── @Bean StripeGatewayInterpreter (IO)             │
│           ├── @Bean MLFraudInterpreter (IO)                   │
│           ├── @Bean PostgresLedgerInterpreter (IO)            │
│           └── @Bean EmailNotificationInterpreter (IO)         │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
**Transaction integration**: `EffectBoundary.runTransactional()` wraps the
entire program execution in a Spring `@Transactional` boundary. Because the
program is data, the boundary knows all operations before execution begins,
enabling pre-execution validation (e.g. reject programs that mix read and
write operations across inconsistent isolation levels).

**Observability**: Spring's `ObservationRegistry` can be injected into the
boundary. Each effect operation is automatically instrumented with
Micrometer spans, providing distributed tracing without modifying the
program or interpreters. The `ProgramAnalyser` metadata (operation counts,
recovery points) can be attached as span attributes.

**Health indicators**: An `EffectHealthIndicator` reports the status of
each interpreter (e.g. "StripeGateway: UP, FraudModel: DEGRADED") using
Spring Boot Actuator.

**Auto-configuration**: `@ConditionalOnClass(Free.class)` triggers
auto-configuration that registers the `EffectBoundary`, health indicators,
and observation hooks. Users add `hkj-spring-boot-starter` to their
classpath and everything wires automatically.

**WebFlux support**: For reactive applications, `EffectBoundary.runReactive()`
returns `Mono<A>` by interpreting into `IO` and converting to a
`CompletableFuture` backed by virtual threads.

#### Gradual adoption path

1. **Module boundary**: One service module uses effects internally; others
   see only the Spring `@Service` interface.
2. **Interpreter per bean**: Each external dependency (Stripe, Postgres,
   email) becomes a Spring-managed interpreter bean.
3. **Test slice**: `@EffectTest` annotation provides a test slice that
   auto-configures recording interpreters and an Id-monad boundary.

#### Why this matters for higher-kinded-j

Without Spring integration, effect handlers remain an academic exercise
in the Java ecosystem. With it, they become a practical architecture
pattern that teams can adopt incrementally alongside their existing
Spring infrastructure.

---

### 14.7 Analysis: Java Platform Evolution and Structured Concurrency

#### Current status of StructuredTaskScope

As of Java 25, `StructuredTaskScope` remains a preview API (JEP 453,
previewed since Java 21). The API has been stable across preview iterations,
and JEP 505 proposes continued refinement. Key developments:

- **Java 21-24**: `StructuredTaskScope` previewed with `ShutdownOnFailure`
  and `ShutdownOnSuccess` policies. API surface stable.
- **Java 25**: Continued preview. The `Joiner` API was introduced to
  replace the policy-based approach, providing more flexible composition
  of subtask results.
- **Java 26+**: Likely candidates include final stabilisation of the API
  and potential integration with `ScopedValue` (JEP 446) for implicit
  context propagation across virtual threads.

#### Impact on the Scope effect design (14.1)

The `ScopePolicy` sealed interface in the current design mirrors Java 21-24's
`ShutdownOnFailure`/`ShutdownOnSuccess` pattern. Java 25's `Joiner` API is
more flexible and may require redesigning `ScopePolicy`:

```java
// Java 25+ Joiner-based design (potential evolution)
sealed interface ScopePolicy<T> {
  record AllSuccessOrThrow<T>() implements ScopePolicy<List<T>> {}
  record FirstSuccess<T>() implements ScopePolicy<T> {}
  record CollectAll<T>() implements ScopePolicy<List<Result<T>>> {}
  record Custom<T, R>(Joiner<T, R> joiner) implements ScopePolicy<R> {}
}
```

**Recommendation**: Wait for `StructuredTaskScope` to leave preview before
implementing the Scope effect. The `par()` step on `FreePathSteps` already
provides basic applicative parallelism; the Scope effect adds timeout,
cancellation, and policy, which depend on a stable API.

#### ScopedValue integration

`ScopedValue` (JEP 446, preview in Java 21+) provides implicit context
propagation for virtual threads, replacing `ThreadLocal` for structured
concurrency. This is directly relevant to effect handlers:

- **ReaderT replacement**: `ScopedValue` provides the same capability as
  `ReaderT` (implicit environment access) but at the JVM level. A hybrid
  approach could use `ScopedValue` for cross-cutting concerns (request ID,
  tenant context) and `ReaderT` for effect-algebra-specific configuration.
- **Interpreter context**: Interpreters could read `ScopedValue` bindings
  to access request-scoped state without threading parameters through the
  effect algebra.

#### Gatherers (Java 24+)

Stream Gatherers (JEP 473, finalized in Java 24) provide custom
intermediate stream operations. While not directly related to Free monads,
they demonstrate Java's direction toward composable, user-defined
abstractions, which aligns with higher-kinded-j's philosophy.

---

### 14.8 Cost-Benefit Analysis and Developer Appeal

#### Per-feature assessment

| Feature | Effort | Benefit to Library | Java Developer Appeal | ROI | Recommendation |
|---|---|---|---|---|---|
| **14.5 EffectBoundary + Spring** | Small (2-3 weeks) | Critical: proves enterprise viability | Very High: speaks Spring developers' language | Highest | Do first |
| **14.1 Structured Concurrency** | Medium (4-6 weeks) | High: unique differentiator vs Haskell/Scala | High: virtual threads are the Java 21+ headline feature | High | Do second, but wait for API stabilisation |
| **14.2 Program Policy** | Small-Medium (2-4 weeks) | Medium: enterprise governance, compliance | Medium: appeals to architects and compliance teams | Medium | Do third: builds on ProgramAnalyser already shipped |
| **14.3 Haxl-style Batching** | Medium (4-6 weeks) | Medium: performance optimisation for data-heavy apps | Medium: appeals to backend developers with N+1 problems | Medium | Do fourth: FreeAp infrastructure already exists |
| **14.4 Program Visualisation** | Small (1-2 weeks) | Low-Medium: debugging and documentation aid | Medium: developers love diagrams; Mermaid is popular | Low-Medium | Do anytime: independent of other features |

#### What Java developers care about

Based on the Java ecosystem's priorities (Spring dominance, microservices,
observability, testing):

1. **"Can I use this with Spring Boot?"** — the first question every Java
   developer asks. EffectBoundary (14.5) directly answers this. Without it,
   adoption is limited to greenfield projects and functional programming
   enthusiasts.

2. **"Does it work with virtual threads?"** — the second question. The Scope
   effect (14.1) positions higher-kinded-j as the only effect system that
   integrates Java's structured concurrency model. This is a genuine
   competitive advantage.

3. **"Can I test without mocks?"** — already answered by the current
   implementation. The payment example demonstrates this convincingly.

4. **"What about performance?"** — the batching feature (14.3) addresses
   the N+1 query problem, which is a real pain point. The FreeAp-based
   approach is elegant and unique in the Java ecosystem.

5. **"Can I see what my program does?"** — visualisation (14.4) and
   policy (14.2) are "wow factor" features that differentiate from DI
   frameworks. Policy validation before execution is unprecedented.

#### What will not appeal to Java developers

- **Category theory terminology** — the codebase already avoids this
  (using "Path" instead of "Monad," "via" instead of "bind"). Continue
  this approach in future features.
- **Pervasive monadic wrapping** — the `Function.identity()` continuation
  in CPS is an ergonomic cost. Future improvements could generate
  convenience overloads that omit the continuation parameter.
- **Verbose composed witness types** — `EitherFKind.Witness<...>` nesting
  is a Java limitation. The `@ComposeEffects` processor should generate a
  named witness type alias (tracked as a future enhancement).

#### Strategic recommendation

The highest-impact path for higher-kinded-j adoption:

1. **Ship Phase 5** (current branch) — proves the pattern works end-to-end
2. **EffectBoundary + Spring starter** — unlocks enterprise adoption
3. **Structured Concurrency** — positions as the Java 21+ effect system
4. **Batching + Policy** — builds the "programs as data" story for
   architects

The visualisation feature can be developed opportunistically at any point,
as it has no dependencies and provides immediate value for documentation
and debugging.
