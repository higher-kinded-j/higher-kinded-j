...
HKJ's navigation is compile-time-generated and reflection-free in the hot path by default; telescope's default path is reflective (~100 ns per field hop) with codegen as an opt-in optimisation. HKJ trades a build step for "fast and compile-verified by default" — the right default for a library whose top value is correctness. The same default decides the **third-party-type** story: HKJ's `@ImportOptics({Foreign.class, …})` (a `@Target({PACKAGE, TYPE})` external spec) emits compile-time, reflection-free optics for classes you cannot annotate, whereas telescope's codegen annotations are all `@Target(TYPE)` — so its only answer for an un-owned type is the reflective `Telescope.of(...)` path (zero ceremony, but reflective and unverified until first call). See §3 and the appendix matrix row.
...
| Navigate/convert **un-owned** (third-party) types | `@ImportOptics({Foreign.class, …})` on a `package-info` → compile-time, reflection-free optics for types you cannot annotate | No codegen form (`@Focus`/`@BeanFocus`/`@Bridge` are all `@Target(TYPE)`); reflective `Telescope.of(Foreign.class)` (zero ceremony) or `@Bridge` only if you own one side | HKJ ahead on type-safety/perf/verification; telescope ahead on zero ceremony — a clean instance of §3 |
