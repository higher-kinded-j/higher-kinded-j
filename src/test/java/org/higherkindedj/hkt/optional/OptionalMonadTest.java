...
class OptionalMonadTest {
  ...
  private final Function<String, Kind<OptionalKind<?>, String>> g = s -> wrap(Optional.of(s + "!"));
  ...
  class ApTests {
    Kind<OptionalKind<?>, Function<Integer, String>> funcKindPresent =
  ...
  class FlatMapTests {
    ...
    void flatMap_chainingExample() {
      ...
      Kind<OptionalKind<?>, Double> finalResult =
          optionalMonad.flatMap(
              x -> { // x = 10
                Kind<OptionalKind<?>, Integer> intermediate =
                    optionalMonad.of(x + 10); // Optional.of(20)
                return optionalMonad.flatMap(
                    safeDivide, intermediate); // safeDivide(20) -> Optional.of(5.0)
              },
              initial);
    ...
    void flatMap_chainingWithEmptyPropagation() {
      ...
      Kind<OptionalKind<?>, Double> finalResult =
          optionalMonad.flatMap(
              x -> { // x = 5
                Kind<OptionalKind<?>, Integer> intermediate =
                    optionalMonad.of(x - 5); // Optional.of(0)
  ...
  class FunctorLaws {
    ...
    void composition() {
      ...
      Kind<OptionalKind<?>, String> rightSide =
          optionalMonad.map(appendWorld, optionalMonad.map(intToString, fa));

      Kind<OptionalKind<?>, String> leftSideEmpty =
          optionalMonad.map(intToStringAppendWorld, faEmpty);
      Kind<OptionalKind<?>, String> rightSideEmpty =
  ...
  class ApplicativeLaws {
    ...
    void identity() {
      Kind<OptionalKind<?>, Function<Integer, Integer>> idFuncKind =
    ...
    void composition() {
      ...
      Kind<OptionalKind<?>, Function<Function<Integer, String>, Function<Integer, String>>>
          mappedCompose = optionalMonad.map(composeMap, gKind);
      Kind<OptionalKind<?>, Function<Integer, String>> ap1 = optionalMonad.ap(mappedCompose, fKind);
      ...
      assertThat(
              unwrap(
                  optionalMonad.ap(
                      optionalMonad.ap(optionalMonad.map(composeMap, gKindEmpty), fKind), v)))
          .isEmpty();
      assertThat(unwrap(optionalMonad.ap(gKindEmpty, optionalMonad.ap(fKind, v)))).isEmpty();
      assertThat(
              unwrap(
                  optionalMonad.ap(
                      optionalMonad.ap(optionalMonad.map(composeMap, gKind), fKindEmpty), v)))
          .isEmpty();
      assertThat(unwrap(optionalMonad.ap(gKind, optionalMonad.ap(fKindEmpty, v)))).isEmpty();
      assertThat(
              unwrap(
                  optionalMonad.ap(
                      optionalMonad.ap(optionalMonad.map(composeMap, gKind), fKind), vEmpty)))
          .isEmpty();
  ...
  class MonadLaws {
    ...
    void associativity() {
      ...
      Kind<OptionalKind<?>, String> innerFlatMapEmpty =
          optionalMonad.flatMap(f, mValueEmpty); // Empty
      Kind<OptionalKind<?>, String> leftSideEmpty =
          optionalMonad.flatMap(g, innerFlatMapEmpty); // Empty
      Kind<OptionalKind<?>, String> rightSideEmpty =
          optionalMonad.flatMap(rightSideFunc, mValueEmpty); // Empty
      assertThat(unwrap(leftSideEmpty)).isEqualTo(unwrap(rightSideEmpty));
    }
  }
  ...
  class MonadErrorTests {
    ...
    void handleErrorWith_shouldHandleRaisedError() {
      ...
      Kind<OptionalKind<?>, Integer> result =
          optionalMonad.handleErrorWith(raisedErrorKind, handler);
    ...
    void handleErrorWith_shouldIgnorePresent() {
      Function<Void, Kind<OptionalKind<?>, Integer>> handler =
          err -> optionalMonad.of(-1); // Should not be called
