...
class EitherMonadTest {
    ...
    class ApTests {
        Kind<EitherKind<TestError, ?>, Function<Integer, String>> funcKindRight = eitherMonad.of(x -> "N" + x);
    ...
    class FlatMapTests {
        ...
        void flatMap_shouldApplyFunctionWhenRight() {
            Kind<EitherKind<TestError, ?>, Double> result = eitherMonad.flatMap(safeDivideEither, rightValue);
        ...
        void flatMap_shouldReturnLeftWhenFunctionReturnsLeft() {
            Kind<EitherKind<TestError, ?>, Double> result = eitherMonad.flatMap(safeDivideEither, zeroValue);
            Either<TestError, Double> either = unwrap(result);
            assertThat(either.isLeft()).isTrue();
        ...
        void flatMap_shouldPropagateLeftWhenInputIsLeft() {
            Kind<EitherKind<TestError, ?>, Double> result = eitherMonad.flatMap(safeDivideEither, leftValue);
            Either<TestError, Double> either = unwrap(result);
            assertThat(either.isLeft()).isTrue();
        ...
        void flatMap_chainingExample() {
            ...
            Function<String, Kind<EitherKind<TestError, ?>, Integer>> parseIntEither = s -> {
                try {
            ...
            Kind<EitherKind<TestError, ?>, Double> finalResult = eitherMonad.flatMap(
                    str -> eitherMonad.flatMap(safeDivideEither, parseIntEither.apply(str)), // nested flatMap
                    initial
            );
        ...
        void flatMap_chainingWithLeftPropagation() {
            ...
            Kind<EitherKind<TestError, ?>, Double> finalResult = eitherMonad.flatMap(
                    str -> eitherMonad.flatMap(safeDivideEither, parseIntEither.apply(str)), // safeDivideEither is never reached
                    initial
            );
    ...
    class FunctorLaws {
        ...
        void identity() {
            ...
            assertThat(unwrap(eitherMonad.map(Function.identity(), fa)))
                    .isEqualTo(unwrap(fa));
            assertThat(unwrap(eitherMonad.map(Function.identity(), faLeft)))
                    .isEqualTo(unwrap(faLeft));
        }
        ...
        void composition() {
            ...
            Kind<EitherKind<TestError, ?>, String> leftSide = eitherMonad.map(appendWorld.compose(intToString), fa);
            Kind<EitherKind<TestError, ?>, String> rightSide = eitherMonad.map(appendWorld, eitherMonad.map(intToString, fa));

            Kind<EitherKind<TestError, ?>, String> leftSideLeft = eitherMonad.map(appendWorld.compose(intToString), faLeft);
            Kind<EitherKind<TestError, ?>, String> rightSideLeft = eitherMonad.map(appendWorld, eitherMonad.map(intToString, faLeft));


            assertThat(unwrap(leftSide)).isEqualTo(unwrap(rightSide));
            assertThat(unwrap(leftSideLeft)).isEqualTo(unwrap(rightSideLeft)); // Check Left propagation consistency
        }
    }
    ...
    class ApplicativeLaws {
        ...
        void identity() {
            Kind<EitherKind<TestError, ?>, Function<Integer, Integer>> idFuncKind = eitherMonad.of(Function.identity());
            assertThat(unwrap(eitherMonad.ap(idFuncKind, v))).isEqualTo(unwrap(v));
            assertThat(unwrap(eitherMonad.ap(idFuncKind, vLeft))).isEqualTo(unwrap(vLeft)); // Check Left propagation
        }
        ...
        void interchange() {
            ...
            Kind<EitherKind<TestError, ?>, String> leftSideFuncLeft = eitherMonad.ap(fKindLeft, eitherMonad.of(y));

            // Right Side: ap(of(f -> f(y)), fKind)
            Function<Function<Integer, String>, String> evalWithY = fn -> fn.apply(y);
            Kind<EitherKind<TestError, ?>, Function<Function<Integer, String>, String>> evalKind = eitherMonad.of(evalWithY);

            Kind<EitherKind<TestError, ?>, String> rightSide = eitherMonad.ap(evalKind, fKind);
            Kind<EitherKind<TestError, ?>, String> rightSideFuncLeft = eitherMonad.ap(evalKind, fKindLeft);

            assertThat(unwrap(leftSide)).isEqualTo(unwrap(rightSide));
            assertThat(unwrap(leftSideFuncLeft)).isEqualTo(unwrap(rightSideFuncLeft)); // Check Left propagation
        }

        @Test
        @DisplayName("4. Composition: ap(map(compose, fKind), gKind) == ap(fKind, ap(gKind, v)) - Adjusted for types")
            // Standard law: ap ( ap ( map (compose) u ) v ) w = ap u ( ap v w )
            // Let u = map(compose, fKind) = map(g -> f.compose(g), fKind) -> This doesn't type check easily here.
            ...
            // Let's try: ap(fg, ap(ga, value)) == ap(ap(map(compose, fg), ga), value) where compose is (b->c) -> (a->b) -> (a->c)
        void composition() {
            Function<Function<String, String>, Function<Function<Integer, String>, Function<Integer, String>>> composeMap =
                    g -> f -> g.compose(f); // (String -> String) -> (Integer -> String) -> (Integer -> String)

            Kind<EitherKind<TestError, ?>, Integer> value = right(10);

            // Left side: ap(ap(map(composeMap, gKind), fKind), value)
            Kind<EitherKind<TestError, ?>, Function<Function<Integer, String>, Function<Integer, String>>> mappedCompose =
                    eitherMonad.map(composeMap, gKind);
            ...
            Kind<EitherKind<TestError, ?>, String> leftSideGLeft = eitherMonad.ap(eitherMonad.ap(eitherMonad.map(composeMap, gKindLeft), fKind), value);
            Kind<EitherKind<TestError, ?>, String> rightSideGLeft = eitherMonad.ap(gKindLeft, eitherMonad.ap(fKind, value));
            assertThat(unwrap(leftSideGLeft)).isEqualTo(unwrap(rightSideGLeft));

            Kind<EitherKind<TestError, ?>, String> leftSideFLeft = eitherMonad.ap(eitherMonad.ap(eitherMonad.map(composeMap, gKind), fKindLeft), value);
            Kind<EitherKind<TestError, ?>, String> rightSideFLeft = eitherMonad.ap(gKind, eitherMonad.ap(fKindLeft, value));
            assertThat(unwrap(leftSideFLeft)).isEqualTo(unwrap(rightSideFLeft));

            Kind<EitherKind<TestError, ?>, Integer> valueLeft = left("VAL_ERR");
            Kind<EitherKind<TestError, ?>, String> leftSideValLeft = eitherMonad.ap(eitherMonad.ap(eitherMonad.map(composeMap, gKind), fKind), valueLeft);
            Kind<EitherKind<TestError, ?>, String> rightSideValLeft = eitherMonad.ap(gKind, eitherMonad.ap(fKind, valueLeft));
    ...
    class MonadLaws {
        ...
        void rightIdentity() {
            ...
            Kind<EitherKind<TestError, ?>, Integer> leftSideLeft = eitherMonad.flatMap(ofFunc, mValueLeft);
        ...
        void associativity() {
            ...
            Kind<EitherKind<TestError, ?>, String> innerFlatMapLeft = eitherMonad.flatMap(f, mValueLeft); // Should be Left
            Kind<EitherKind<TestError, ?>, String> leftSideLeft = eitherMonad.flatMap(g, innerFlatMapLeft); // Should be Left

            Kind<EitherKind<TestError, ?>, String> rightSideLeft = eitherMonad.flatMap(rightSideFunc, mValueLeft); // Should be Left
