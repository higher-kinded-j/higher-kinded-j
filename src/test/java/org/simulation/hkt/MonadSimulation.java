...
public class MonadSimulation {
  ...
  public static void main(String[] args) {
    ...
    monadSimulation.applicativeExample();
  }
  ...
  void listMonadExample() {
    ...
    // 3. Using flatMap
    // Function that takes an integer and returns a ListKind of integers (e.g., the number and
    // itself * 10)
    ...
    System.out.println(
        "flatMap(duplicateAndMultiply): "
            + ListKindHelper.unwrap(flatMappedList)); // Output: [1, 10, 2, 20, 3, 30]
  }
  ...
  void optionalMonadExample() {
    ...
    System.out.println(
        "of(\"hello\"): " + OptionalKindHelper.unwrap(ofOpt)); // Output: Optional[hello]
    System.out.println(
        "of(null): " + OptionalKindHelper.unwrap(ofNullOpt)); // Output: Optional.empty
    ...
    System.out.println(
        "map on value: " + OptionalKindHelper.unwrap(mappedValue)); // Output: Optional[Value: 5]
    System.out.println(
        "map on empty: " + OptionalKindHelper.unwrap(mappedEmpty)); // Output: Optional.empty
    ...
    OptionalKind<Double> result2 =
        optionalMonad.flatMap(safeDivideBy, zeroOptKind); // Will be empty
    OptionalKind<Double> result3 =
        optionalMonad.flatMap(safeDivideBy, emptyOptKind); // Will be empty

    System.out.println(
        "flatMap safeDivideBy(5): " + OptionalKindHelper.unwrap(result1)); // Output: Optional[20.0]
    System.out.println(
        "flatMap safeDivideBy(0): " + OptionalKindHelper.unwrap(result2)); // Output: Optional.empty
    System.out.println(
        "flatMap safeDivideBy(empty): "
            + OptionalKindHelper.unwrap(result3)); // Output: Optional.empty
    ...
    OptionalKind<Integer> addedTenResult =
    ...
    OptionalKind<Double> finalResult =
        optionalMonad.flatMap(
    ...
    System.out.println(
        "Chained Optional flatMap result: "
            + OptionalKindHelper.unwrap(finalResult)); // Output: Optional[5.0]
  }
  ...
  void maybeMonadExample() {
    ...
    System.out.println(
        "map sqrt on Just(36): " + MaybeKindHelper.unwrap(mappedJust)); // Output: Just(6.0)
    System.out.println(
        "map sqrt on Nothing: " + MaybeKindHelper.unwrap(mappedNothing)); // Output: Nothing

    // 3. Using flatMap for safe inversion
    Function<Integer, Kind<MaybeKind<?>, Double>> safeInvertMaybe =
        num -> MaybeKindHelper.wrap((num == 0) ? Maybe.nothing() : Maybe.just(1.0 / num));
    ...
    MaybeKind<Double> resultMaybe2 =
        maybeMonad.flatMap(safeInvertMaybe, zeroMaybeKind); // Becomes Nothing
    MaybeKind<Double> resultMaybe3 =
        maybeMonad.flatMap(safeInvertMaybe, nothingKind); // Stays Nothing

    System.out.println(
        "flatMap safeInvertMaybe(10): "
            + MaybeKindHelper.unwrap(resultMaybe1)); // Output: Just(0.1)
    System.out.println(
        "flatMap safeInvertMaybe(0): " + MaybeKindHelper.unwrap(resultMaybe2)); // Output: Nothing
    System.out.println(
        "flatMap safeInvertMaybe(Nothing): "
            + MaybeKindHelper.unwrap(resultMaybe3)); // Output: Nothing

    // Chaining flatMap (Maybe)
    MaybeKind<Integer> initialMaybe = MaybeKindHelper.wrap(Maybe.just(4));

    // Step 1: Multiply by 5
    MaybeKind<Integer> multipliedResultMaybe =
        maybeMonad.flatMap(
    ...
    MaybeKind<Double> finalResultMaybe =
        maybeMonad.flatMap(
    ...
    System.out.println(
        "Chained Maybe flatMap result: "
            + MaybeKindHelper.unwrap(finalResultMaybe)); // Output: Just(0.05) (1.0 / (4*5))

    // Example showing Nothing propagation in chaining
    MaybeKind<Integer> initialNothingMaybe = MaybeKindHelper.wrap(Maybe.nothing());
    MaybeKind<Integer> chainedResultMaybeNothing1 =
        maybeMonad.flatMap(
            x -> MaybeKindHelper.wrap(Maybe.just(x * 5)), initialNothingMaybe // Start with Nothing
            );
    MaybeKind<Double> finalResultMaybeNothing =
        maybeMonad.flatMap(
            safeInvertMaybe, chainedResultMaybeNothing1 // This is already NothingKind
            );
    System.out.println(
        "Chained Maybe flatMap starting with Nothing: "
            + MaybeKindHelper.unwrap(finalResultMaybeNothing)); // Output: Nothing
  }
  ...
  void eitherMonadExample() {
    ...
    Kind<EitherKind<String, ?>, String> rightStringKind =
        EitherKindHelper.wrap(Either.right("456"));
    Kind<EitherKind<String, ?>, String> rightInvalidStringKind =
        EitherKindHelper.wrap(Either.right("abc"));
    Kind<EitherKind<String, ?>, String> leftStringKind =
        EitherKindHelper.wrap(Either.left("Initial error"));

    // flatMap on Right(valid string) -> Right(parsed int)
    Kind<EitherKind<String, ?>, Integer> fmResult1 =
        eitherMonad.flatMap(parseIntEither, rightStringKind);
    System.out.println(
        "flatMap parseIntEither on Right(\"456\"): "
            + EitherKindHelper.unwrap(fmResult1)); // Output: Right(456)

    // flatMap on Right(invalid string) -> Left(error message)
    Kind<EitherKind<String, ?>, Integer> fmResult2 =
        eitherMonad.flatMap(parseIntEither, rightInvalidStringKind);
    System.out.println(
        "flatMap parseIntEither on Right(\"abc\"): "
            + EitherKindHelper.unwrap(fmResult2)); // Output: Left(Invalid integer format: 'abc')

    // flatMap on Left -> Left propagates
    Kind<EitherKind<String, ?>, Integer> fmResult3 =
        eitherMonad.flatMap(parseIntEither, leftStringKind);
    System.out.println(
        "flatMap parseIntEither on Left(\"Initial error\"): "
            + EitherKindHelper.unwrap(fmResult3)); // Output: Left(Initial error)

    // 4. Chaining flatMap
    Kind<EitherKind<String, ?>, String> initialEitherStr =
        EitherKindHelper.wrap(Either.right(" 20 ")); // Start with a string

    // Step 1: Parse the string
    Kind<EitherKind<String, ?>, Integer> parsedResult =
        eitherMonad.flatMap(parseIntEither, initialEitherStr);
    ...
    Kind<EitherKind<String, ?>, Double> finalResultEither =
        eitherMonad.flatMap(safeDivideEither, parsedResult);
    System.out.println(
        "Chained Either: parseInt -> safeDivide: "
            + EitherKindHelper.unwrap(finalResultEither)); // Output: Right(5.0)

    // Chaining example with failure propagation
    Kind<EitherKind<String, ?>, String> initialEitherBadStr =
        EitherKindHelper.wrap(Either.right("xyz"));
    Kind<EitherKind<String, ?>, Integer> parsedResultFail =
        eitherMonad.flatMap(parseIntEither, initialEitherBadStr);
    // parsedResultFail holds Left("Invalid integer format: 'xyz'")
    Kind<EitherKind<String, ?>, Double> finalResultEitherFail =
        eitherMonad.flatMap(safeDivideEither, parsedResultFail);
    // safeDivideEither is never called because parsedResultFail is Left
    System.out.println(
        "Chained Either starting with bad string: "
            + EitherKindHelper.unwrap(
                finalResultEitherFail)); // Output: Left(Invalid integer format: 'xyz')
  }

  void applicativeExample() {
    // Inside a test method or main:
    OptionalMonad optionalMonad = new OptionalMonad();

    // Function wrapped in OptionalKind: Kind<OptionalKind<?>, Function<Integer, String>>
    Kind<OptionalKind<?>, Function<Integer, String>> funcKind =
        optionalMonad.of(x -> "Value is " + x);

    // Value wrapped in OptionalKind: Kind<OptionalKind<?>, Integer>
    Kind<OptionalKind<?>, Integer> valueKind = optionalMonad.of(10);
    Kind<OptionalKind<?>, Integer> emptyKind =
        optionalMonad.of(null); // Represents Optional.empty wrapped

    // Using ap
    Kind<OptionalKind<?>, String> result1 = optionalMonad.ap(funcKind, valueKind);
    Kind<OptionalKind<?>, String> result2 = optionalMonad.ap(funcKind, emptyKind); // Will be empty

    System.out.println(
        "ap result 1: " + OptionalKindHelper.unwrap(result1)); // Output: Optional[Value is 10]
    System.out.println(
        "ap result 2: " + OptionalKindHelper.unwrap(result2)); // Output: Optional.empty

    // Example with map2 (using default implementation in Applicative interface)
    Kind<OptionalKind<?>, Integer> val1 = optionalMonad.of(5);
    Kind<OptionalKind<?>, Integer> val2 = optionalMonad.of(7);
    Kind<OptionalKind<?>, Integer> val3 = optionalMonad.of(null); // empty

    Kind<OptionalKind<?>, Integer> sum1 = optionalMonad.map2(val1, val2, (a, b) -> a + b);
    Kind<OptionalKind<?>, Integer> sum2 =
        optionalMonad.map2(val1, val3, (a, b) -> a + b); // Will be empty

    System.out.println("map2 sum 1: " + OptionalKindHelper.unwrap(sum1)); // Output: Optional[12]
    System.out.println("map2 sum 2: " + OptionalKindHelper.unwrap(sum2)); // Output: Optional.empty
  }
}
