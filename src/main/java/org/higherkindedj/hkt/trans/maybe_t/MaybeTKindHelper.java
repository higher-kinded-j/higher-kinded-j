...
/**
 * A utility class for simulating higher-kinded types (HKT) with {@link MaybeT}.
 * It provides methods to safely wrap a concrete {@link MaybeT} instance into its
 * {@link Kind} representation ({@link MaybeTKind}) and to unwrap it back.
 *
 * <p>This helper is essential for using {@code MaybeT} with generic HKT abstractions
 * like {@code Monad<MaybeTKind<F, ?>, A>}. It encapsulates the necessary type casting
 * and provides runtime safety checks.
 *
...
public final class MaybeTKindHelper {
  ...
  /**
   * Error message for an invalid state where the internal holder contains a null MaybeT.
   * This should ideally not be reachable if wrap ensures non-null inputs.
   */
  ...
  /**
   * Unwraps a {@link Kind} representation of {@code MaybeT} (i.e., a {@link MaybeTKind})
   * back to its concrete {@link MaybeT}{@code <F, A>} type.
   *
   ...
   * @throws KindUnwrapException if {@code kind} is null, not an instance of the internal
   * {@code MaybeTHolder}, or if (theoretically) the holder contained a null {@code MaybeT}.
   */
  @SuppressWarnings("unchecked")
  public static <F, A> @NonNull MaybeT<F, A> unwrap(
      @Nullable Kind<MaybeTKind<F, ?>, A> kind) {
  ...
  /**
   * Wraps a concrete {@link MaybeT}{@code <F, A>} instance into its {@link Kind}
   * representation ({@link MaybeTKind}{@code <F, A>}).
   *
   * @param <F> The witness type of the outer monad in {@code MaybeT}.
   * @param <A> The type of the value potentially held by the inner {@code Maybe}.
  ...
  /**
   * Internal record implementing the {@link MaybeTKind} interface.
   * This class acts as the actual holder of the concrete {@link MaybeT} instance
   * when it's treated as a {@link Kind}. It is not intended for direct public use.
   *
   * @param <F> The witness type of the outer monad.
   * @param <A> The type of the value in the {@code MaybeT}.
   * @param maybeT The concrete {@link MaybeT} instance. Must not be null, as {@link MaybeT}
   * itself requires a non-null underlying value.
   */
  record MaybeTHolder<F, A>(@NonNull MaybeT<F, A> maybeT)
