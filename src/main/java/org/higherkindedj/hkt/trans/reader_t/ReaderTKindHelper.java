...
public final class ReaderTKindHelper {
    ...
    public static final String INVALID_HOLDER_STATE_MSG = "ReaderTHolder contained null ReaderT instance";
    ...
    /**
     * Unwraps a ReaderTKind back to the concrete {@code ReaderT<F, R, A>} type.
     * Throws KindUnwrapException if the Kind is null, not a ReaderTHolder,
     * or the holder contains a null ReaderT instance.
     */
    @SuppressWarnings("unchecked")
    public static <F, R, A> @NonNull ReaderT<F, R, A> unwrap(@Nullable Kind<ReaderTKind<F, R, ?>, A> kind) {
        if (kind == null) {
    ...
    /**
     * Wraps a concrete {@code ReaderT<F, R, A>} value into the ReaderTKind simulation type.
     * Requires a non-null ReaderT instance as input.
     */
    ...
    record ReaderTHolder<F, R, A>(@NonNull ReaderT<F, R, A> readerT) implements ReaderTKind<F, R, A> {}
}