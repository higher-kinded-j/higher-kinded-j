...
public class WorkflowModels {
  ...
  /**
   * Holds the state as the workflow progresses.
   * Fields are @Nullable initially and populated by workflow steps.
   * Accessors now return @NonNull, relying on the workflow's flow control
   * to ensure they are only called after the corresponding data is available.
   */
  public record WorkflowContext(
    ...
    public @NonNull WorkflowContext withPaymentConfirmation(@NonNull PaymentConfirmation pc) {
      ...
      java.util.Objects.requireNonNull(pc, "PaymentConfirmation cannot be null for withPaymentConfirmation");
     ...
     * Assumes this is only called after the validation step has successfully completed.
    ...
    public @NonNull ValidatedOrder validatedOrder() {
      ...
      assert validatedOrder != null : "validatedOrder() called before validation completed successfully";
      return validatedOrder;
    }

    /**
     * Returns the payment confirmation details.
     * Assumes this is only called after the payment step has successfully completed.
     * @return The non-null PaymentConfirmation.
     */
    public @NonNull PaymentConfirmation paymentConfirmation() {
      // No runtime check - relies on flow control in the runner
      assert paymentConfirmation != null : "paymentConfirmation() called before payment completed successfully";
      return paymentConfirmation;
    }

    /**
     * Returns the shipment information.
     * Assumes this is only called after the shipment step has successfully completed.
