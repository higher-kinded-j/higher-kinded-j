...
/**
 * Represents a stateful computation S -> (A, S).
 * It wraps a function that takes an initial state and returns a pair
 * containing the computed value and the final state.
 *
...
public interface State<S, A> {
  ...
  /**
   * Maps the result value A to B while keeping the state transition.
   * map(f) is equivalent to s -> { (a, s') = run(s); return (f(a), s'); }
   */
  ...
  /**
   * Composes this State computation with another function that returns a State.
   * flatMap(f) is equivalent to s0 -> { (a, s1) = run(s0); return f(a).run(s1); }
   */
  ...
  /**
   * Creates a State that returns the given value and leaves the state unchanged.
   * This is the 'pure' or 'of' operation for the State monad.
   */
  ...
  /**
   * Creates a State that returns the current state as the value and leaves the state unchanged.
   * get = s -> (s, s)
   */
  ...
  /**
   * Creates a State that replaces the current state with the given new state and returns no value (Unit).
   * set(newState) = s -> (null, newState)
   */
  ...
  /**
   * Creates a State that modifies the current state using the given function and returns no value (Unit).
   * modify(f) = s -> (null, f(s))
   */
