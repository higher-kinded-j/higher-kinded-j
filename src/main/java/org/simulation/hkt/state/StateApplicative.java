...
public class StateApplicative<S> extends StateFunctor<S> implements Applicative<StateKind<S, ?>> {
  ...
  public <A, B> @NonNull Kind<StateKind<S, ?>, B> ap(
      @NonNull Kind<StateKind<S, ?>, Function<A, B>> ff,
      @NonNull Kind<StateKind<S, ?>, A> fa) {

    State<S, Function<A, B>> stateF = unwrap(ff);
    State<S, A> stateA = unwrap(fa);

    // Implement ap for State: s0 -> { (f, s1) = stateF(s0); (a, s2) = stateA(s1); return (f(a), s2); }
    State<S, B> stateB = State.of(initialState -> {
      ...
      if (func == null) {
        // Decide behavior if function is null. Common is to throw, or propagate conceptually.
        // Let's throw for now, or adjust based on desired semantics (e.g., return default/null B).
