...
/**
 * Represents a computation that depends on a read-only environment R
 * to produce a value A. Essentially a wrapper around Function<R, A>.
 *
...
public interface Reader<R, A> {
  ...
  /**
   * Maps the result of this Reader using function f.
   * map(f) is equivalent to r -> f(run(r)).
   */
  ...
  /**
   * Composes this Reader with another function that returns a Reader.
   * flatMap(f) is equivalent to r -> f(run(r)).run(r).
   */
