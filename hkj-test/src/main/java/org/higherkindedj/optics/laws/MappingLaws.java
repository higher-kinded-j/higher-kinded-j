...
public final class MappingLaws {
   ...
   *       constructed bean, which is what a binder makes of an empty body: a default the bean gives
   *       itself (a field initialiser, a constructor assignment, a getter that creates its value)
   *       then reads as sent and fails the law wherever it differs from {@code domainSample}, where
   *       handing each setter {@code null} would have overwritten an initialiser's or a
   *       constructor's default and hidden it.
   ...
   * @param allAbsentWire a wire with every property absent: a freshly constructed bean
