// Fixture for hkj-book/src/hkts/order-walkthrough.md
//
// The page walks the same order workflow as order-composition.md, and elides the same pieces, so
// the two fixtures declare the same domain. The exception-based version the page opens with needs a
// legacy service of its own, which is declared here too.
//
...
record OrderRequest(
    ...
    List<ValidatedOrderLine> items,
    ShippingAddress address,
    ShippingAddress shippingAddress) {}

/** The exception-based services the page's "pyramid of doom" calls. */
class ValidationException extends RuntimeException {}

class InventoryException extends RuntimeException {}

record Reservation(boolean success, String reason) {
...
interface LegacyInventoryService {
  Reservation reserve(List<ValidatedOrderLine> items) throws InventoryException;
}
...
class Fixture {
  ...
  static final LegacyInventoryService legacyInventoryService = sample();
  ...
  static ValidatedShippingAddress validateAddress(ShippingAddress address)
