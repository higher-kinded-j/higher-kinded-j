 ...
 *
 * <p>Key Concepts: - flatMap: chains dependent computations - Each step can decide what to do
 * based on previous results - Automatically handles the context (no manual unwrapping needed) -
...
public class Tutorial04_MonadChaining {
   ...
   *
   * <p>When should you use flatMap vs map? - Use map when your function returns a plain value -
   * Use flatMap when your function returns a wrapped value (Either, Maybe, etc.)
   *
   ...
   * between map (plain values) and flatMap (wrapped values) ✓ That flatMap short-circuits on
   * errors ✓ How flatMap works with different types (Either, Maybe, List) ✓ How to combine
   * dependent values with flatMap2
   *
