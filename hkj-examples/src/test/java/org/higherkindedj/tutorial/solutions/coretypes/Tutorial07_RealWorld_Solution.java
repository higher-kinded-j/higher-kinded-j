...
public class Tutorial07_RealWorld_Solution {
                        ...
                        .flatMap(
                            email ->
                                validatePassword
                                    ...
                                    .flatMap(
                                        password ->
                                            validateAge.apply(25).map(age -> new Registration(username, email, password, age)))));
        ...
        greetUser
            ...
            .flatMap(
                greeting ->
                    Reader.<Config>ask()
                        .map(
                            config -> greeting + " (v" + config.version() + ")"));
  ...
  void exercise5_batchOperations() {
    ...
    List<Either<String, String>> processed = items.stream()
        .map(processItem)
        .collect(Collectors.toList());
  ...
  void exercise6_miniWorkflow() {
    ...
    Either<String, ProcessedOrder> result = validateOrder.apply(order).map(o -> processOrder.apply(config).apply(o));
