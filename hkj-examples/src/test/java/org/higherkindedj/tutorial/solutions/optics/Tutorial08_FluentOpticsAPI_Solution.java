 ...
 * operations with Either, Maybe, and Validated - Query operations (exists, count, find) -
 * Cleaner syntax for common patterns
 *
 * <p>This tutorial covers the fluent API. For the Free Monad DSL approach, see Tutorial 09.
 */
public class Tutorial08_FluentOpticsAPI_Solution {
  ...
  void exercise4_queryOperations() {
    ...
    Team team =
        new Team(
            List.of(
                new Player("Alice", 120), new Player("Bob", 85), new Player("Charlie", 95)));
   ...
   *
   * <p>OpticOps.modifyAllValidated() validates multiple values and accumulates ALL errors, not
   * just the first one.
   *
  ...
  void exercise7_validationWithValidated() {
    ...
    Order order =
        new Order(
            "ORD-001",
            List.of(
                new Item("Widget", 10.0), new Item("Gadget", -5.0), new Item("Gizmo", 0.0)));
