...
public class Tutorial01_KindBasics_Solution {
  ...
  void exercise2_narrowKindToEither() {
    ...
    // SOLUTION: Use EITHER.narrow() to convert Kind back to Either
  ...
  void exercise3_listKindConversion() {
    ...
    // SOLUTION: Use LIST.widen() to convert List to Kind
    Kind<ListKind.Witness, String> kind = LIST.widen(list);

    // SOLUTION: Use LIST.narrow() to convert Kind back to List
   ...
   * EitherKind.Witness<L> for Either<L, R> - ListKind.Witness for ListOf<A> - MaybeKind.Witness
   * for Maybe<A>
   *
  ...
  void exercise4_understandingWitnessTypes() {
    ...
    // SOLUTION: The witness type for Either<String, Boolean> is EitherKind.Witness<String>
   ...
   *
   * <p>You now understand: ✓ What Kind<F, A> represents ✓ How to widen concrete types to Kind ✓
   * How to narrow Kind back to concrete types ✓ What witness types are and how they work
   *
