 ...
 *
 * <p>Common uses: - All elements in a List - All values in a Map - All fields of a specific type
 * in nested structures
 */
public class Tutorial04_TraversalBasics_Solution {
  ...
  void exercise1_modifyingAllElements() {
    ...
    Team team =
        new Team(
            "Team Alpha", List.of(new Player("Alice", 100), new Player("Bob", 90)));

    Traversal<Team, Player> playersTraversal = TeamTraversals.players();

    // SOLUTION: Use Traversals.modify() to double all player scores
    Team updated = Traversals.modify(
        playersTraversal,
        player -> new Player(player.name(), player.score() * 2),
        team);
  ...
  void exercise3_traversalWithLens() {
    ...
    Traversal<Team, Integer> allScores = TeamTraversals.players().andThen(PlayerLenses.score().asTraversal());
   ...
   * compose Traversals to reach nested collections ✓ How to combine Traversals with Lenses ✓ How
   * to filter elements with filtered() ✓ How to extract all values with getAll() ✓ How to apply
