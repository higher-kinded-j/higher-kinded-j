...
public final class TutorialGettingStarted {
  ...
  private static void verifyTypeClasses() {
    ...
    Either<String, Integer> result =
        Either.right(5).flatMap(x -> Either.right(10).map(y -> x + y));
   ...
   *
   * <p>Higher-Kinded-J requires Java 24+ to leverage modern language features. This method
   * confirms your JDK version is sufficient.
   */
  private static void verifyJavaFeatures() {
    ...
    // Test sealed interfaces (Java 17+)
   ...
   *
   * <p>Execute this before starting the tutorials to ensure everything is configured correctly.
   * Any assertion failures indicate a setup problem that should be resolved first.
   *
