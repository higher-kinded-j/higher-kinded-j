...
public class ForComprehensionExample {
  ...
  private static void maybeExample() {
    final MonadZero<MaybeKind.Witness> maybeMonad = Instances.monadZero(maybe());
  ...
  private static void stateTExample() {
    final MonadZero<OptionalKind.Witness> optionalMonad = Instances.monadZero(optional());
    final Monad<StateTKind.Witness<Integer, OptionalKind.Witness>> stateTMonad =
  ...
  private static void traverseExample() {
    ...
    Kind<MaybeKind.Witness, List<Integer>> traverseResult =
        For.from(Instances.monadZero(maybe()), MAYBE.just(Arrays.asList(1, 2, 3)))
