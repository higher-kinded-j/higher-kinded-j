...
val bookVerify = tasks.register<Test>("bookVerify") {
    ...
    // The Claude Code skills are documentation too, and the build plugins install the hkj-* ones
    // into consumer projects. Until they were gated, nothing compiled their code.
...
// The Compiler Messages page (hkj-book/src/mapping/compiler_errors.md) is generated by
// hkj-book/tools/compiler-messages, which compiles every entry's reproducer with the same classpath
// and processor path the book gate uses. This writes those paths, and the toolchain's javac, where
// that script reads them.
