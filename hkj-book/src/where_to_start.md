...
| You are mapping a domain record to/from a wire DTO, every bad field located | [Record Mapping](mapping/ch_intro.md) |
| A sealed error hierarchy repeats `code`/`message`/`context` on every variant | [`@GenerateErrorEnvelope`](mapping/merge_envelopes.md#generating-error-envelopes-generateerrorenvelope) |
...
| A domain record and a wire DTO to map both ways, with validation | [`@GenerateMapping`](mapping/ch_intro.md) |
| A REST PATCH where an absent field means "leave unchanged" | [Multi-Edit and Sparse Updates](optics/multi_edit.md), generated by [`UpdateSpec`](mapping/beans_patch.md#sparse-patch-write-back-updatespec) |
...
| Mapping domain ↔ wire DTO with validation | [Record Mapping](mapping/ch_intro.md) |
| Sparse REST PATCH (absent means keep) | [`UpdateSpec`](mapping/beans_patch.md#sparse-patch-write-back-updatespec) |
