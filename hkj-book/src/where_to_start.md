...
| You are mapping a domain record to/from a wire DTO, every bad field located | [Record Mapping](optics/record_mapping.md) |
| A sealed error hierarchy repeats `code`/`message`/`context` on every variant | [`@GenerateErrorEnvelope`](optics/record_mapping.md#generating-error-envelopes-generateerrorenvelope) |
...
| A domain record and a wire DTO to map both ways, with validation | [`@GenerateMapping`](optics/record_mapping.md) |
| A REST PATCH where an absent field means "leave unchanged" | [Multi-Edit and Sparse Updates](optics/multi_edit.md), generated by [`UpdateSpec`](optics/record_mapping.md#sparse-patch-write-back-updatespec) |
...
| Mapping domain ↔ wire DTO with validation | [Record Mapping](optics/record_mapping.md) |
| Sparse REST PATCH (absent means keep) | [`UpdateSpec`](optics/record_mapping.md#sparse-patch-write-back-updatespec) |
