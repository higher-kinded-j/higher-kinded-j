...
A lambda, a method reference, or a variable of a `Supplier` type picks the deferred overload; any other argument picks the eager one. Reach for the deferred form when building the error is not free - it formats a message, reads a `MessageSource`, or captures a stack trace - because on the `Just` branch the supplier is never called. For a plain record the eager form reads better and costs nothing.

An error type that is *itself* a functional interface would be read as a supplier, so name the eager overload for it: `maybeUser.<MyError>toEitherPath(myError)`.
~~~
