...
The optics side has one requirement of its own, and no index. A [navigator](../optics/focus_navigation.md#which-fields-get-a-navigator) into a record declared in a dependency composes that record's own `Focus` class by name, so **a module whose records are navigated from elsewhere needs the processor too**, exactly as a module that declares specs does. A dependency that declares `@GenerateFocus` records without generating their companions leaves the consumer no class to compose, and the consumer falls back to the plain `FocusPath` for those components. Rebuild dependencies whose `Focus` classes predate 0.4.11 rather than mixing generations: an older companion carries no navigator inner classes for a consumer to reach.
