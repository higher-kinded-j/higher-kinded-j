...
- **Java-Friendly APIs** – Three complementary APIs that make optics feel native to Java: the Focus DSL for path-based navigation, the Fluent API for validation-aware updates, and the Free Monad DSL for programs-as-data. Backed by annotation-driven code generation (`@GenerateLenses`, `@GenerateFocus`, `@GeneratePrisms`, and friends). Also home to [Record Mapping](record_mapping.md): the compile-time domain ↔ DTO boundary, generated from a spec interface, with every bad field reported at once.
...
6. [Java-Friendly APIs](ch4_intro.md) - Focus DSL, Fluent API, record mapping, code generation
...
- **Mapping a domain record to/from a wire DTO?** Go straight to [Record Mapping](record_mapping.md); it needs none of the optics curriculum first.
