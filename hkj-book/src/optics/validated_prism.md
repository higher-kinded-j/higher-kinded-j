...
- Constructing a `ValidatedPrism` with `ValidatedPrism.of`, wrapping a throwing parser lawfully with `ValidatedPrism.canonical`, and landing on the railway with `parsePath`
...
* **`canonical(message, parse, render)` wraps a throwing parser lawfully**: the render defines the canonical form and the per-value guard rejects every other spelling — the render's injectivity stays your obligation
