...
The `For` builder supports up to **8 chained bindings** (generators, value bindings, or focus/match operations). Arities 1-5 are hand-written; arities 6-8 are generated by the `hkj-processor` annotation processor. Both the spread-style yield (`(a, b, c, ...) -> result`) and tuple-style yield (`tuple -> result`) are supported at all arities.
~~~

### Extended Arity Example (6+ Bindings)

When a comprehension needs more than five steps, the generated `Steps6`-`Steps8` classes continue the same fluent API seamlessly:
...
At arities 6-8, the tuple-style `yield` is especially convenient for accessing accumulated values by position:
