...
The `For` builder supports up to **8 chained bindings** (generators, value bindings, or focus/match operations). Step 1 is hand-written; steps 2-8 are generated by the `hkj-processor` annotation processor. Both the spread-style yield (`(a, b, c, ...) -> result`) and tuple-style yield (`tuple -> result`) are supported at all arities.
~~~

### Extended Arity Example (6+ Bindings)

The generated `Steps2`-`Steps8` classes provide the same fluent API seamlessly:
