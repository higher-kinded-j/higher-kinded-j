...
Read from multiple scoped values and combine:

```java
// Read two values and combine
Context<?, RequestInfo> gatherRequestInfo =
    Context.ask(AppContext.TRACE_ID).flatMap(traceId ->
        Context.ask(AppContext.LOCALE).flatMap(locale ->
            Context.ask(AppContext.TENANT_ID).map(tenant ->
                new RequestInfo(traceId, locale, tenant))));

// Using a helper for cleaner syntax
public static <A, B, C> Context<?, C> map2(
        Context<?, A> ca,
        Context<?, B> cb,
        BiFunction<A, B, C> f) {
...
Context<?, String> combined = map2(
    Context.ask(AppContext.TRACE_ID),
    Context.ask(AppContext.LOCALE),
    (traceId, locale) -> traceId + " / " + locale.toLanguageTag());
```
...
```java
@Test
void shouldComposeMultipleContexts() {
    Context<?, RequestInfo> ctx =
        Context.ask(TRACE_ID).flatMap(traceId ->
            Context.ask(LOCALE).map(locale ->
