...
| A PATCH bean's fields must start out `null`, which is not checked: an initialised field reads its default as sent. Law-check a sparse spec with a freshly constructed bean as the all-absent wire and a current value unlike any default, which catches it | [Beans and Sparse PATCH](beans_patch.md#sparse-patch-write-back-updatespec) |
