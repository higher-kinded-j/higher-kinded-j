...
| An **array** component is mutable and has identity `equals`: an identity leg shares the array with the other side (a `List` can at least be immutable), and `MappingLaws` cannot law-check a record carrying one - assert the round trip elementwise | [Law-checking](#law-check-the-generated-mapping) |
