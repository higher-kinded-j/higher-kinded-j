...
| An **array** component has identity `equals`, and an identity leg hands over a clone, so a record carrying one is never equal to its own round trip and `MappingLaws` cannot law-check it - assert the round trip elementwise | [Testing with hkj-test](../tooling/test_assertions.md#optic-laws) |
| An identity container is copied only where its declared type can be: a subtype (`ArrayList`, `TreeSet`), any other interface (`Deque`), a type variable, or an element declared through a wildcard is handed over as it is | [What an identity leg hands over](basics.md#what-an-identity-leg-hands-over) |
