...
Code that calls a mapping more than once binds it once and reuses it, as `personMapping` does above. Where it lives depends on how widely it is used: a local for a few calls in one method, a `private static final` field in the class that owns the boundary, or an injected bean when callers should depend on the mapping rather than on the generated class ([Injecting and testing generated mappings](testing.md#injecting-and-testing-generated-mappings)). Bind it in the code that calls it, not as a constant on the spec itself; [the fine print](#bind-in-the-caller) says why.
...
MapStruct's idiom declares a mapper's instance on the mapper's own interface. The same move here, `PersonMappingImpl MAPPER = PersonMappingImpl.INSTANCE;` declared on `PersonMapping`, compiles but can read `null`. The Impl implements the spec, and once the spec has a `default` method (every leaf and every derived field is one) the JVM initialises the spec as part of initialising the Impl. When a program touches `PersonMappingImpl` before `PersonMapping`, the spec's constant is evaluated while the Impl's own `INSTANCE` is still unassigned, and it keeps that `null` for good. Which of the two a program touches first depends on its code paths, so the failure comes and goes. A local, a field in the calling class or an injected bean has no such cycle.

---

~~~admonish info title="Key Takeaways"
* **A mapping is an interface you own**: `@GenerateMapping` on a `MappingSpec<Domain, Wire>` generates `<Spec>Impl` with `build` and `parse`; bind its `INSTANCE` once in the calling code and reuse it
