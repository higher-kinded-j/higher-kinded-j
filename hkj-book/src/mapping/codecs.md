...
| `@Flatten` marker | the **domain**, by the method's name | inert, except on a sealed pair, or on an [`UpdateSpec`](beans_patch.md#sparse-patch-write-back-updatespec) whose PATCH bean carries the group's inner properties |
