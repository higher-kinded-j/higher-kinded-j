...
_What each refusal from the mapping processor means, and the code that answers it._

The processor refuses a spec it cannot write lawful code for, at the declaration, with a message that says what is wrong, why, and what to write. Find the fragment your compiler printed in the tables below, and follow it to its entry. Each entry gives the fix in the open; the full message, and a declaration that produces it, fold beneath. The rule behind a message is on [Rules and Limits](rules.md).

~~~admonish info title="Error or note?"
Every message on this page is an **error**, which stops the build, bar one **note**, which stops nothing: it says an annotation changed nothing. Each full message is the processor's own output for the declaration beside it, and the build recompiles every declaration and checks that its message still carries the fragment the heading quotes.
~~~
...
**Spec members** ([entries](#spec-members)):
...
| [`names no component of`](#leaf-names-no-component) | A leaf's name matches no component, usually a typo |
...
| [`which cannot be reached from`](#cannot-be-reached) | A member names a type the spec's package cannot see |

**Optional fields** ([entries](#optional-fields)):
...
**Shared vocabulary** ([entries](#shared-vocabulary)):
...
**Bean wires** ([entries](#bean-wires)):

| The message says | What it means |
|---|---|
| [`does not support on the domain side`](#bean-domain) | The domain is a bean, not a record |
| [`but no getter, so the mapping leaves it out`](#setter-with-no-getter) | An accessor has no partner |
...
**Sparse PATCH** ([entries](#sparse-patch)):
...
**Generic specs** ([entries](#generic-specs)):
...
**Merge and error envelopes** ([entries](#merge-and-error-envelopes)):
...
**At your call site** ([entries](#at-your-call-site)):

| The message says | What it means |
|---|---|
| [`cannot find symbol`](#no-impl-class) | The generated Impl does not exist |
| [`cannot find symbol`](#no-such-surface) | The tier does not offer that method |
...
```mermaid
flowchart TD
    S["Your spec"] --> P{"Can the processor<br/>write lawful code for it?"}
    P -->|no| R["Refused at the spec:<br/>most of this page"]
    P -->|yes| G["Impl generated"]
    G --> J{"Does javac accept<br/>the generated Impl?"}
    J -->|no| X["An error inside an Impl<br/>you did not write"]
    J -->|yes| C{"Does your call site<br/>use a surface the Impl has?"}
...
Most messages come from the first branch: the processor reads your spec, finds a shape it cannot map lawfully, and says so where you declared it. An error inside a generated `*Impl` file is the processor's gap rather than your mistake, because it aims to refuse every such spec at the declaration; report it with the spec that caused it. A `cannot find symbol` at your own call site means the Impl is missing, or does not offer the method you called.
...
~~~admonish example title="The full message, and a declaration that produces it" collapsible=true
...
**Fix.** Add the leaf the message spells out, or map the pair with a spec of its own. A `List` against a `Set` lands here too: a container lifts only when both sides declare the same one.
...
**Fix.** Rename it after the component it parses (the message names the nearest one when it is close), or make it `private` or `static` if it is a helper.
...
A wire smaller than the domain also declares a derived field, and the write-back could never keep a value `build` recomputes.
...
A leaf, rename or marker declares its own `<R>`, which the generated Impl has nowhere to declare.
...
**Fix.** Add the `@OptionalBridge` marker the message spells out. The whole-`Optional` leaf it offers second also compiles, but keeps a `null` a located error.
...
### `@OptionalBridge on 'x' is redundant on a bean wire` {#redundant-on-a-bean-wire}

A bean wire bridges a domain `Optional` without the marker, so the annotation changes nothing.

**Fix.** Remove it, or keep it where a shared vocabulary also serves a record wire.
...
~~~admonish example title="The full message, and a declaration that produces it" collapsible=true
<!-- verify:rejects "which the spec extends raw" -->
```java
interface Renames<T> {
  @MapField(to = "fullName")
  T name();
...
**Fix.** Remove the whole-map leaf so the keys convert and the values copy, or remove the key leaf to keep converting the map whole.
...
A sealed dispatch has a domain subtype that no spec maps, so the dispatch would be partial.
...
**Fix.** Give the inner pair a spec of its own, against a nested wire component, or flatten it into the outer record itself.
...
### `bean property 'x' on 'Y' has a setter, setX(T), but no getter, so the mapping leaves it out` {#setter-with-no-getter}
...
**Fix.** Add the missing accessor, or correct the misspelt one the message names. On a PATCH bean, a setter meant to stay out takes an `@Unmapped` marker.

The rule: [When an unpaired accessor is refused](rules.md#unpaired-accessors).

~~~admonish example title="The full message, and a declaration that produces it" collapsible=true
<!-- verify:rejects "but no getter, so the mapping leaves it out" -->
```java
...
A PATCH property is a primitive, which always carries a value, so an omitted field could not be told apart.
...
A generic spec maps a bean or a PATCH wire, and generic mappings are record-to-record.

**Fix.** Map a concrete instantiation here, or use record types on both sides.
...
The generated Impl does not exist, most often because the spec lacks `@GenerateMapping` or the processor did not run.

**Fix.** Annotate the spec, and check that `hkj-processor` is on the annotation processor path, not only the compile classpath. Where the first mention is `CustomerMappingImpl.INSTANCE`, javac words the same cause as `package CustomerMappingImpl does not exist`.
...
The Impl has no such method because the pair cannot lawfully support it: a leaf withholds `asIso()`, and a projection has no `parse`.

**Fix.** Use the surface the tier offers; the tier table says which one your spec gets.

The rule: [The Emission Tiers](tiers.md).
