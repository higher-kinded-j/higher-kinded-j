...
- **Inherited vocabulary the sparse tier cannot use stays inert.** A derived field or an `@OptionalBridge` marker arriving from a [mix-in](codecs.md#shared-vocabulary-mix-in-interfaces) is never consulted here, so one vocabulary serves a full spec and its PATCH sibling; declaring either on the `UpdateSpec` itself is still an error, reported where it was written.
