...
- **Every PATCH bean field must start out `null`, and this rule cannot be checked for you.** Absence is read, never declared: `updateFrom` counts a property as sent whenever its getter answers anything but `null`. A field initialiser (`private List<String> tags = new ArrayList<>()`, `private String status = "ACTIVE"`) makes every request that omits the field arrive carrying the default, and the edit writes it over the domain value with nothing failing. It is the primitive's problem again, with a reference type, but no signature shows an initialiser, so the processor cannot refuse it. Leave PATCH bean fields uninitialised; for a generated DTO, configure the generator to leave containers `null` (openapi-generator's `containerDefaultToNull`, for one). The sparse laws below catch an initialiser when the all-absent wire is a freshly constructed bean, which is what a binder makes of an empty body.
