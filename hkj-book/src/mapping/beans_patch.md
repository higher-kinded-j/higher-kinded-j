...
The precise rules for bean wires, from which accessors pair to what a getter-only `List` must declare, are in [Rules and Limits](rules.md#bean-wires).
...
- [**A spec extending both `MappingSpec` and `UpdateSpec` is rejected.**](rules.md#one-tier-per-spec)
- [**A primitive wire property is rejected.**](rules.md#no-primitive-patch-property)
...
- [**A bean read one way only is rejected.**](rules.md#patch-bean-read-and-written)
- [**A setter with no getter is rejected.**](rules.md#every-patch-setter-has-a-getter)
...
- [**An inherited derived field or `@OptionalBridge` marker stays inert.**](rules.md#inherited-vocabulary-on-a-patch)
...
- [**A same-typed nested record, `Optional`, `List` or `Map` replaces wholesale**](rules.md#patch-replaces-wholesale) through identity, the fallback when no more specific leaf applies. Deep merge is out of scope.
