...
- **A shared vocabulary crosses a module boundary** ([#844](https://github.com/higher-kinded-j/higher-kinded-j/issues/844)): `@MapField`, `@OptionalBridge` and `@MapKey` were discarded after compilation, so a [mix-in vocabulary](mapping/codecs.md#shared-vocabulary-mix-in-interfaces) published by one module lost exactly the members that gave it meaning. A downstream spec extending it from a jar saw bare methods: a rename was refused as "neither a rename, a leaf, nor a bridge", and a `@MapKey` leaf whose two key types already agreed simply stopped validating, with no diagnostic at all. Leaves and derived fields, which carry no annotation, crossed intact, so the pattern worked right up until a vocabulary carried an annotated member.
  ...
  All three are now retained in the class file, joining `@Flatten`, which already was. A vocabulary interface means the same thing to a dependent compilation as to its own, and the publishing module needs only `hkj-annotations` on its compile path, not the processor, since a mix-in is a plain interface rather than a spec. One API module can own the house vocabulary that every service module's specs extend. See [Across modules](mapping/structure.md#across-modules).

  _One note for existing builds:_ these annotations now occupy a few bytes in the class files of any module that declares them. They are not visible to reflection (`CLASS`, not `RUNTIME`), and nothing reads them at runtime.
