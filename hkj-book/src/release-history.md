...
- **An empty `Optional` writes `null` to a bean, so the bean's own defaults no longer read back as present** ([#871](https://github.com/higher-kinded-j/higher-kinded-j/issues/871)): a domain `Optional<T>` mapped onto a bean property `T` skipped the write when empty, so the property kept whatever the bean was constructed with. A field initialiser such as `private String status = "ACTIVE"`, or a builder's own default, then came back through `parse` as `Optional[ACTIVE]`, and `MappingLaws` failed the round trip. `build` now hands the setter or builder setter `null`, as a record wire's constructor already received it, so the built bean no longer depends on how it was constructed.
  ...
  The same default defeats a sparse `UpdateSpec`, where there is no write to correct it: a PATCH bean field declared `private List<String> tags = new ArrayList<>()` reads as a present empty list when the client omits it, and `updateFrom` replaces the domain's list with it. No signature shows an initialiser, so this cannot be refused. The book, the `UpdateSpec` javadoc and the refusal for a getter-only `List` now say that a PATCH bean's fields must start out `null`, not only that the property needs a setter, and the sparse law examples take a freshly constructed bean as the all-absent wire, which is what a binder makes of an empty body and which catches an initialiser in your own tests wherever the current value differs from the default. See [Bean-shaped wire targets](mapping/beans_patch.md#bean-shaped-wire-targets) and [Sparse PATCH](mapping/beans_patch.md#sparse-patch-write-back-updatespec).
