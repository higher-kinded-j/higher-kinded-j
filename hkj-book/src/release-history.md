...
- **HKJ compiles under its own checker**: every module now builds with `-Xplugin:HKJChecker` (bar `hkj-checker` itself), at zero findings. Getting there fixed two false positives that had survived only because the checker had never been run over this codebase: a null guard such as `Objects.requireNonNull(path, "…")` was read as an effect built and dropped (it hands back the effect it was given — 101 sites in hkj-core alone), and `discarded-effect` judged every `Chainable` by the lazy-effect rule when `Chainable` is not uniformly lazy. The check now gates on a new sealed [`Deferred`](https://github.com/higher-kinded-j/higher-kinded-j/blob/main/hkj-core/src/main/java/org/higherkindedj/hkt/effect/capability/Deferred.java) capability covering the ten paths that actually defer, so `Path.just(1).peek(log)` — eager, `log` already ran — is no longer reported as a silent no-op, and a newly added path type has to choose a side. See [Compile-Time Checks](tooling/compile_checks.md).

- **`@EffectAlgebra` no longer requires the result type parameter to be named `A`** ([#699](https://github.com/higher-kinded-j/higher-kinded-j/issues/699)): the generated `*Ops` declared its own `<A>` but copied each record component's type verbatim, so an algebra written `RenamedOp<T>` produced `Function<Unit, T> k` inside a method declaring `<A>` — `cannot find symbol: class T`, reported against a file the author never wrote. Only `*Ops` was affected; the other four generated types do not copy component types. The component's type variable is now rewritten to the one the generated method declares, at every depth: nested arguments, arrays and wildcards included. Three permit shapes that generated uncompilable source are now rejected at their declaration instead: one that drops the algebra's parameter, one that pins it to a concrete result (`record Fixed<T>(…) implements Op<String>`), and one that bounds it (`<T extends Number>`) — a bound cannot be carried, since the generated `Kind` and `Functor` range over every type. A bounded parameter on the algebra itself is rejected for the same reason.

- **`@ComposeEffects` generates typed composition support** ([#697](https://github.com/higher-kinded-j/higher-kinded-j/issues/697)): the generated `*Support` spelled its factories as raw `Inject` and `Functor`, with a `BoundSet` whose components were `Object` and whose type parameter went unused, behind a blanket `{"unchecked", "rawtypes"}` suppression. It now reads each field's `Class<XOp<?>>` to name `XKind.Witness`, so `injectX()` returns `Inject<XKind.Witness, <composed witness>>`, `functor(...)` takes one `Functor` per effect and returns the composed `EitherFFunctor`, and `BoundSet`'s components are each algebra's `Bound` at the composed type. Declaring the composed witness lets `InjectInstances` infer, so the generated code carries no casts and needs no suppression at all. See [Effect Handlers](effect/effect_handlers.md#composing-effects).

  _Behaviour change, observable:_ the last effect in a composition was wrapped in one `injectRightThen` too many at every arity — at arity 2 it injected a level below the composition entirely — so a program dispatching to it through the generated `*Support` mis-routed at runtime (`ClassCastException` in the interpreter). The nesting now matches the arity. Raw generated types are what hid this; with the witness declared, javac rejects the wrong nesting.

  _Breaking changes / migration:_ `@ComposeEffects` shipped in v0.4.1 documenting each field as a `Class<?>` reference; that shape is now **enforced** as `Class<XOp<?>>` naming an `@EffectAlgebra`, and anything else is a compile error — declare the algebra's type argument. An algebra with more than one type parameter cannot take part, because its witness would itself be generic; `ErrorOp`/`StateOp` are hand-written for that reason and were never `@EffectAlgebra`. `BoundSet<F>` becomes `BoundSet`: drop the type argument, or use `var`. Binary compatibility: `functor()`'s erasure changes from `Functor` to `EitherFFunctor`, and `BoundSet`'s constructor and accessors from `Object` to typed `Bound`, so code compiled against a previously generated `*Support` must be recompiled. `injectX()` is binary-compatible. A `*Support` is generated in your own build from your own annotated record and never ships inside an HKJ artefact, so for most projects the rebuild is the whole migration.

- **The checker stops flagging code that cannot act on it**: `hkj-checker` no longer reports on types annotated `org.higherkindedj.optics.annotations.Generated`, so `@EffectAlgebra`'s generated `*Ops` class is no longer told to use the generated `*Ops` smart constructors, and `@ComposeEffects`' generated Support class is no longer told to use `@ComposeEffects`. A single declaration now opts out with `@SuppressWarnings("<check-id>")`, or out of every check with `@SuppressWarnings("hkj-checker")` — until now `disable=` was project-wide and there was no way to accept one deliberate call site. Suppression is per check and accumulates outward-in; javac's blanket `"all"` is deliberately not honoured. A file with several top-level types is also now scanned once per type rather than once per type *per type*, which removes duplicated diagnostics. See [Compile-Time Checks](tooling/compile_checks.md#opting-out-one-declaration).
