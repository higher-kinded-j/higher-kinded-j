...
**Path-native resilience: the full `with*` vocabulary on every carrier, railway-aware**

Retry, timeout, circuit breaker, and bulkhead now speak one `with*` vocabulary across the Path family: `IOPath` gains `withTimeout`/`withCircuitBreaker`/`withBulkhead` beside its existing retry, `VTaskPath` gains `withTimeout`, and `VResultPath` gains `withRetry`/`withCircuitBreaker`/`withBulkhead` beside its typed `withTimeout`, all instance-chained, because resilience wraps a *computation*. On the eager `EitherPath` the same combinators are static, taking the step as a `Supplier`. The typed carriers share a single railway-aware lowering: a business `Left` is a value (never retried by default, and never trips a circuit breaker), while `withRetry(retryOn, policy)` opts selected transient typed errors in (exhaustion returns the *last* `Left`, staying on the typed channel) and the `onTimeout`/`onOpen`/`onFull` overloads land timeouts, open-circuit rejections, and bulkhead rejections as typed `Left`s instead of thrown exceptions. Losing computations are not interrupted by a timeout; this is documented loudly, along with the rule that non-idempotent steps (payment) must never sit inside retry; the order example's `ConfigurableOrderWorkflow` now composes these core combinators directly, retrying only its idempotent pre-flight and running the commit exactly once ([#607](https://github.com/higher-kinded-j/higher-kinded-j/issues/607)). See [Resilience Patterns](resilience/ch_intro.md).
