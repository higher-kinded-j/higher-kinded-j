...
- `Each.eachWithIndex()` is **deprecated for removal in 0.5.0**. It still works as a bridge (so existing code compiles), but new code should narrow to `EachIndexed` and call `indexedTraversal()`. `Each.supportsIndexed()` now reports `this instanceof EachIndexed`.
