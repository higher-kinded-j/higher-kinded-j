...
- **A client keeps the `@OnStatus` overrides it inherits from a jar** ([#849](https://github.com/higher-kinded-j/higher-kinded-j/issues/849)): an `@HkjHttpClient` interface extending a base interface from another module now generates the same per-status decoding as one whose base is compiled alongside it. `@OnStatus` and its repeated-annotation container were discarded after compilation, so a method read from a class file carried no overrides: the generated client decoded a 404 into the declared error type rather than the mapped subtype, and nothing was reported. Both annotations are now retained in the class file, and an inherited override is checked just as a local one is.

  A problem with an inherited method, whether in an override or in its return type, is reported on the client interface and names the method and the interface it came from, since a method read from a class file has no file or line of its own to point at. An override naming a type that is missing from the client's classpath is reported as missing. See [Declarative HTTP clients](spring/declarative_http_clients.md#1-per-method-onstatus).

  _Two notes for existing builds._ A base interface compiled against an earlier release has already lost its overrides, so rebuild it to pick them up. And an inherited override is now checked where before it was silently ignored, so one naming a type that is not assignable to the method's declared error type, or one missing from the client's classpath, fails a build that previously compiled.
