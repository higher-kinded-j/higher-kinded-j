...
`@GenerateMapping` now maps generic records three ways. A **concrete instantiation** (`extends MappingSpec<Page<User>, PageDto<UserDto>>`) classifies every component under the substitution, so leaves, nesting, container lifting and the null doctrine apply unchanged. A **threaded spec** (`PageMapping<T> extends MappingSpec<Page<T>, PageDto<T>>`) generates one generic Impl serving every instantiation behind the `instance()` generic-singleton convention (`EitherMonad.instance()`). An **element-mapped spec** threads the two sides under different variables and declares the element mapping as an abstract `ValidatedPrism<TDto, T>` leaf; the generated Impl takes one prism per leaf through a public `of(...)` factory (no singleton: the Impl carries the prisms). All three nest: instantiated mappings register like any other, threaded specs resolve at use sites by type-argument unification, and element-mapped specs compose in place (`of(entries())`), with failures located through the whole path (`entries.items.1`). Generic mappings stay record-to-record only; raw and wildcard uses are diagnosed with what/why/fix pointers, while array arguments are concrete and unify structurally. See [Generic records](optics/record_mapping.md#generic-records-concrete-threaded-and-element-mapped).

**Shared vocabulary: mix-in interfaces for specs ([#623](https://github.com/higher-kinded-j/higher-kinded-j/issues/623))**

A `@GenerateMapping` spec may now extend plain **mix-in interfaces** alongside `MappingSpec`/`UpdateSpec`, sharing renames, leaves and derived fields across specs. Inherited members count as if declared on the spec, with Java's own precedence (a local override hides the mix-in's member; agreeing renames from unrelated mix-ins fold into one, conflicting targets are diagnosed), and diagnostics about inherited members name their declaring interface. A mix-in that is itself a mapping spec, or a generic one, is rejected with what/why/fix diagnostics; `@GenerateMerge` specs still declare everything directly. See [Shared vocabulary](optics/record_mapping.md#shared-vocabulary-mix-in-interfaces).
