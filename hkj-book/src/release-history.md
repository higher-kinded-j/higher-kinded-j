...
Following the same pattern, a group of HKJ-owned types that still wrapped `widen` in an internal `*Holder` record now extend their `Kind` interface directly. `widen` becomes an allocation-free upcast (no wrapper object), `narrow` is a plain type check, and twelve `*Holder` records are deleted ([#568](https://github.com/higher-kinded-j/higher-kinded-j/issues/568)).

- Converted: `Try`, `Reader`, `State`, `Writer`, `Trampoline`, `Context`, `Free`, `Coyoneda`, `StateOp`, `Const` — including the `@FunctionalInterface` types `Reader` and `State`, which stay single-abstract-method because `Kind` is an empty marker. Each `narrow` drops its `rawtypes` suppression.
- `Lazy` deliberately keeps its holder: it is a mutable memoising thunk, and the immutable holder keeps the `Kind` carrier immutable (an architecture invariant enforced by `ImmutabilityRules`). JDK-wrapped types (`Optional`, `List`, `Stream`, `CompletableFuture`) keep theirs by necessity, and a few internal applicative/algebra carriers (`FreeAp`, `ErrorOp`, `EitherF`, `Fetch`, `OpticOp`) are left for a follow-up.
- Behaviour-preserving for normal use: a widened `Kind` is now the value itself rather than a wrapper (`narrow(widen(x))` still round-trips); `Free.widen` additionally rejects null now, matching the rest of the family. Tests that asserted the holder type were updated. Purely internal; no public API change.
