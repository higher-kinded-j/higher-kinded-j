...
- **A dependency's `@GenerateFocus` record is navigable** ([#847](https://github.com/higher-kinded-j/higher-kinded-j/issues/847)): navigator generation asks a field's type whether it carries `@GenerateFocus`, and the annotation was discarded after compilation, so a record annotated in another module could not answer. The field fell back to a plain `FocusPath` and nothing was reported, which meant `CompanyFocus.headquarters().street()` compiled when `Address` was a sibling source file and did not exist when the same record arrived as a jar.

  `@GenerateFocus` and `@TraverseField` are now retained in the class file, so a dependency's record answers exactly as a source file does: the same navigator, and the same path type per component. Both were needed together, since a navigation method declares what the target's own `Focus` method returns and `@TraverseField` is what decides that for a `Kind` component. A target whose module did **not** run the processor keeps the plain path, because navigating into it would compose a companion class nobody generated. See [Which fields get a navigator](optics/focus_navigation.md#which-fields-get-a-navigator).

  _One note for existing builds._ A published jar whose `Focus` classes were generated before this change does not carry the navigator inner classes a consumer now expects to compose, so rebuild dependencies rather than mixing generations. Within one build nothing is needed: everything recompiles together. `@GenerateFocus`'s element values also become part of a published record's compiled surface, so editing `generateNavigators`, `excludeFields` or `maxNavigatorDepth` upstream now changes what downstream generates, where previously it could not be seen at all.
