...
- **`@ComposeEffects` generates typed composition support** ([#697](https://github.com/higher-kinded-j/higher-kinded-j/issues/697)): the generated `*Support` spelled its factories as raw `Inject` and `Functor`, with a `BoundSet` whose components were `Object` and whose type parameter went unused, behind a blanket `{"unchecked", "rawtypes"}` suppression. It now reads each field's `Class<XOp<?>>` to name `XKind.Witness`, so `injectX()` returns `Inject<XKind.Witness, <composed witness>>`, `functor(...)` takes and returns typed `Functor`s, and `BoundSet`'s components are each algebra's `Bound` at the composed type. Declaring the composed witness lets `InjectInstances` infer, so the generated code carries no casts and needs no suppression at all. The field shape the annotation always documented is now enforced rather than assumed, which is what made the witness knowable. Typing it also surfaced a latent nesting bug: the last effect was wrapped in one `injectRightThen` too many at every arity — at arity 2 it injected one level below the composition — so any program dispatching to the last algebra through the generated Support would have mis-routed. See [Effect Handlers](effect/effect_handlers.md#composing-effects).
