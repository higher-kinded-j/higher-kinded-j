...
`@GenerateMapping` now maps generic records as **concrete instantiations**: `extends MappingSpec<Page<User>, PageDto<UserDto>>` classifies every component under the substitution, so leaves, nesting, `List`/`Optional`/`Map` lifting, the null doctrine and index location all apply unchanged, and the instantiated mapping registers for nesting like any other (`Report(Page<User> results)` resolves it automatically). Concrete generic instantiations are supported for record-to-record mappings only (bean-shaped wires and `UpdateSpec` mappings do not support them yet); raw uses — including raw nested arguments — wildcard arguments and generic *specs* (`PageMapping<T>`) are diagnosed with what/why/fix pointers, while array arguments are concrete and map fine. Threaded specs land in the same release: `PageMapping<T> extends MappingSpec<Page<T>, PageDto<T>>` generates one generic Impl serving every instantiation behind the `instance()` generic-singleton convention (`EitherMonad.instance()`), with multi-parameter and bounded variables threaded through `build`/`parse` and a lossless pair's `asIso()`. A threaded spec is not yet nestable, and the element-mapped form (an abstract `ValidatedPrism<TDto, T>` leaf for `Page<T> ↔ PageDto<TDto>`) stays diagnosed — both arrive with the element-mapped slice.
