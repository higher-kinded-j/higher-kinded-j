...
- **Generated optics and companions compile under `-Werror`** ([#867](https://github.com/higher-kinded-j/higher-kinded-j/issues/867)): a generic type written without its type arguments gave generated code that failed a build run with `-Xlint:rawtypes` and `-Werror`, in a file the `@SuppressWarnings("rawtypes")` on your own declaration cannot reach. It could sit in a record component, `List tags` or `Optional<List> contacts`; in the bound of a type parameter, `record Doc<T extends List>`; in the `Iso` a `@GenerateIsos` method returns; or in a variant's or a context record's component under `@GenerateErrorEnvelope`.

  `@GenerateLenses`, `@GenerateGetters`, `@GenerateSetters`, `@GenerateFolds`, `@GenerateFocus` and `@GenerateTraversals` write the component type, and the bounds of the type parameters they redeclare, into the members they generate; a fold's and a traversal's implementation class names that type again in its superinterface clause. `@GeneratePrisms` writes the bounds a permitted subtype declares. A `@GenerateFocus` navigator writes the field types of the record it navigates into. `@GenerateAssembly` writes a component into each stage's field, constructor and method, and infers them all in `assemble`. Each generated member that writes out or infers such a type now carries that suppression itself, in the same way the generated mapping `Impl` does.
