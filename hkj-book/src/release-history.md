...
- **`@GenerateMapping` lifts `Set`, array and `Map`-key components** ([#675](https://github.com/higher-kinded-j/higher-kinded-j/issues/675)): element lifting covered `List`, `Optional` and a `Map`'s values. A `Set<EmailAddress>` component could only be copied whole or mapped through a hand-written whole-container leaf, an array could not lift at all, and a `Map` whose two sides keyed differently was a hard error. All three now lift through the leaf or nested spec named after the component, in both directions and on every tier - dense, projection, bean wire and sparse `UpdateSpec` - and a same-typed `Set` or reference array carries the identity null scan the other containers already did.
  ...
  _One source-compatibility note:_ `parseAll` and `buildAll` are now overloaded on the container, so a call passing a bare `null` literal needs the type it stands for (`parseAll((List<String>) null)`). Every call passing an actual container is unaffected.
