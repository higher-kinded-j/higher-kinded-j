...
`@GenerateMapping` now maps a record domain to a **bean-shaped wire**, not only a record: a mutable class with a no-args constructor and getters/setters, or an immutable one with a builder (`builder()`/`newBuilder()`), with the JAXB getter-only collection convention (`getItems().addAll(...)`) also recognised. The full feature set carries over: renames, leaves, derived fields, `List`/`Optional`/`Map` lifting, and nesting both ways. Because an unset bean property is `null`, every reference-typed `parse` read is null-guarded into a located `FieldError`, so `parse` stays total and accumulating; those guards make `asIso()` truthful only for an all-primitive bean. A domain `Optional<T>` bridges to a nullable bean property `T` (empty to absent). See [Bean-shaped wire targets](optics/record_mapping.md#bean-shaped-wire-targets). Read-only/write-only bean tiers and Optional-through-nested-spec bridging remain follow-ons; the validated-`patch` projection tier ships for record wires in this same release ([#625](https://github.com/higher-kinded-j/higher-kinded-j/issues/625), below).

**Sparse PATCH write-back: `UpdateSpec` ([#645](https://github.com/higher-kinded-j/higher-kinded-j/issues/645))**

`@GenerateMapping` gains a sparse sibling of the full mapping: a spec extending `UpdateSpec<Domain, Wire>` (bean wire only) generates `Edits.Accumulated<Domain> updateFrom(Wire)`, the REST PATCH contract as a validated fold. A `null` bean property means absent, leave unchanged; a present property is set, or parsed through its leaf into a located `FieldError`, with every invalid present field reported at once and the writes applied only if all validate. Coverage is one-sided (every wire property needs a domain target, while unmapped domain components such as an `id` simply never change), a present nested or container value replaces its component wholesale, and shapes that cannot honour the null-as-absent contract (record wires, primitive properties, `Optional`-bridged components, sealed hierarchies) are rejected with what/why/fix diagnostics. The sparse tier is law-checked through a `MappingLaws` overload (identity on the all-null wire, idempotence, composition under `Edits.combine`), and the example app gains a worked `PATCH /api/users/{id}` endpoint ([#647](https://github.com/higher-kinded-j/higher-kinded-j/pull/647)). See [Sparse PATCH write-back](optics/record_mapping.md#sparse-patch-write-back-updatespec).

**Generic records: concrete instantiations and threaded specs ([#624](https://github.com/higher-kinded-j/higher-kinded-j/issues/624))**

`@GenerateMapping` now maps generic records two ways. A **concrete instantiation** (`extends MappingSpec<Page<User>, PageDto<UserDto>>`) classifies every component under the substitution, so leaves, nesting, container lifting and the null doctrine apply unchanged, and the instantiated mapping nests into siblings like any other. A **threaded spec** (`PageMapping<T> extends MappingSpec<Page<T>, PageDto<T>>`) generates one generic Impl serving every instantiation behind the `instance()` generic-singleton convention (`EitherMonad.instance()`), threading multi-parameter and bounded variables through `build`/`parse` and a lossless pair's `asIso()`. Both are record-to-record only (bean-shaped wires and `UpdateSpec` mappings stay concrete); raw and wildcard uses are diagnosed with what/why/fix pointers, while array arguments are concrete and map fine. A threaded spec is not yet nestable, and the element-mapped form (an abstract `ValidatedPrism<TDto, T>` leaf) stays diagnosed; both remain follow-ups under the same issue. See [Generic records](optics/record_mapping.md#generic-records-concrete-instantiations-and-threaded-specs).

**Shared vocabulary: mix-in interfaces for specs ([#623](https://github.com/higher-kinded-j/higher-kinded-j/issues/623))**

A `@GenerateMapping` spec may now extend plain **mix-in interfaces** alongside `MappingSpec`, sharing renames, leaves and derived fields across specs: an inherited member counts exactly as if declared on the spec, collected across the whole hierarchy with Java's own precedence (a local override hides the mix-in's member; a diamond counts once; unrelated mix-ins agreeing on an abstract rename fold into one declaration, while conflicting rename targets are diagnosed naming both interfaces). Interface statics stay inert, threaded generic specs and `UpdateSpec` mappings inherit the same way, and diagnostics about an inherited member name its declaring interface. Two shapes are rejected with what/why/fix diagnostics: a mix-in that is itself a mapping spec (vocabulary and Impl-generating specs stay distinct concepts), and a generic mix-in (substitution under an instantiation is not supported yet). `@GenerateMerge` specs still declare everything directly. See [Shared vocabulary](optics/record_mapping.md#shared-vocabulary-mix-in-interfaces).

**Index-located container failures and element nulls ([#660](https://github.com/higher-kinded-j/higher-kinded-j/issues/660))**

The null doctrine now reaches *inside* containers: `ValidatedPrism.parseAll` locates every failing element by its index — a plain positional segment matching `parseValues`' key grammar (bracketed rendering such as `emails[1]` is deliberately deferred to the future sealed path-segment model) — and a null element or map value is a located, accumulating `must not be null` at its index or key, never an exception. **Behaviour changes**, both observable: list-element failures previously carried no index (`emails: not an email address` becomes `emails.1: not an email address` — 422 payload paths gain the segment), and a null element/value previously threw `NullPointerException` out of `parseAll`/`parseValues`. A null *list*, *map* or map *key* stays the caller's `requireNonNull` (a null key is a structurally broken map, not a wrong value); the build direction (`buildAll`/`buildValues`) remains total-and-throwing.
...
Record-wire `parse` retires the "a record can never read null" premise: a JSON binder leaves a missing property `null` on a record component just as on an unset bean property, so every reference-typed `parse` leg is now null-guarded into a located, accumulating `FieldError` (`must not be null`) — one null doctrine across both wire shapes, and nulls locate through nesting (`customer.name: must not be null`) so the [422 leg](spring/spring_boot_integration.md#the-422-leg) stays a 422 instead of a 500. **Behaviour change:** `parse` on a null-carrying record wire previously threw (`NullPointerException` out of `Validated` or a leaf); it now returns `Invalid` — the friendly direction, but observable if code relied on the throw. The Iso tier is deliberately asymmetric: a bean's guarded reads stay fallible (an unset property is a representable state, so `asIso()` remains all-primitive-only), while a lossless *record* mapping keeps `asIso()` — its guards exist for hostile bindings, and the parse-iso coherence law is scoped to wires whose reference components are non-null. A null wire itself remains the caller-contract `requireNonNull` (nulls *inside* container values are located too — [#660](https://github.com/higher-kinded-j/higher-kinded-j/issues/660), above). Spec methods that collide with generated Impl members are also now rejected with what/why/fix diagnostics at the spec ([#654](https://github.com/higher-kinded-j/higher-kinded-j/issues/654)), instead of being silently overridden or breaking the generated file's compile.

**The 422 leg: one response, every bad field by path ([#627](https://github.com/higher-kinded-j/higher-kinded-j/issues/627))**

A Spring controller returning a fallible mapper `parse` result (`Validated<NonEmptyList<FieldError>, Domain>`, directly or via `ValidationPath`) now renders as a single **422 Unprocessable Content** response listing every bad field: each error carries the dot-joined display `path`, the exact `segments`, and the `message`, honouring the dotted-key caveat (a key containing a dot is only ambiguous in the rendered `path`; `segments` stays lossless). The leg is selected purely by payload shape (an `Invalid` made entirely of located `FieldError`s), so `Path.fields()` assemblies and `@GenerateAssembly` results take it too; mixed, empty or other payloads keep the generic rendering. The status is configurable via `hkj.web.validation-field-error-status`, and the example app gains a `POST /api/users/parse` showcase over a bean-shaped wire DTO, where an absent field becomes a located `"must not be null"` error rather than an exception. See [The 422 leg](spring/spring_boot_integration.md#the-422-leg).

_Behaviour change:_ a pre-existing endpoint returning an all-`FieldError` payload moves from 400 to **422** on upgrade; set `hkj.web.validation-field-error-status: 400` to restore the old status. In the same change, tests and examples adopt Spring Framework 7's `HttpStatus.UNPROCESSABLE_CONTENT` (the RFC 9110 rename; `UNPROCESSABLE_ENTITY` is deprecated).

**Leaf-carrying projections: the validated `patch` tier ([#625](https://github.com/higher-kinded-j/higher-kinded-j/issues/625))**

A lossy `@GenerateMapping` projection (record wire with fewer components than the domain) may now carry fallible correspondences: whole-component leaves, nested specs, and `List`/`Optional`/`Map` lifting, resolved exactly like full-tier components. Any fallible correspondence swaps the total `asLens()` write-back for the **validated `patch` tier**: `patch(domain, wire)` returns `Validated<NonEmptyList<FieldError>, Domain>`, validating every projected component at once (a `null` projected read becomes a located `"must not be null"`; a null inside a *nested* wire value follows the nested spec's own parse contract) while unprojected components survive untouched by construction. Dense semantics, deliberately the opposite of `UpdateSpec`'s sparse null-as-absent `updateFrom`; the result is the [422 leg](spring/spring_boot_integration.md#the-422-leg)'s shape at a Spring boundary. Law-checked via a new `MappingLaws` overload (projection identity, idempotence, located validation); all-identity projections keep the lawful total `asLens()`, output unchanged. Two small behavioural notes: a leaf declared on a projected component now takes effect (previously it was silently ignored) and selects the patch tier, replacing `asLens()`; and `patch` joins `build` as a member name the generated Impl reserves on leaf-carrying projections. See [Leaf-carrying projections](optics/record_mapping.md#leaf-carrying-projections-the-validated-patch).
