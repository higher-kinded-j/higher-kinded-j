...
- [@GenerateMapping (Step-0)](optics/record_mapping.md): Annotate `interface UserMapping extends MappingSpec<User, UserDto>` and the processor generates a total `build` plus an accumulating `parse` returning `Validated<NonEmptyList<FieldError>, User>`. `@MapField` renames, nesting, `List`/`Optional` container lifting, sealed-interface dispatch, and truthful emission tiers (`asIso` / `asLens` / accumulating `parse`) all ship in this slice ([#600](https://github.com/higher-kinded-j/higher-kinded-j/issues/600))
