...
- **The mapping documentation becomes its own chapter**: the record-mapping material moved from a single page under Optics into a dedicated [Mapping at the Boundary](mapping/ch_intro.md) chapter (basics, codecs, structure, tiers, beans and sparse PATCH, generics, merge and envelopes, injection and testing), opening with a problem-first introduction and closing with a compiled, law-checked capstone: [One 422, Every Bad Field](mapping/capstone.md). The old `optics/record_mapping.html` URL redirects to the new chapter.
- **`@GenerateTraversals` sees a `Collection` component, and says so when it cannot see one** ([#756](https://github.com/higher-kinded-j/higher-kinded-j/issues/756)): `@GenerateTraversals` asks the `TraversableGenerator` SPI for each record component, and no generator claimed `java.util.Collection`, so `record Job(Collection<String> tags, List<String> notes)` generated `notes()` and nothing else — no error, no warning, and the gap surfaced as a missing method at the call site. `@GenerateFocus` had recognised `Collection` since #725, which left the two annotations disagreeing about a container. A `CollectionGenerator` now ships with the JDK generators, and the traversal it writes calls the new `Traversals.traverseCollection`, which `Traversals.forCollection()` now delegates to as well: a set source comes back an unmodifiable set in its own iteration order, and every other source comes back a list. The decision is made in that one helper rather than written into generated source, so a `Collection` reached through `@GenerateTraversals`, `@GenerateFocus`, `@ImportOptics` and `@ThroughField` rebuilds the same way. `@ThroughField` auto-detects a `Collection` field too — the interface itself, not a `Deque` or a `Queue`, which the rebuilt list could not be handed back to — and `@ImportOptics` names `Traversals.forCollection()` for it. The silence is fixed on its own terms: a component that holds elements but reaches no method — a `Collection` or `Map` subtype no generator on the processor path supports, or a claimed container whose element type cannot be read — is reported as a **note** on the component, in the what/why/fix format. A component that is not a container at all is passed over silently, as before. See [Collection Components](optics/traversals.md#collection-components) and [Compiler Errors](optics/compiler_errors.md#generatetraversals-no-traversal-was-generated-for-component-xy-of-type-dequet-a-note).

  _Behaviour change, a new note:_ a `@GenerateTraversals` record with a `Deque`, `Queue`, `SortedMap` or `ArrayList` component, or a raw `List` or `Set`, compiled silently in 0.4.9 and now compiles with a note at that component. It is a note rather than a warning on purpose: the annotation has no per-component opt-out and a processor warning cannot be suppressed, so a warning would have failed every `-Werror` build with no remedy short of changing the record. Nothing generated changes, and no build that passed fails. Declare the component as a container a generator supports — `List`, `Set`, `Collection`, `Map`, `Optional`, an array, or a plugin-supported type — or put a `TraversableGenerator` for its type on the annotation processor path. The limits `Traversals.forCollection()` states are inherited by the generated traversal: a `SortedSet` source keeps its elements but not its comparator, and a source that is neither a list nor a set comes back a list.

- **`@GenerateMapping` reads a generic mix-in under the spec's instantiation** ([#752](https://github.com/higher-kinded-j/higher-kinded-j/issues/752)): a mix-in declaring type parameters was refused at the declaration, with a message saying substitution "is not supported yet". It is supported now: a member inherited from `Emails<T>` is read as the spec has it, so `extends Emails<EmailAddress>` contributes `ValidatedPrism<String, EmailAddress>` and the generated Impl carries that rather than a name nothing declares. This is the shape a shared vocabulary interface exists for, and the previous remedy, declaring the members directly on each spec, cost exactly the reuse it was there to provide. Every place a spec member's type is read now goes through one helper: the two that reach generated code (the element-mapped leaf fields and the `@MapField` rename stubs), the three that compare a member against the instantiated pair (`findLeaf`, the derived-field `Getter` shape, and the hand-written-mapper heuristic, that last reading both halves of the signature), and two diagnostics where a free type variable had been leaking into the message text. A **generic** ancestor reached through a raw supertype is still refused where it contributes something to read, because erasure travels downwards and leaves the spec inheriting members it never declared; a non-generic one is untouched by erasure and is left alone. The diagnostic names the interface whose clause is raw, which is not always the spec's own, and a raw or wildcard argument on the spec's own mix-in clause is refused for the same reason its `MappingSpec` clause always was. See [Generic Specs](mapping/generics.md#generic-mix-ins).

  _Behaviour change, accepting where it refused:_ a spec extending a generic mix-in compiled with an error before and generates now, so nothing that worked stops working. The one direction to check is a spec that was relying on the refusal to catch a raw `extends` clause: that is still refused when the ancestor beneath it is generic, with the message changed from "is generic" to "is extended raw by ..."; a raw route to a non-generic ancestor is now accepted, since nothing of its is erased.

- **A `Set` or `Collection` component reaches a traversal that rebuilds it** ([#725](https://github.com/higher-kinded-j/higher-kinded-j/issues/725)): `@GenerateFocus` recognises five containers by name, and widened three of them — `List`, `Set` and `Collection` — through the same no-argument `.each()`. That method carries a `List` traversal behind an unchecked cast, so a `Set` component compiled and then threw `class java.util.HashSet cannot be cast to class java.util.List` on its first traversal, read or write; a `Collection` component threw the same way whenever the runtime value was not a `List`, and worked by accident when it was. No `Set`-valued Focus path has ever worked. The recognised-container table now pairs each type with the `Each` that rebuilds it: `List` keeps the no-argument `.each()`, a `Set` widens through `EachInstances.setEach()`, and a `Collection` through the new `EachInstances.collectionEach()`. The tiers are unchanged, so no generated signature moves. See [Custom Containers](optics/focus_containers.md#supported-container-types).

  _Behaviour change, source-breaking at the declaration:_ naming an `Each` gives the widening type arguments javac infers from the component, which is the same rule raw and wildcard-carrying SPI containers already fall under. A `Set` or `Collection` component that is raw, or carries a wildcard of any kind — `Set<?>`, `Set<? extends T>`, `Set<? super T>`, `Collection<?>` — is now rejected where it is declared, at any nesting depth the analysis reaches: `List<Set<?>>` and `Optional<Set<?>>` at the default settings, and `Map<String, Set<?>>` under `widenCollections = true`. Every one of those compiled in 0.4.9 and threw `ClassCastException` on the first traversal, so nothing that worked stops working. Name the type argument — `Set<Object>` for an unbounded wildcard, `Set<Leaf>` for `? extends Leaf` — or drop `@GenerateFocus` from the record and keep `@GenerateLenses` and `@GenerateTraversals`, which compose no optic instance and take the component as written. `Optional`, `Maybe` and `List` are unaffected and still take a wildcard silently. See [Compiler Errors](optics/compiler_errors.md#generatefocus-record-component-xy-has-a-wildcard-type-argument-in-set-extends-t).

  _Behaviour change, one set-rebuild policy:_ `EachInstances.setEach()` traversed through `Set.copyOf`, whose iteration order the JDK randomises per JVM run and which rejects a null element — so the same modification returned a differently-ordered set on each run, and `getAll`, which kept source order, disagreed with it. It now uses `Traversals.forSet()`, which every other annotation already emitted, so a `Set` reached through `@GenerateFocus`, `@GenerateTraversals`, `@ImportOptics` and `@ThroughField` rebuilds the same way: source iteration order preserved, nulls carried through, and — as `traverseList` already did — an **unmodifiable** set handed back. `Traversals.forSet()` and `Traversals.traverseSet` previously returned a mutable `LinkedHashSet`; code that mutated the result of a set traversal in place now meets an `UnsupportedOperationException`, and should build its own copy.

  _Also:_ `Traversals.forCollection()` and `EachInstances.collectionEach()` are new. A `Collection` says nothing about ordering or uniqueness, so they rebuild the shape they were handed — a set source comes back a set, anything else a `List`. Rebuilding every source as a list would let a modification that maps two elements onto one value leave duplicates in a collection that had none. Two limits follow from `Collection` being all the declaration says: a `SortedSet` source keeps its elements but not its comparator, and a source that is neither `List` nor `Set` — an `ArrayDeque`, a `PriorityQueue` — comes back a `List`, so the identity modification does not return an *equal* collection; those types inherit identity `equals`, and no rebuild could satisfy it.

- **A generated prism's focus has to be a variant of its source** ([#755](https://github.com/higher-kinded-j/higher-kinded-j/issues/755)): a prism runs both ways, and every prism generated from a spec interface builds back with identity — `value -> value`, the function `Prism.build` stands up. That is only a source when the focus is one, and nothing checked it. A focus that was a *value* rather than a variant, `Prism<JsonNode, String>`, reached javac as *"incompatible types: cannot infer type-variable(s) S,A,T,T"* inside the generated file, with no line of the spec interface in it and no diagnostic from the processor at all. The requirement belongs to the prism rather than to either hint, so `@InstanceOf` and `@MatchWhen` are now both held to it, at the declaration, in what/why/fix form. It is asked after the hint parses, so a method carrying no hint is still told that first — the nearer problem. See [Compiler Errors](optics/compiler_errors.md).

  _Documentation change:_ the shape was one the documentation recommended in three places, none of which compiled — `@MatchWhen`'s own javadoc (`Prism<JsonNode, String> text()`), the [Spec Interfaces](optics/optics_spec_interfaces.md) page (`Prism<Value, String> string()`), and that page's Protocol Buffers suggestion, where a oneof field's type is often a scalar. All three now focus a variant, and the page says why. Where the value type is genuinely the point, `Prism.of` with a real build side is the hand-written route; the spec interface has no way to name one.

  _Behaviour change, source-breaking:_ a spec declaring such a prism compiled in 0.4.9 only in the sense that the processor accepted it — the generated file never compiled — so nothing that ever built can break. What changes is where the error appears: on the offending method, naming the remedy, rather than in a file the author never wrote.

- **An `@InstanceOf` prism narrows to what its test can check** ([#733](https://github.com/higher-kinded-j/higher-kinded-j/issues/733)): the annotation takes a class constant, which is always raw, so a prism onto a parameterised target narrowed through a raw `instanceof` and handed the value back under whatever instantiation the caller asked for. `ShapeOptics.<String>circle()` matched a `Circle<Integer>`, returned a populated `Optional<Circle<String>>`, and threw `ClassCastException` on the first read of `tag()`. The generated test is now written under the type arguments the *source type* pins down — `Circle<X> implements Shape<X>` reached from `Shape<U>` pins `X` to `U`, so the test is `source instanceof Circle<U>`, which javac itself accepts as a checked one — and an unbounded wildcard wherever the source pins nothing, so `Prism<Shape, Circle<?>>` is generated as `source instanceof Circle<?>`. An array target is narrowed through its component, `List[].class` reaching `source instanceof List<?>[]`, because a class constant naming an array is exactly as raw as one naming its element type. A focus asking for more than that is refused at the declaration, naming both remedies: widen it to the wildcard, or narrow through `@MatchWhen`, which reads the argument off the source rather than inventing it. The `@SuppressWarnings({"unchecked", "rawtypes"})` those methods had been carrying is gone with the hazard it was hiding. See [Spec Interfaces](optics/optics_spec_interfaces.md#parameterised-targets) and [Compiler Errors](optics/compiler_errors.md#instanceof--declares-its-focus-as-circlet-which-the-test-cannot-narrow-to).

  _Behaviour change, source-breaking in three narrow places:_ a spec whose `@InstanceOf` focus names a type argument the source type does not pin — `Prism<Shape, Circle<T>>` on `OpticsSpec<Shape>`, or `Prism<Object, List<String>[]>` for an `@InstanceOf(List[].class)` — compiled with warnings in 0.4.9, and is now refused; widen the focus to `Circle<?>` or `List<?>[]`, or narrow through `@MatchWhen`. A prism whose focus does not accept the class the annotation names is refused too, where before it generated source javac rejected in a file the author never wrote. And a target that is a parameterised member of a *generic* type — `Outer<X>.Inner<Y>` — is refused, because the test cannot name it with its own arguments at all; declare the member `static`, or use `@MatchWhen`. Prisms whose arguments the source does pin are unaffected, and lose only the suppression.

- **`@GeneratePathBridge` emits a bridge the consuming build compiles** ([#748](https://github.com/higher-kinded-j/higher-kinded-j/issues/748)): the processor accepted several shapes it had no compiling rendering of, and the author met the result as an error in a file they never wrote. The whole `IO` row was non-functional: it mapped to `Path.io`, which takes a `Supplier`; the factory that takes an `IO` is `Path.ioPath`. So every `IO<T>` bridge the annotation advertised was source javac refused. A `Validated` whose error type is a wildcard emitted a `ValidationPath` and a `Semigroup` naming that type twice, which capture makes two different types. A raw type anywhere the bridge copies, `Optional` as a return type or `List` as a parameter, landed as a `[rawtypes]` warning in the one file the author's own `@SuppressWarnings` cannot reach. An inherited `@PathVia` was invisible, because the walk read enclosed elements rather than members, so a bridge for `StringStore extends Store<String>` was generated with a constructor and nothing else, silently. Members are now read under the annotated interface's own instantiation, an inherited `<R extends T>` becoming `<R extends String>` where the bridge can name it, and deduplicated so that two unrelated superinterfaces declaring one method do not produce a class that declares it twice. Along the way: a `throws` clause is carried across rather than dropped, varargs are kept rather than erased to an array, the delegate is called as `this.delegate` so a parameter named `delegate` no longer redirects the call to itself, an appended `Semigroup` takes a name no parameter has claimed, and a generic method's call carries an explicit type witness so a self-referential bound such as `<R extends Enum<R>>` still infers. Six shapes with no correct rendering are refused at the declaration instead, each with what, why and a remedy: see [Compiler Errors](optics/compiler_errors.md#generatepathbridge-and-pathvia).

  _Behaviour change, source-breaking in three narrow places:_ a varargs delegate's bridge method is now varargs rather than `T[]`, so an existing call passing a bare `null` for that parameter draws *"non-varargs call of varargs method with inexact argument type"*, a mandatory warning and therefore a failure under `-Werror`; write `(String[]) null`. A raw effect return type and a wildcard `Validated` error type previously generated source (uncompilable, or warning-laden) and are now refused at the declaration; name the type arguments. And an interface with no `@PathVia` method anywhere now draws a processor warning, which cannot be suppressed and so fails a `-Werror` build; the annotation is not inherited by an override, so a method overriding an annotated one needs its own. Bridges that already compiled are unaffected, apart from picking up any `@PathVia` methods they inherit.

- **`@ImportOptics` declares the type parameters the generated signature names** ([#730](https://github.com/higher-kinded-j/higher-kinded-j/issues/730)): a generated optic method took its type parameters from the **source type's own declaration** while building the rest of the signature from the instantiated source type, so the two agreed only by name coincidence. A spec that renamed them — `interface BoxOpticsSpec<U> extends OpticsSpec<Box<U>>` — emitted `static <T> Lens<Box<U>, String> label()`, which javac rejected inside the generated file; a spec naming a concrete instantiation, `OpticsSpec<Box<String>>`, declared an unused `<T>`; a partial application, `OpticsSpec<Pair<A, String>>`, declared `<A, B>` where nothing could ever infer `B`; and a generic spec over a *non-generic* source, `Prism<Shape, Circle<T>>` on `OpticsSpec<Shape>`, declared nothing at all and named `T` anyway. Each method now declares the parameters its own source and focus types reach, in the spec's declaration order, carrying along any that a kept parameter's bound names so the bound still resolves. An optic method that declares parameters of *its own* is rejected at the declaration, rather than generating a method no call could resolve. An `@InstanceOf` prism onto a parameterised target answered its own `unchecked`/`rawtypes` warnings, since the class constant it tests carries no type arguments; [#733](https://github.com/higher-kinded-j/higher-kinded-j/issues/733) then removed the need for them by narrowing the test to what the source type pins down. See [Spec Interfaces](optics/optics_spec_interfaces.md#generic-spec-interfaces).

  _Behaviour change, source-breaking for explicit type arguments:_ a generated method's arity can fall. `OpticsSpec<Pair<A, String>>` generated `<A, B> … left()` and now generates `<A> … left()`; `OpticsSpec<Box<String>>` generated an unused `<T>` and now generates no parameters at all. Both compiled in 0.4.9, so a call site that supplied the witnesses explicitly — `PairOptics.<String, Object>left()` — stops compiling. Drop the explicit arguments and let inference do it; the inferred result is unchanged. A spec that renamed the source type's parameters never compiled at all, so nothing can depend on that.

- **`@ViaCopyAndSet(copyConstructor = ...)` reaches the generated code** ([#723](https://github.com/higher-kinded-j/higher-kinded-j/issues/723)): the attribute was read, carried through the analyser and stored on the strategy, and then dropped — the generator's two branches built the same `new SourceType(source)`, so a spec that named a copy constructor parameter type silently got the default. The branch was covered by tests the default arm satisfied equally, so nothing caught it. The name now resolves to the supertype of `S` it stands for and is emitted as a cast on the argument, `new Config((BaseConfig) source)` — which is what picks between overloaded copy constructors, the one case `new Config(source)` cannot express, since a lone `Config(BaseConfig other)` already takes the source by widening. The supertype is cast as `S`'s own `extends` clause instantiates it, so a base declared `Holder<String>` is cast as `Holder<String>` rather than raw — name the class alone, `com.example.Holder`, and the processor supplies the arguments. Naming `S` itself emits no cast. A name that does not resolve — the attribute is a plain string, so it is not read against the interface's imports — or one naming a type `S` does not extend or implement is now rejected at the declaration, rather than generating a cast javac cannot compile — as are two more the cast introduces: a supertype the generated class cannot see, and one no constructor of `S` accepts. See [Copy Strategies](optics/copy_strategies.md#viacopyandset-legacy-types-with-a-copy-constructor-and-setters) and [Compiler Errors](optics/compiler_errors.md).

  _Behaviour change, both ways:_ the attribute was ignored outright before, so a spec that set it compiled and silently got `new Config(source)`. A name that resolves to a supertype now emits a cast, which can select a **different** copy constructor than the one that ran in 0.4.9 — and a copy constructor can only copy what its parameter type can see, so a narrower one drops the state declared below it. Check any spec that sets `copyConstructor` against an overloaded type, and run `LensLaws` over it. A name that was previously discarded — a simple name rather than a fully qualified one, one carrying type arguments, a non-supertype, a package-private type, or one no constructor accepts — is now rejected at the declaration; qualify it, or drop the attribute to restore the 0.4.9 behaviour exactly.

  _Also:_ the same one-error-per-problem rule now holds for the two sibling hints. A rejected `@InstanceOf` target or an undetectable `@ThroughField` container used to draw its own error **and** a misleading "requires a … hint annotation" from the caller; each parse method now reports where the problem is found, and all three assert `hadErrorCount(1)`.

- **`@GenerateTraversals` reads a wildcard type argument** ([#721](https://github.com/higher-kinded-j/higher-kinded-j/issues/721)): a component written `List<? extends Leaf>` copied its type argument verbatim into the generated optic, which emitted `new Traversal<Holder, ? extends Leaf>() {` — a wildcard where javac wants a class or interface — so the generated file did not compile. It was not confined to third-party containers: `List` and `Map<String, ? extends Leaf>` failed as readily as `Either<String, ? extends Leaf>`, as did every unbounded and super-bounded form, and `Map<?, Leaf>`, where the wildcard sits in an argument the generator does not even focus. The argument is now resolved where it is read, the way `@GenerateFocus` reads it wherever it looks inside a container — `? extends T` is `T`, `?` and `? super T` are `Object` — so `HolderTraversals.items()` hands back `Traversal<Holder, Leaf>`. See [Wildcard Element Types](optics/traversals.md#wildcard-element-types).

  _Also:_ the generated map traversal calls `Traversals.traverseMapValues` rather than rebuilding the map from a list of entries, four statements becoming one, and that helper stopped copying the map it is building on every entry — a 16 000-entry map traverses in 1.3 ms where it took 851 ms, and `Traversals.forMapValues()` was already on that quadratic path. `Traversals.traverseList` takes its function as `Function<? super A, ? extends Kind<F, ? extends B>>`, matching `traverseSet`, `traverseArray` and `traverseMapValues`, which lets the first two drop an unchecked cast each, and its list as `List<? extends A>`, which is what lets a wildcard-typed field be handed over directly. Every JDK, HKJ and third-party container shape is now compiled through the real processor by a sweep that also pins the element type each one focuses, so a shape that generates uncompilable source fails a test rather than a user's build.

  _Four more component shapes that generated uncompilable source:_ an **array of a type that cannot be created by name** (`List<Leaf>[]`, `Leaf[][]`) emitted `new List<Leaf>[size]` or `new Leaf[][size]`; the array is copied to length instead, which keeps its runtime component type. An **array of a primitive** (`int[]`, and every one of the eight — `Arrays.stream` has an overload for only three, and those return a stream that cannot collect to a `List`) is now walked and boxed an element at a time, and unboxed back into a fresh array. A **generic record** (`record Holder<T>(List<T> items)`) generated `Traversal<Holder, T>` in a static method that had never heard of `T`; the method now carries the record's own type variables, as `@GenerateLenses` and `@GenerateFocus` already did. Where a record claims `F` for a type parameter of its own, the effect takes another name — one the processor declaring it and the generators writing uses of it both read from a single place, since a body that says `Kind<F, …>` against a record's `F` names the wrong variable.

  _Behaviour change, narrow:_ a `Map` component's traversal no longer throws on a null **key** in the source map, nor on a null the function hands back. It used to rebuild the map through `Map.entry` and `Collectors.toMap`, which reject both — the key too, though a values traversal never touches it. A null value already in the map was always handed to the function, and still is. The `List`, `Set` and array siblings have always stored whatever the function returned, null included.

- **A component reports one path type, whichever way you reach it** ([#719](https://github.com/higher-kinded-j/higher-kinded-j/issues/719)): `ShapesFocus.tags()` and `OuterFocus.shapes().tags()` are generated from one declaration by one processor run, and disagreed about it — two analyses answered the same question and nothing reconciled them. An `ArrayList<String>` was a `FocusPath` at the static method and a `TraversalPath` at the navigator; a `Map<String, String>` widened at the navigator whatever `widenCollections` said; a nested `List<Optional<String>>` composed to the element in one and stopped at the `Optional` in the other. There is now one widening analysis, and a navigation method composes the static Focus method for the field it reaches rather than re-deriving the widening, so the two cannot drift apart. See [Navigation and Composition](optics/focus_navigation.md#path-widening).

  _Shapes that generated source javac rejected, now compiling:_ a `Kind<F, A>` component on a navigator-reachable record (the navigator asked a widened path for its lens); a navigator over an SPI container of records whose target record has a plain navigable field of its own (`Map<String, Mid>` where `Mid` holds a `Leaf` — the composed path is now handed back where the target's navigator cannot hold it, rather than being passed to it); and a record whose `targetPackage` sends its companion elsewhere, which navigators looked for where the record was declared.

  _Also:_ a navigated path carries its field-name segments now, so `pathString()` locates it the way a static path does.

  _Behaviour change, source-breaking for navigator callers:_ three navigator return types move to what the static method has always reported. A `Collection` **subtype** (`ArrayList`, `LinkedList`) is a `FocusPath` over the container, not a `TraversalPath` over its elements — the traversal it used to hand back rebuilds a plain `List`, which the component's declared type will not accept, and threw `ClassCastException` on a one-element list; declare the component as `List<E>`, or register a `TraversableGenerator` that knows how to rebuild the concrete type. An SPI `ZERO_OR_MORE` container holding a non-navigable element (`Map<String, String>`) stops at the container until the record that **declares** it sets `widenCollections = true`; set the flag, or widen at the call site with `.each(EachInstances.mapValuesEach())`. A nested container composes to the leaf: `List<Optional<String>>` reached through a navigator is `TraversalPath<S, String>`, so drop the `.some()` that used to follow it. A container whose element is itself a `@GenerateFocus` record is stepped into either way, flag or no flag, because that is how its navigator reaches the element.

- **`@Nullable` detection reaches the annotation you actually write** ([#711](https://github.com/higher-kinded-j/higher-kinded-j/issues/711)): `@GenerateFocus` widened a `@Nullable` component to an `AffinePath` only when the annotation landed on the record component element. That is where JSR-305's and Jakarta's land, since they declare no `@Target` and so apply to every declaration, but it is not where the other four go. JSpecify's is `TYPE_USE`-only, so it lands on the component's type; JetBrains', AndroidX's and SpotBugs' are declaration annotations that reach the accessor. All four silently generated a plain `FocusPath`. The component's type and its accessor are now read alongside the component, so every recognised annotation widens, and the processor tests carry each vendor's real `@Target` instead of a stand-in wide enough to make any shape look detected. See [Focus DSL Reference](optics/focus_reference.md).

  _Precedence, now that the flag is reachable:_ a container decides the widening on its own (`@Nullable Optional<T>` is `.some()`, `@Nullable List<T>` is `.each()`), and `@Nullable` widens a field that the container and SPI analysis leave as a plain focus. Navigators used the opposite order and applied `.nullable()` to a nullable container, which typed the path by the element while the path held the container; they now agree with the static Focus methods.

  _Behaviour change, source-breaking where the annotation was silently inert:_ a component carrying JSpecify's, JetBrains', AndroidX's or SpotBugs' `@Nullable` now generates `AffinePath<S, A>` where it generated `FocusPath<S, A>`, so a declaration written against the old return type stops compiling. Read it with `getOptional` as the other absence-carrying paths are read, or drop a `.nullable()` that is now chained onto an `AffinePath`. Position counts as Java defines it: `String @Nullable []` is a nullable array, while `@Nullable String[]` and `List<@Nullable String>` annotate the elements and leave the field itself non-null.

- **`@EffectAlgebra` accepts any name for the result type parameter** ([#699](https://github.com/higher-kinded-j/higher-kinded-j/issues/699)): the generated `*Ops` declared its own `<A>` while copying each record component's type verbatim, so an algebra written `RenamedOp<T>` failed with `cannot find symbol: class T` in generated source. The component's type variable is now rewritten to the one the generated method declares, at every depth — nested arguments, arrays and wildcards included. See [Defining Effects](effect/effect_handlers.md#defining-effects).

  _Diagnostics:_ three permit shapes that used to emit uncompilable source are now rejected where they are declared: one that drops the algebra's type parameter, one that pins it (`record Fixed<T>(…) implements Op<String>`), and one that bounds it (`<T extends Number>`). A bound cannot be carried at all — the generated `Kind` and `Functor` range over every type — so a bounded parameter on the algebra is rejected too.

- **`@ComposeEffects` generates typed composition support** ([#697](https://github.com/higher-kinded-j/higher-kinded-j/issues/697)): the generated `*Support` had raw `Inject` and `Functor` factories and a `BoundSet` of `Object` components. It now reads each field's `Class<XOp<?>>` to name `XKind.Witness`, so `injectX()` returns `Inject<XKind.Witness, <composed witness>>`, `functor(...)` takes one `Functor` per effect and returns the composed `EitherFFunctor`, and `BoundSet`'s components are each algebra's `Bound` at that witness — with no casts and no suppression. See [Effect Handlers](effect/effect_handlers.md#composing-effects).

  _Behaviour change, observable:_ the last effect in a composition was injected one level too deep at every arity, so a program dispatching to it through the generated `*Support` mis-routed at runtime. The nesting now matches the arity.

  _Breaking changes / migration:_ each field must be `Class<XOp<?>>` naming an `@EffectAlgebra` — declare the type argument. An algebra with more than one type parameter cannot take part, since its witness would itself be generic. `BoundSet<F>` becomes `BoundSet`: drop the argument, or use `var`. `functor()` and `BoundSet`'s accessors change erasure, so recompile — a `*Support` is generated in your own build and never ships inside an HKJ artefact, so for most projects that is the whole migration.

- **HKJ compiles under its own checker**: every module builds with `-Xplugin:HKJChecker` (bar `hkj-checker` itself), at zero findings. Getting there took two fixes to `discarded-effect`. It now gates on a new sealed `Deferred` capability rather than `Chainable`, which is not uniformly lazy: `Path.io(() -> 1).peek(log)` is a silent no-op and is reported, while `Path.just(1).peek(log)` has already run `log` and is not — a new path type has to declare which it is. And a guard such as `Objects.requireNonNull(path, "…")` hands back the effect it was given, so it is no longer read as one built and dropped. See [Compile-Time Checks](tooling/compile_checks.md).
- **Compile-time checks stop reporting on generated code**: `hkj-checker` skips any type annotated `org.higherkindedj.optics.annotations.Generated` — it had been telling `@EffectAlgebra`'s generated `*Ops` to use the generated `*Ops` smart constructors. A single declaration can now opt out of one check with `@SuppressWarnings("<check-id>")`, or of every check with `@SuppressWarnings("hkj-checker")`; `disable=` remains the project-wide switch. Suppression is per check and accumulates outward-in, and javac's blanket `"all"` is deliberately not honoured. Diagnostics are no longer duplicated in a file with several top-level types. See [Compile-Time Checks](tooling/compile_checks.md#opting-out-one-declaration).
- **Mapper parse ceiling lifted** ([#670](https://github.com/higher-kinded-j/higher-kinded-j/issues/670)): `@GenerateMapping`'s `parse` and validated `patch`, and `@GenerateMerge`'s fallible merge, now assemble records wider than 16 components through chunked `Validated.fields()` ladders combined applicatively, so an externally fixed flat 20-30 field DTO maps without grouping into nested records. Error semantics are unchanged and law-checked at width: same located labels, same declaration-order accumulation. The only remaining width bound is the JVM's constructor parameter-slot limit on the record itself (254 components in practice, fewer with `long`/`double`), which javac enforces at the record declaration. The hand-written `fields()` ladder keeps its 16-field arity. See [Diagnostics and limits](mapping/testing.md#diagnostics-and-limits).
- **Public canonical smart constructor** ([#690](https://github.com/higher-kinded-j/higher-kinded-j/issues/690)): `ValidatedPrism.canonical(message, parse, render)` (plus a `FieldError` overload) wraps a throwing parser and a total render with the build-parse section law guarded per value: an accepted source must render back to itself, so a spelling the render cannot reproduce is a located rejection, never a silent normalisation. Generic in the source type, it is the guard `StandardCodecs` uses internally, now public so a custom-canon codec (an uppercase-UUID wire, say) is lawful without hand-writing the check. See [Laws](optics/validated_prism.md#laws).
- **Stock codec vocabulary** ([#671](https://github.com/higher-kinded-j/higher-kinded-j/issues/671)): `StandardCodecs` in hkj-core ships one `ValidatedPrism` factory per standard conversion family (`uuid`, `uri`, `localDate` with optional `DateTimeFormatter`, `instant`, `offsetDateTime`, `enumByName` naming the permitted constants, `bigDecimal`, `intFromString`/`longFromString`/`doubleFromString`, `booleanStrict`, `currency`, `locale`), so a typical DTO boundary maps without hand-written leaves. Every failure is a located, copy-worthy `FieldError` that feeds the 422 leg unchanged, and every codec accepts only the canonical form it renders, honouring the `ValidatedPrism` section law (rejections, never silent normalisations). See [Standard codecs](mapping/codecs.md#standard-codecs).
- **One leaf vocabulary across mapper tiers** ([#672](https://github.com/higher-kinded-j/higher-kinded-j/issues/672)): the sparse `UpdateSpec` tier now lifts element leaves over present containers, exactly as the dense tiers do: a present `List`/`Optional`/`Map`-valued PATCH property parses through the element leaf named after the component, replacing wholesale with every failing element located (`phones.1`), so one mix-in vocabulary serves a full spec and its PATCH sibling. A whole-container leaf wins as the more specific declaration. Two 0.4.9 behaviours change deliberately: a present same-typed identity container was previously written unscanned (a null element could reach the domain record), and now carries the dense tiers' null scan, under which a null element or value is a located rejection (`tags.1: must not be null`); and an `Optional`-typed PATCH property with a differing element type, previously rejected, now patches through its element leaf (a present empty Optional expresses "set to empty", as same-typed Optional pairs already did by identity). See [Sparse PATCH write-back](mapping/beans_patch.md#sparse-patch-write-back-updatespec).
