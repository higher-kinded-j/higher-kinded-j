...
- **`MaybePath.toEitherPath` and `OptionalPath.toEitherPath` take a supplier** ([#794](https://github.com/higher-kinded-j/higher-kinded-j/issues/794)): both now carry the deferred overload `Maybe.toEither` has had all along, so an error that is not free to build - one that formats a message, reads a `MessageSource`, or captures a stack trace - is built only on the branch that uses it. A lambda, a method reference, or a variable of a `Supplier` type picks the deferred overload; any other argument picks the eager one, so existing call sites are unchanged. An error type that is itself a functional interface would be read as a supplier, and names the eager overload for it: `path.<MyError>toEitherPath(myError)`. `toValidationPath` keeps only its eager form: its `Semigroup<E>` parameter pins the same `E` in both candidate signatures, so neither is more specific and the overload is ambiguous at every call site. Five book pages documented the deferred form before it existed; [Type Conversions](effect/conversions.md#maybepath--eitherpath) is the one that teaches it.
