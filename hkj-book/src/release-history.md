...
- **A dependency's `@GenerateFocus` record is navigable** ([#847](https://github.com/higher-kinded-j/higher-kinded-j/issues/847)): navigator generation asks a field's type whether it carries `@GenerateFocus`, and the annotation was discarded after compilation, so a record annotated in another module could not answer. The field fell back to a plain `FocusPath` and nothing was reported, which meant `CompanyFocus.headquarters().street()` compiled when `Address` was a sibling source file and did not compile when the same record arrived as a jar.
  ...
  `@GenerateFocus` is now retained in the class file, so a dependency's record is navigable as a source file is. Navigating into it composes the `Focus` class the dependency generated, reading each field's path type from the method it published rather than working it out again, so a navigator agrees with its dependency by construction, even across different processor versions or generator plugins. Where a navigator cannot be completed, the processor says why in a note against the field: a dependency that did not run the processor keeps the plain path, and a field naming a type that is not on this module's compile classpath is left out of the navigator. See [Which fields get a navigator](optics/focus_navigation.md#which-fields-get-a-navigator).

  _One note for existing builds._ A dependency built before this change carries no `@GenerateFocus` in its class files, so its records keep the plain path until it is rebuilt. Rebuilding it changes what its consumers generate: `CompanyFocus.headquarters()` becomes a `HeadquartersNavigator<Company>` rather than a `FocusPath<Company, Address>`, and a container of such records is stepped into rather than kept in focus, so code typed against the plain path may need `.toPath()` or a different path type. Rebuilding a dependency with different `@GenerateFocus` settings changes its consumers' navigators in the same way.
