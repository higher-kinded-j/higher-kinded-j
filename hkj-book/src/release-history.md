...
- **Idiomatic runtime and module docs:** the `hkj-spring/client` runtime is written in the library's own idioms (the JSON error decoder folds through `Try` and `Either`; `Retry-After` parsing and configured-class resolution route through `Try`), so the integration exercises the patterns it promotes; behaviour-preserving. The client is documented across the `hkj-spring` module docs (`CONFIGURATION.md`, `SECURITY.md`, `JACKSON_SERIALIZATION.md`) plus a dedicated `HTTP_CLIENT.md` reference.
