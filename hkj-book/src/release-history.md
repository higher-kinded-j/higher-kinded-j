...
`@GenerateMapping` now maps generic records as **concrete instantiations**: `extends MappingSpec<Page<User>, PageDto<UserDto>>` classifies every component under the substitution, so leaves, nesting, `List`/`Optional`/`Map` lifting, the null doctrine and index location all apply unchanged, and the instantiated mapping registers for nesting like any other (`Report(Page<User> results)` resolves it automatically). Raw uses, wildcard arguments and generic *specs* (`PageMapping<T>`) are diagnosed with what/why/fix pointers; threaded type parameters remain the issue's open remainder for the full mapper.
