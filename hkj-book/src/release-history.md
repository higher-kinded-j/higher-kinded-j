...
- **`@ImportOptics` imports an inner class of a generic class, and pairs a wither only when it hands back the class itself** ([#878](https://github.com/higher-kinded-j/higher-kinded-j/issues/878)): an inner class of a generic class, imported as `@ImportOptics({Outer.Line.class})`, now gets lenses named under its enclosing class's type parameters, `<X> Lens<Outer<X>.Line, String>`, as a generic spec interface already could. Written without them the type was raw: that failed a build run with `-Xlint:rawtypes` and `-Werror`, and did not compile at all once a field was typed by the enclosing class's parameter or the inner class declared parameters of its own. An inner class that declares a type parameter under a name an enclosing class already uses is refused, with a spec interface as the fix, since one method cannot declare both. See [Wither Classes to Lenses](optics/importing_optics.md#wither-classes-to-lenses).

  A wither now pairs with its getter only when it hands back the class under its own type arguments, a subtype of it, or a retag such as `<U> Tagged<U> withId(String)`, which the call infers back to the class, just as the getter already had to return exactly what the wither takes. A spec interface's `@Wither` is held to the same return, read under the spec's own instantiation, so `Draft<String> withId(String)` serves an `OpticsSpec<Draft<String>>`; a method that hands back another type is refused at the spec method, naming what it returns. The refusal for a class with no wither that pairs now spells out what pairs. See the entry in [Compiler Errors](optics/compiler_errors.md#wither--returns--not-the-source-type-).

  _Behaviour change:_ a wither returning a raw `Tagged` from a `Tagged<T>`, which compiled with an unchecked warning, no longer pairs on a class imported by class literal and is refused under a spec's `@Wither`. On a class imported by class literal, one returning `Tagged<String>` or `Tagged<?>`, which did not compile, is now skipped too. A class left with no wither that pairs, including one imported with `allowMutable = true`, is refused with the existing error.

- **Mapping refusals offer fixes you can follow, and a whole-map leaf no longer silently bypasses a `@MapKey` leaf** ([#882](https://github.com/higher-kinded-j/higher-kinded-j/issues/882)): a `@GenerateMapping` or `@GenerateMerge` refusal ends with the code that fixes it, and a few of those lines did not compile, declared a method the spec already had, or pointed past the real cause. Each now offers a line that compiles and maps the component, or says that the shape is not supported yet.
  - A leaf over a whole `Map` beside a `@MapKey` leaf for the same component compiled cleanly, and the key leaf never ran, since the whole-map leaf is tried first. A key leaf the spec declares itself is now refused beside one, and the fix offers the value leaf in place of the whole-map one where that leaves the key leaf converting the keys, or else removing the key leaf. A key leaf inherited from a mix-in stays inert beside a whole-map leaf, as other shared vocabulary does.
  - A container that does not lift as declared, an `ArrayList`, a `SortedSet` or a `List<? extends Customer>`, was offered only a hand-written leaf over the whole container. The refusal now says that lifting takes the same exact container on both sides, one level deep. Where the component would lift once declared as, say, `List<Customer>` against `List<CustomerDto>`, it offers that declaration and names the leaf or spec that would then map the elements. For a same-named leaf of the wrong shape, the refusal lists the shapes the pair accepts, and the fix replaces that leaf rather than adding a second method of the same name.
  - A primitive component was offered a leaf such as `ValidatedPrism<int, Integer>`, which does not compile. An `int` against an `Integer` is now asked to use one of the two types on both sides, and any other pair to align the types, or to declare the wrapper types and add a leaf over them. On an `UpdateSpec`, the fix offers the wrapper on the PATCH property, which writes straight into a primitive component.
  - A domain `Optional` against a primitive bean property was described as bridging to a "nullable" property and offered a leaf over the primitive. It is now refused because a primitive cannot hold the `null` that stands for an empty `Optional`, and the fix offers the wrapper type, with the leaf the wrapper then needs. The same applies where an `@OptionalBridge` marker names a primitive member, whose fix no longer suggests removing the annotation. The element leaf a bean bridge offers no longer carries `@OptionalBridge`, which a bean wire does not need; a record wire keeps it.
  - A PATCH bean with setters and a `private` no-args constructor was refused as having no way to be written. `updateFrom` only reads the bean, so it is now accepted; a bean with a builder keeps the builder as its writer.
  - A sparse `UpdateSpec` that could not nest a pair only a build-only spec maps now names that spec and what it lacks, as a `MappingSpec` does.
  - Nesting an element-mapped spec with several abstract leaves offered a leaf named after the component, which only a single-leaf spec consults. The fix now offers a leaf over the whole pair that builds the composition with the generated Impl's `of(...)`, and a spec only for a record pair.
  - A sealed pair whose permitted subtype is generic, an enum, or a class a spec cannot map asked for a `@GenerateMapping` spec that could never serve the dispatch. Every such subtype is now named in one refusal as not supported yet.

  See [Converting Map keys](mapping/structure.md#converting-map-keys) and [Sealed hierarchies](mapping/structure.md#sealed-hierarchies).

  _Notes for existing builds._ A spec that declares a `@MapKey` leaf beside a leaf over the whole `Map` for one component now fails to compile; keep the one it means. A PATCH bean with setters whose constructor the generated code cannot call, which was refused, now compiles.

- **Generated code, and the annotations hkj's processors read, draw no `-Xlint:processing` warning**: javac reports every annotation that no annotation processor declares it handles (javac's word is *claims*), and each hkj processor claimed only the annotation it generates for. A build run with `-Xlint:all` and `-Werror` therefore failed whenever hkj generated code, which carries `@Generated`, and wherever a source used an annotation hkj reads alongside a generating one, such as `@MapField` on a mapping spec, `@Wither` on an optics spec, `@TraverseField` on a record component, `@PathVia` on a bridged service or `@OnStatus` on an `@HkjHttpClient` method. A new `CompanionAnnotationProcessor` in `hkj-processor` claims them, and one in `hkj-spring-boot-client-processor` claims the client's, for a build that runs the client processor on its own. Neither generates anything, and both are registered with Gradle as isolating, so incremental compilation is unaffected. The client processor also reports the latest source version javac supports, so a newer JDK compiling at a newer `-source` draws no source-version warning from it.

  Annotations from other libraries are still reported: jspecify's `@NullMarked`, which `@EffectAlgebra` and `@ComposeEffects` write onto the classes they generate, and Spring's, which every `@HkjHttpClient` client carries. See [Build-time impact](optics/production_readiness.md#build-time-impact).

  _Notes for existing builds._ A build that names its processors, with `-processor` or Maven's `<annotationProcessors>`, adds `org.higherkindedj.optics.processing.CompanionAnnotationProcessor` to the list. A processor that supports every annotation, such as Lombok's, belongs ahead of `hkj-processor` on the processor path: javac stops offering a round to further processors once every annotation in it is claimed.

- **Generated effect and client classes draw no `cast` or `missing-explicit-ctor` warning**: the functor `@EffectAlgebra` generates for an algebra without `mapK` cast the result of `widen` to the type it already returns, which `-Xlint:cast` reports, and it no longer casts. The interpreter class `@EffectAlgebra` generates, and the configuration class `@HkjHttpClient` generates, each declares its constructor, so a named module that exports their package compiles them without a warning under `-Xlint:missing-explicit-ctor`.

- **Traversals over an `Optional`, `Maybe`, `Try`, `Validated` or `Either` component compile under `-Werror`** ([#877](https://github.com/higher-kinded-j/higher-kinded-j/issues/877)): the traversal generated for such a component, as in `record Doc(String id, Optional<String> more)`, cast the effect it maps to the type that effect already had. javac reports that as a redundant cast under `-Xlint:cast`, so a build run with that lint and `-Werror` failed in the generated file. The generated traversal no longer casts, for a record annotated with `@GenerateTraversals`, and for an `Optional` component of a record imported with `@ImportOptics({Doc.class})`, the only one of these types that route traverses. See [Supported Types at a Glance](tooling/generator_plugins.md#supported-types-at-a-glance).

- **An empty `Optional` writes `null` to a bean, so the bean's own defaults no longer read back as present** ([#871](https://github.com/higher-kinded-j/higher-kinded-j/issues/871)): a domain `Optional<T>` mapped onto a bean property `T` skipped the write when empty, so the property kept whatever the bean was constructed with. A field initialiser such as `private String status = "ACTIVE"`, or a builder's own default, then came back through `parse` as `Optional[ACTIVE]`, and `MappingLaws` failed the round trip. `build` now hands the setter or builder setter `null`, as a record wire's constructor already received it, so the built bean no longer depends on how it was constructed.

  The same default defeats a sparse `UpdateSpec`, where there is no write to correct it: a PATCH bean field declared `private List<String> tags = new ArrayList<>()` reads as a present empty list when the client omits it, and `updateFrom` replaces the domain's list with it. No signature shows an initialiser, so this cannot be refused. The book, the `UpdateSpec` javadoc and the refusal for a getter-only `List` now say that a PATCH bean needs more than a setter: every getter must answer `null` until its property is set, whether the default would come from a field initialiser, the constructor or a getter that creates its value. The sparse law examples now take a freshly constructed bean as the all-absent wire, which is what a binder makes of an empty body and which catches such a default in your own tests wherever the current value differs from it. See [Bean-shaped wire targets](mapping/beans_patch.md#bean-shaped-wire-targets) and [Sparse PATCH](mapping/beans_patch.md#sparse-patch-write-back-updatespec).

  _Notes for existing builds._ A bean built from an empty `Optional` now holds `null` in that property, not the value it was constructed with, so an outbound DTO that relied on an initialiser to send a default (`currency = "GBP"`, an empty list) no longer sends it; carry the default in the domain value instead. A setter or builder setter that refuses `null` now throws from `build` when its `Optional` is empty, where before it was not called. That includes a setter that copies defensively (`List.copyOf(v)`), which needs a guard (`v == null ? null : List.copyOf(v)`), and a generated builder such as protobuf's or Immutables', which cannot take one: declare that component without the `Optional`, or give it a leaf over the whole `Optional` that encodes absence the builder's way. A nullness checker reading the generated source sees the `null` too, so in a `@NullMarked` package declare a bridged setter's parameter `@Nullable`.

- **Optics imported from a class compile under `-Werror`, and traverse a wildcard element** ([#873](https://github.com/higher-kinded-j/higher-kinded-j/issues/873)): a generic type written without its type arguments, in a record or wither class imported with `@ImportOptics({Doc.class})`, gave generated code that failed a build run with `-Xlint:rawtypes` and `-Werror`, in a file the `@SuppressWarnings("rawtypes")` on your own declaration cannot reach. It could sit in a component or field, `List tags` or `Optional<List> contacts`, or in the bound of a type parameter, `record Doc<T extends List>`, which every generated method redeclares. The lenses and `with` methods generated for either kind of type, and the traversals generated for a record's containers, now carry that suppression on each member that writes out or infers such a type, as the optics `@GenerateLenses` and `@GenerateTraversals` generate for the same record already do. A traversal's implementation class repeats the type as well, the bounds in its type-parameter clause and a raw element type in its superinterface clause, and carries the suppression too. Only `rawtypes` is suppressed: an unchecked operation in generated code is still reported.

  An imported record's container whose element type is a wildcard, `List<? extends Number>`, `List<?>` or `Set<? super Integer>`, now gets a traversal focusing the type the wildcard stands for, `Number` or `Object`, as `@GenerateTraversals` does. The generated traversal used to write the wildcard into its implementation's declaration, and the file did not compile. See [Container Fields Get Traversals](optics/importing_optics.md#container-fields-get-traversals).

- **A bean accessor with no partner is refused where leaving it out would drop a value** ([#868](https://github.com/higher-kinded-j/higher-kinded-j/issues/868)): a bean property is a getter and a writer that share a name, and a getter or setter without a partner is left out of the mapping. Whenever some other property paired, that happened without a word. On an `UpdateSpec` it meant a field the client sent could be ignored: with `setEmail(String)` beside a misspelt `getEmial()`, a sent `email` left the domain unchanged and `updateFrom` still returned `Valid`. On a `MappingSpec` the same bean compiled, and `build` never wrote the property.

  An unpaired accessor named after a domain component the bean carries under no name, the one the component maps under (its own, or the one a `@MapField` rename gives it), is now refused on both tiers. The diagnostic names the accessor that would pair it. A nearby accessor of the other kind at the same type is offered as the likely misspelling, so the bean above is told to rename `getEmial()` to `getEmail()`; otherwise it offers the `@Unmapped` marker below, for an accessor that is meant to stay out. An `UpdateSpec` also refuses a `setX` setter with no getter whatever it is named, since that is a field the client can send and the update would ignore. Otherwise an accessor no domain component names, such as a computed `getSummary()` or a builder's singular adder, is still left out without complaint.

  Some beans leave an accessor unpaired on purpose, and often cannot be edited: a response DTO reused as a PATCH body whose `getId()` the client must not change, a view whose `getStatus()` is computed on the wire, a generated request with a setter the domain does not model. The new `@Unmapped` marker, an abstract method named after the accessor's property, says so. It withholds the refusal and changes nothing else: the component the accessor names stays unmapped, and a wire narrower than the domain is still a projection. A marker the spec declares itself must name an accessor the bean leaves unpaired, and one inherited from a mix-in binds where it can and is otherwise inert, so one vocabulary serves wires that differ.

  A `Boolean isX()` getter now counts as a getter, as `boolean isX()` does, so the shape JAXB declares for an optional boolean pairs with its `setX(Boolean)` and maps both ways. Where a bean declares both `getX()` and `isX()` for one name, `getX()` reads it, whichever is declared first. See [Bean-shaped wire targets](mapping/beans_patch.md#bean-shaped-wire-targets).

  _Notes for existing builds._ A bean that compiled with an unpaired accessor named after a domain component, or a PATCH bean with a setter and no matching getter, now fails to compile: pair the accessor, or remove it. A `Boolean isX()` accessor is now a property like any other, which changes a few beans that compiled before:
  - a property read only through `Boolean isX()` now maps, so `build` writes it and `updateFrom` applies it;
  - a `Boolean isX()` and `setX(Boolean)` pair the domain does not model is now an extra wire property, which needs a domain component, a [derived field](mapping/basics.md#derived-wire-fields) on a `MappingSpec`, or removing from a PATCH bean;
  - `Boolean isX()` beside `setX(boolean)` is refused as a property read and written at different types;
  - a bean with setters and no getter but such an `isX()`, which mapped build-only, now reads some names and writes others, and is refused naming both;
  - a bean filled only through getter-only `List` properties that also declares such an `isX()` now maps parse-only, and a note says so.

  A library whose mapping over such a bean was compiled by an earlier release should be rebuilt before a module that nests it, so that both agree on the bean's properties.

- **Generated optics and companions compile under `-Werror`** ([#867](https://github.com/higher-kinded-j/higher-kinded-j/issues/867)): a generic type written without its type arguments gave generated code that failed a build run with `-Xlint:rawtypes` and `-Werror`, in a file the `@SuppressWarnings("rawtypes")` on your own declaration cannot reach. It could sit in a record component, `List tags` or `Optional<List> contacts`; in the bound of a type parameter, `record Doc<T extends List>`; in the `Iso` a `@GenerateIsos` method returns; in a variant's or a context record's component under `@GenerateErrorEnvelope`; or in the focus type an `@ImportOptics` spec method declares. A raw bound on a `@GeneratePathBridge` interface was refused outright, so a service whose bound was written bare could have no bridge at all.

  `@GenerateLenses`, `@GenerateGetters`, `@GenerateSetters`, `@GenerateFolds`, `@GenerateFocus` and `@GenerateTraversals` write the component type, and the bounds of the type parameters they redeclare, into the members they generate; a fold's and a traversal's implementation class names that type again in its superinterface clause. `@GeneratePrisms` writes the bounds a permitted subtype declares. A `@GenerateFocus` navigator writes the field types of the record it navigates into. `@GenerateAssembly` writes a component into each stage's field, constructor and method, and infers them all in `assemble`. An `@ImportOptics` optic method carries it for the focus the spec declared, for the lens focus a checked composition restates, and for the bounds of the type parameters it redeclares, folded into the same `@SuppressWarnings` as the `unchecked` suppression such a method may already need, since one declaration takes a single `@SuppressWarnings`. A `@GeneratePathBridge` bridge carries it on its class, where the interface's bounds are repeated. Each generated member that writes out or infers such a type now carries that suppression itself, in the same way the generated mapping `Impl` does.

  Only `rawtypes` is suppressed: an unchecked operation in generated code is still reported. Nothing about widening changed: a raw `List` still folds and focuses over `Object` elements, a raw `Map` is written as it is declared, and `@GenerateFocus` still refuses a raw container it would have to name an optic instance for, such as a raw `Set`.

- **A mapping over a raw type compiles under `-Werror`** ([#865](https://github.com/higher-kinded-j/higher-kinded-j/issues/865)): a `@GenerateMapping` spec over a generic type written without its type arguments compiled with javac's default warnings but failed a build run with `-Xlint:rawtypes` and `-Werror`. The raw type could sit in a marker such as `@OptionalBridge Optional<List> contacts()` or `@MapField(to = "labels") List tags()`, in an abstract leaf such as `ValidatedPrism<List, T> item()`, in a record component typed `Optional<List>`, or in the bound of a spec's type parameter. The generated `Impl` repeated it where your own `@SuppressWarnings("rawtypes")` cannot reach: in the marker method it implements, in an abstract leaf's field, accessor, constructor and factory, in its own type-parameter clause, and in lambdas whose parameters javac infers to a type holding it, such as `Optional<List>`.

  The generated code now carries that suppression itself, on each member that can repeat a raw type and, for a raw bound, on the `Impl` class, so a spec that compiles without a warning generates an `Impl` that compiles without one too. Only `rawtypes` is suppressed: an unchecked operation in generated code is still reported. `@GenerateMerge` does the same; there a target wider than sixteen components failed the same way.

- **An optional list maps through its elements' spec** ([#860](https://github.com/higher-kinded-j/higher-kinded-j/issues/860)): a domain `Optional<List<Contact>>` against a nullable `List<ContactDto>`, the shape an optional JSON array takes, was refused even with a `ContactMapping` in the build, and so was the same bridge over an element leaf. A `List<Contact>` against a `List<ContactDto>` always lifted through the spec, and an `Optional<Contact>` against a nullable `ContactDto` nests through it; only the bridged container lifted nothing, so it needed a hand-written leaf over the whole list.

  A bridged value now maps exactly as the same pair would without the `Optional`, so the bare `@OptionalBridge` marker is all a record wire needs, and a bean wire needs nothing. `build` writes `null` for an empty `Optional` and lifts a present list through the spec; `parse` reads `null` as empty and lifts a present list element by element, so a failure locates at its index (`contacts.1.email`) and a `null` element is still a located `must not be null`. `Set`, arrays and `Map` values lift the same way, through an element leaf or a spec, and a `@MapKey` leaf converts the keys of a bridged `Map`. A container whose elements nothing converts is refused naming the element pair, and the fix offered is an element leaf or a spec; a leaf over the whole container still works as an override. The refusal for a record component left unmarked offers the bare marker when a spec maps the elements. A container without the bridge is offered the same fix: a `List<Contact>` whose elements nothing converts is offered a leaf over the elements rather than over the whole list, as a sparse `UpdateSpec` already was, and a `Map` whose keys convert through a `@MapKey` leaf is offered a leaf over its values on either tier. See [Optional nested objects](mapping/structure.md#optional-nested-objects).

  _Notes for existing builds._ An element leaf on a bridged container whose element types already matched, such as `@OptionalBridge default ValidatedPrism<String, String> tags()` on an `Optional<List<String>>`, was passed over and the list copied as it was; it now validates each element and, as on a `List<String>` component, parse and build return a new unmodifiable list rather than the one passed in, so code that changed the parsed or built list in place should copy it first.

- **A mapping can name a type another annotation processor generates** ([#861](https://github.com/higher-kinded-j/higher-kinded-j/issues/861)): a `@GenerateMapping` or `@GenerateMerge` spec naming a type that another processor writes in the same compilation was mapped before that type existed. javac hands a processor a placeholder for such a type, and the placeholder passed the mapper's checks. A valid spec over a generated wire was refused as "not a usable bean-shaped wire", and a spec whose records held generated components was given an `Impl` that copied one generated type straight into another, which failed to compile once the types were written. Nothing pointed at the real cause.

  A spec now waits until the types it reads exist, and so does any spec that nests it, so it maps exactly as it would were those types written by hand. A spec whose type never appears generates nothing, and javac's own error names the missing type. A spec that another processor generates also nests through the specs declared beside it with `-Ahkj.mapping.index=false` or in a named module, where before it found none. See [Mapping over types other processors generate](tooling/manual_setup.md#mapping-over-types-other-processors-generate).

- **A generated class whose package two modules declare is reported as that**: when modules compiled together declare the same package, javac cannot tell which of them a generated class belongs to, and refuses to write it. `@GenerateMapping`, `@GenerateMerge`, `@GenerateErrorEnvelope` and `@GenerateAssembly` reported the refusal as a class that already exists and suggested renaming, which could not help. They now name the shared package and the modules declaring it, and suggest declaring the package in only one of them or compiling the modules separately.

- **An `Optional` PATCH property is the remedy offered** ([#680](https://github.com/higher-kinded-j/higher-kinded-j/issues/680)): an `UpdateSpec` refusing a plain PATCH property against a domain `Optional` component suggested a nested record or a sentinel value first, both of which change the wire contract. It now offers the shape that changes nothing on the wire: the same property typed `Optional<T>`, its field defaulting to `null`. That shape already carries JSON Merge Patch's three states: an omitted property leaves the value unchanged, an explicit `null` clears it, and a value sets it. A field defaulting to `Optional.empty()` instead reads every omitted property as a clear, so the default matters, and the `@OptionalBridge` refusal on an `UpdateSpec` now says so too. See [Sparse PATCH write-back](mapping/beans_patch.md#sparse-patch-write-back-updatespec).

- **A client keeps the `@OnStatus` overrides it inherits from a jar** ([#849](https://github.com/higher-kinded-j/higher-kinded-j/issues/849)): an `@HkjHttpClient` interface extending a base interface from another module now generates the same per-status decoding as one whose base is compiled alongside it. `@OnStatus` and its repeated-annotation container were discarded after compilation, so a method read from a class file carried no overrides and the generated client ignored them: a 404 decoded without the subtype its override named, and nothing was reported. Both annotations are now retained in the class file, and an inherited override is checked just as a local one is.

  A problem with an inherited method, whether in an override or in its return type, is reported on the client interface and names the method and the interface it came from. That holds whether the base comes from a jar, where a method read from a class file has no file or line of its own to point at, or is compiled alongside the client. An override in a jar naming a type that is missing from the client's classpath is reported as missing from the classpath rather than as not assignable. See [Declarative HTTP clients](spring/declarative_http_clients.md#1-per-method-onstatus).

  _Notes for existing builds._ A base interface compiled against an earlier release has already lost its overrides, so rebuild it to pick them up. An inherited override is now checked where it was silently ignored, so one naming a type that is not assignable to the method's declared error type, or one missing from the client's classpath, fails a build that compiled before, and the duplicate-status and `MaybePath` warnings reach an inherited override too, which fails a build compiled with `-Werror`. The two annotations occupy a few bytes in the class files of the interfaces that declare them; they are not visible to reflection (`CLASS`, not `RUNTIME`). A module compiling against those class files without `hkj-spring-boot-client` warns under `-Xlint:all`, which fails a `-Werror` build, so a module declaring `@OnStatus` on an interface other modules use should expose the client to them (Gradle's `api` configuration).

- **An optional nested object maps through its own spec** ([#825](https://github.com/higher-kinded-j/higher-kinded-j/issues/825)): a domain `Optional<Address>` bridged to a nullable `AddressDto` was refused even with an `AddressMapping` in the build, and the fix the refusal asked for was a hand-written leaf delegating to the Impl the processor had already generated. A `List<Address>` against a `List<AddressDto>`, or an `Optional<Address>` against an `Optional<AddressDto>`, always nested through that spec; only `Optional<Address>` against a plain `AddressDto` did not.

  The bridge now nests too, so the bare `@OptionalBridge` marker is all a record wire needs, and a bean wire, which bridges automatically, needs nothing. `build` writes the nested build of a present value and `null` for an empty one; `parse` reads `null` as empty and hands a present value to the nested `parse`, so its failures locate under the component (`address.city`), exactly as through a `List`. An element leaf still wins over the spec, two specs for the pair are still ambiguous, and a pair no usable spec covers is still refused. That refusal now offers a spec for a record pair, and names one that maps the pair but cannot be nested, such as a projection. The refusal for a record component left unmarked offers the bare marker whenever exactly one spec maps the element pair, and every fix that offers a leaf in a spec's place spells out the whole declaration, with `@OptionalBridge` on it where the component carries the marker. See [Optional nested objects](mapping/structure.md#optional-nested-objects).

- **A dependency's `@GenerateFocus` record is navigable** ([#847](https://github.com/higher-kinded-j/higher-kinded-j/issues/847)): navigator generation asks a field's type whether it carries `@GenerateFocus`, and the annotation was discarded after compilation, so a record annotated in another module could not answer. The field fell back to a plain `FocusPath` and nothing was reported, which meant `CompanyFocus.headquarters().street()` compiled when `Address` was a sibling source file and did not compile when the same record arrived as a jar.

  `@GenerateFocus` is now retained in the class file, so a dependency's record is navigable as a source file is. `@TraverseField` is retained alongside it, so an incremental build that reprocesses an unchanged record from its class file gives a `Kind` field the path a clean build does. Navigating into it composes the `Focus` class the dependency generated, reading each field's path type from the method it published rather than working it out again, so a navigator never composes a method or path type the dependency did not publish, whichever processor version or generator plugins built it. Where a navigator cannot be generated in full, the processor says why in a note against the field: a dependency that did not run the processor keeps the plain path, and a field is left out when it names a type that is not on this module's compile classpath, or when the dependency's `Focus` class has no matching method for it. See [Which fields get a navigator](optics/focus_navigation.md#which-fields-get-a-navigator).

  _Behaviour change for consumers of a rebuilt dependency:_ a dependency built before this change carries no `@GenerateFocus` in its class files, so its records keep the plain path until it is rebuilt. Once it is, consumers with `generateNavigators = true` generate navigators into it: `CompanyFocus.headquarters()` becomes a `HeadquartersNavigator<Company>`, whose `.toPath()` gives the `FocusPath` back, and an SPI container such as `Map<String, Address>` is stepped into rather than kept in focus. `List`, `Set`, `Collection`, `Optional` and `Maybe` fields are unaffected. To keep a field's previous shape, list it in the consuming record's `excludeFields`. A container that reaches such a record through a wildcard, such as `Map<String, ? extends Address>`, compiled while the record had no companion and is now refused like any wildcard container a navigator would step into; declare its type arguments exactly, or exclude the field.

- **`@GenerateMapping` maps a bean that can only be read, or only be written** ([#703](https://github.com/higher-kinded-j/higher-kinded-j/issues/703)): a bean-shaped wire had to support both directions, so two common shapes could not map at all: a getter-only read model (a generated client's response type, an immutable view built through its constructor, a third-party result) and a write model filled through setters or a builder with no getters (an outbound request). Each now maps the one way it can. A bean with getters and nothing that writes it generates `parse` and `asValidatedParse()` and no `build`; one that can be written but declares no getter generates `build` and `asValidatedBuild()` and no `parse`. The missing direction is absent from the Impl rather than a method that throws, and a note names the direction the bean was read in, so a bean meant to be built whose constructor the generated Impl cannot reach does not lose its `build` unremarked. A bean that reads some names and writes others, the shape a misspelt accessor leaves, is still refused, and the diagnostic now lists both.

  Coverage follows the direction: a parse needs a getter for every domain component and ignores the rest, and a build needs a source for every writer, a derived field included. Renames, leaves, container lifting and the automatic `Optional` bridge work in whichever direction exists. A one-directional mapping nests wherever only its direction is used, lifted through containers like any other: a parse-only spec inside a parse-only mapping, a sparse `UpdateSpec` or a `@GenerateMerge` source, and a build-only spec inside a build-only mapping. A sparse `UpdateSpec` still needs a PATCH bean that is both read and written, since only such a bean can leave a property unset. `MappingLaws` gained an overload for each new surface. See [One-directional beans](mapping/beans_patch.md#one-directional-beans).

  To carry this, `ValidatedPrism`'s two directions became types of their own. `ValidatedParse` holds `parse`, `parsePath` and the parse bulk forms, `ValidatedBuild` holds `build` and the build bulk forms, and `ValidatedPrism` extends both, so every prism serves wherever either half is asked for and code compiled against 0.4.10 keeps working. `parseEntries` and `buildEntries` take a half for the values. See [One direction at a time](optics/validated_prism.md#one-direction-at-a-time).

  _Two changes in what compiles._ A bean with getters and no usable way to be written, including one whose setters sit behind a constructor the generated Impl cannot call, used to be refused as not a usable bean; it now maps parse-only, with a note naming the reason. And a bean with no setters whose getters mix a getter-only `List` with other properties used to map both ways over the `List` alone, filled through its getter, with its other getters dropped; that shape is a read model, so it now maps parse-only over all of its getters. Reflection that looks the moved `ValidatedPrism` methods up with `getDeclaredMethod` now finds them on the half that declares them.

- **A shared vocabulary carrying `@Flatten` serves a PATCH sibling too** ([#836](https://github.com/higher-kinded-j/higher-kinded-j/issues/836)): a [mix-in](mapping/codecs.md#shared-vocabulary-mix-in-interfaces) carrying a [`@Flatten`](mapping/structure.md#flattening-a-nested-component-onto-a-flat-wire) marker could not be extended by a full `MappingSpec` and its sparse `UpdateSpec` alike. The marker was refused wherever it was declared, so the refusal landed on the shared interface, and both fixes it prescribed broke the full spec that needed the member.

  The marker is now judged against the PATCH wire. An inherited marker is inert whenever the bean carries none of the group's inner properties that nothing else fills, which covers a bean declaring the group's own component (`Address getAddress()`, patched whole by identity) and one omitting the group entirely. A bean carrying one is asking for a spread, which has no sparse edit shape yet, and keeps a refusal naming both the marker and the mix-in it came from, now offering the replacement that resolves it. A marker the `UpdateSpec` declares itself is still refused, as a local derived field or `@OptionalBridge` is, and both fix lines now name the remedy that keeps a full sibling working. See [Sparse PATCH write-back](mapping/beans_patch.md#sparse-patch-write-back-updatespec).

- **A mapping spec names one tier** ([#837](https://github.com/higher-kinded-j/higher-kinded-j/issues/837)): a spec extending both `MappingSpec` and `UpdateSpec` is now refused where it is declared, naming both supertypes. One spec generates one Impl on one tier, and the two emit disjoint members, so the sparse clause silently won: the Impl carried `updateFrom` alone while the spec was still offered to other specs as a nested mapping. A parent nesting the pair then generated a call to a `parse` that was never written, and the build failed with `cannot find symbol` inside a generated file, naming a class the author had not written for a mistake made in a third one. A pair of tiers is a pair of specs, with a [shared vocabulary mix-in](mapping/codecs.md#shared-vocabulary-mix-in-interfaces) carrying what they have in common. A spec in a dependency that carries the shape is no longer silently unusable either: a use site that needed the pair is told which spec it is and why it has no parse, the way a dependency whose Impl has gone missing already was. See [Sparse PATCH write-back](mapping/beans_patch.md#sparse-patch-write-back-updatespec).

  _Behaviour change, refusing a shape that compiled:_ as long as nothing nested the pair, such a spec compiled in 0.4.10, its `MappingSpec` clause silently ignored and only `updateFrom` emitted. It is now refused at the declaration. Split it into a `MappingSpec` and an `UpdateSpec`; the sparse half's generated surface is unchanged, so its `updateFrom` call sites are too.

- **A shared vocabulary crosses a module boundary** ([#844](https://github.com/higher-kinded-j/higher-kinded-j/issues/844)): `@MapField`, `@OptionalBridge` and `@MapKey` were discarded after compilation, so a [mix-in vocabulary](mapping/codecs.md#shared-vocabulary-mix-in-interfaces) published by one module lost exactly the members that gave it meaning. A downstream spec extending it from a jar saw bare methods: a rename was refused as "neither a rename, a leaf, nor a bridge", and a `@MapKey` leaf whose two key types already agreed simply stopped validating, with no diagnostic at all. Leaves and derived fields, which carry no annotation, crossed intact, so the pattern worked right up until a vocabulary carried an annotated member.

  All three are now retained in the class file, joining `@Flatten`, which already was. A vocabulary interface means the same thing to a dependent compilation as to its own, and the publishing module needs no annotation processor, since a mix-in is a plain interface rather than a spec. One API module can own the house vocabulary that every service module's specs extend. See [Across modules](mapping/structure.md#across-modules).

  _Two notes for existing builds._ These annotations now occupy a few bytes in the class files of any module that declares them; they are not visible to reflection (`CLASS`, not `RUNTIME`), and nothing reads them at runtime. And a rename's `to` is now part of a published vocabulary's compiled surface, so editing one recompiles the modules that extend it, where previously the edit was invisible to them in both senses.

- **One shared vocabulary serves every spec that extends it** ([#826](https://github.com/higher-kinded-j/higher-kinded-j/issues/826), [#835](https://github.com/higher-kinded-j/higher-kinded-j/issues/835)): a [mix-in vocabulary](mapping/codecs.md#shared-vocabulary-mix-in-interfaces) carrying a rename or a derived field could not be shared as widely as the book promised. A derived field was refused on a PATCH sibling; a rename was refused by any spec whose wire omitted its target or whose domain omitted the component it renamed, which meant a projection, a PATCH bean, or a sealed dispatch could not extend a vocabulary its own siblings used. The refusal landed on the shared interface, and every fix it prescribed broke the specs that needed the member.

  An inherited member that binds to nothing now stays inert, the rule leaves, bridges and `@MapKey` key leaves already followed, so the same interface serves a full mapping, its projections, its PATCH sibling and its sealed dispatch alike. Which side a member binds against decides what "binds to nothing" means: a leaf, bridge, key leaf or `@Flatten` marker is matched against the domain, a derived field against the wire, and a rename against both at once. A member the spec declares *itself* keeps its refusal, which is what catches a typo. A `@Flatten` marker is the one member judged against the wire as well, and is still refused outright on a sealed pair, where there are no components to spread.

  Nothing is silently mismapped by an inert member, because every wire component must still name a source. The cost is narrower: a `to` misspelled in the mix-in itself is now caught only where some spec's wire carries the intended name. See [Shared vocabulary](mapping/codecs.md#shared-vocabulary-mix-in-interfaces).

- **`@GenerateMapping` lifts `Set`, array and `Map`-key components** ([#675](https://github.com/higher-kinded-j/higher-kinded-j/issues/675)): element lifting covered `List`, `Optional` and a `Map`'s values. A `Set<EmailAddress>` component could only be copied whole or mapped through a hand-written whole-container leaf, an array could not lift at all, and a `Map` whose two sides keyed differently was a hard error. All three now lift through the leaf or nested spec named after the component, in both directions and on every tier - dense, projection, bean wire and sparse `UpdateSpec` - and a same-typed `Set` or reference array carries the identity null scan the other containers already did.

  Each container locates a failure by whatever identifies an element in it. An array has indices, so it reads like a list (`reserves.1: not an email address`). A set has none, and its iteration order is not part of its contract, so numbering would name a different element on the next run: a set element locates by its own rendering (`members.nope`), extending the caveat the map-key grammar already carried. A `null` set element has no rendering at all and a set holds at most one, so it reports unlocated as `members: must not contain a null element`, kept distinct from `must not be null`, which says the set itself is absent. A primitive array (`int[]`) is copied whole: a `ValidatedPrism` cannot focus a primitive, and a primitive element cannot be null.

  A `Map`'s keys convert through `@MapKey`, alone or beside the value leaf. The annotation names the domain component rather than the method, because the component's value leaf already owns that name and Java forbids two zero-parameter methods sharing one. A failing key locates by the *source* key, so the path names what the caller sent rather than what it parsed to, and an entry wrong on both sides reports both reasons there. Without a key leaf the key types must still match, and the refusal now offers a pasteable `@MapKey` declaration instead of telling you to restructure. `ValidatedPrism` gained the matching bulk forms: `parseAll`/`buildAll` over a `Set` and over arrays, plus `parseKeys`/`parseEntries` and their total twins.

  Mapping is not injective, so a container can collapse. A set drops the duplicate silently: the element that remains is equal to the one dropped, so the set still holds every distinct value it was given, although the two source spellings that collapsed can no longer be told apart. Two map keys parsing to one domain key are a located `duplicates an earlier key` instead, since the dropped entry takes its own value with it. Either way the element prism doing it already breaks the `ValidatedPrism` section law, so no lawful mapping collapses. Neither reaches the lossless `asIso()` tier, which a leaf already excludes. Lifting still needs the same container on both sides: a `List` against a `Set`, or an array against a `List`, stays a plain type mismatch rather than silently changing what the collection promises. See [Nesting and containers](mapping/structure.md#nesting-containers-and-recursion) and [Converting Map keys](mapping/structure.md#converting-map-keys).

  _Two source-compatibility notes,_ both from `parseAll` and `buildAll` becoming overloaded on the container. A call passing a bare `null` literal now needs the type it stands for (`parseAll((List<String>) null)`). And the method reference `prism::parseAll` is now *inexact*, so it no longer drives inference on its own: where nothing else fixes the container type it needs a target type or a lambda (`xs -> prism.parseAll(xs)`). A reference whose type is already pinned - by an assignment target, or by another argument, as in `map(prism::parseAll)` - is unaffected, as is every call passing an actual container. `ValidatedPrism` is sealed, so no implementation outside the library can be affected by the new overloads.

- **Bean-shaped projections take the validated `patch`** ([#702](https://github.com/higher-kinded-j/higher-kinded-j/issues/702)): a bean wire with fewer properties than the domain now maps when any of its properties is reference-typed, where it used to be refused with a pointer to the record-wire tier. The Impl emits the same dense `patch(domain, wire) : Validated<NonEmptyList<FieldError>, Domain>` a record projection gets: every projected property is validated, an unset property is a located `must not be null` (on a bean an ordinary state, not only bad input), every bad field accumulates, and unprojected components are read from the domain argument. Leaves, nested specs, container lifting and the automatic `Optional` bridge all apply (an unset bridged property reads as empty), and the existing `MappingLaws` patch overload law-checks it. The Impl still carries `build`, so the bean needs setters or a builder. An all-primitive bean projection keeps its lawful `asLens()`, and a record wire adopted in place of the bean keeps working unchanged. See [Bean projections](mapping/beans_patch.md#bean-projections).

- **`@GenerateMapping` spreads a nested component across a flat wire** ([#674](https://github.com/higher-kinded-j/higher-kinded-j/issues/674)): a domain record nested inside another (`Customer` holding an `Address`) can now map to a wire that carries the address as plain fields (`street`, `city`, `postcode`), in both directions. The wire format is often fixed by someone else, and a leaf could not help, since no single wire component carries the address. A `@Flatten` marker named after the domain component declares the spread: `build` fills each flat component from the record's same-named component, and `parse` assembles the record through its own `Validated.fields()` ladder, so every failure accumulates and locates under the domain path (`address.street: must not be null`), which the flat wire never named. The whole vocabulary applies inside the group by name: a `@MapField` rename points an inner component at a differently named wire field, a `default` leaf converts one, an `@OptionalBridge` bridges an inner `Optional`, an inner record nests through its own spec, and containers lift. An all-identity group keeps the mapping lossless, so `asIso()` survives and reassembles the record, and a mapping carrying a group is nested by other specs like any other, in the same compilation or from a dependency. Names must be unambiguous: an inner component may not share its name with a domain component or with another group's, a derived field may not be named after one, and a wire component named after the flattened one must be fed by a rename. Spreading is one level deep: a record inside the group nests through its own spec. Not supported yet, each refused with a diagnostic: bean-shaped wires, generic specs, projections, sparse `UpdateSpec`s, and a group wider than one `fields()` ladder. See [Flattening a nested component](mapping/structure.md#flattening-a-nested-component-onto-a-flat-wire).

- **`@GenerateMapping` nests, dispatches and merges across modules** ([#676](https://github.com/higher-kinded-j/higher-kinded-j/issues/676)): a spec compiled in one module is found by the specs of another. Nested and sealed resolution, and `@GenerateMerge` fills, used to see the specs of their own compilation only, so a `CustomerMapping` in `:orders-api` was invisible to an `InvoiceMapping` in `:billing`, and the pair had to be delegated by hand through a leaf. The processor now writes an index entry beside every generated Impl of a `MappingSpec` (an empty class in `org.higherkindedj.mapping.index`, carrying `@MappingIndexEntry` with the spec's name), and a compilation reads the index off its classpath and registers each spec it names from its class file, exactly as if it were declared alongside; concrete, threaded and element-mapped specs alike, with the same unification at the use site. Your own spec wins: a spec in the compilation shadows a classpath spec for the same pair, so a new dependency never changes a resolution that already worked, and a note names the shadowed spec. Two dependencies mapping one pair draw the existing ambiguity error, each candidate listed with its qualified name and `(classpath)` provenance, and the fix names the remedy that applies: a leaf for a nested component, a spec of your own for a sealed subtype pair. A stale entry is passed over, and a use site that needed a spec whose Impl has gone missing is told which dependency to rebuild. Named modules neither write nor read the index, a package two modules cannot share, not supported yet; a resolution that fails inside one says so. See [Across modules](mapping/structure.md#across-modules) and [Multi-module builds](tooling/manual_setup.md#multi-module-builds).

  _Behaviour change in the artefact:_ a module that declares specs now ships index classes in the package `org.higherkindedj.mapping.index`. On a classpath they are exactly what a downstream compilation's annotation processing reads, and nothing at run time touches them; on a module path the package is shared, so two spec-carrying jars cannot be resolved together as automatic modules. A module with its own `module-info` writes no entries and is unaffected, and a library bound for a module path turns the index off with the processor option `-Ahkj.mapping.index=false`, which writes no entries and reads none.

- **`@GenerateMapping` bridges an `Optional` component to a nullable record wire, by opt-in** ([#673](https://github.com/higher-kinded-j/higher-kinded-j/issues/673)): a domain `Optional<T>` against a bean wire's nullable property `T` has always bridged, since bean conventions leave `Optional` off property types. The same pair against a *record* wire did not, so a DTO carrying optional data the way a JSON binder writes it, as a nullable component, had no mapping: `null` parsed to a located `must not be null` while `build` emitted `null` quite happily, and the only remedy was to change the wire contract to `Optional<T>`. `@OptionalBridge` now opts one named component into the bean bridge's semantics: `parse` reads `Optional.ofNullable`, so an absent value is empty rather than a located error, and a present one still converts through the component's leaf and, if it is a container, is still scanned for null elements. `build` writes `null` for an empty `Optional`, on a record wire as on a bean. The annotation goes on a bare abstract marker when the element copies, or on that component's own `default` leaf when it converts; the two placements cannot be combined, because a marker and a same-named leaf are one method with two incompatible return types. Nothing is inferred: wherever the annotation is absent, a `null` on a record wire is still an error, which is the right default for most components. A bridged component is a non-identity correspondence, so a mapping carrying one withholds `asIso()` and a projection carrying one takes the validated `patch`, exactly as a bean bridge has always done. See [Optional fields](mapping/basics.md#optional-bridge).

  _One behaviour change on bean wires:_ a spec declaring a leaf for a bridged component whose element type already matched the property had that leaf silently dropped, and the value was copied unvalidated. An explicit leaf now wins over the bridge's identity copy, the rule every other correspondence already followed, so such a leaf starts running; a normalising one will change what the mapping produces. Otherwise nothing that compiled changes: the record-wire pair was a compile error in 0.4.10, refused with an offer of a whole-`Optional` leaf, which maps the pair without giving it an absent state, and the refusal now names the bridge first and in full. On a bean wire, where the bridge is automatic, the annotation is redundant rather than wrong and draws a note, so one shared mix-in can carry the vocabulary for both wire shapes and for a PATCH sibling.

- **`@GenerateMapping` holds spec members to what the Impl can carry** ([#762](https://github.com/higher-kinded-j/higher-kinded-j/issues/762)): the generated Impl writes a leaf as a constructor-supplied field and a rename as a stub, and three member shapes used to reach javac inside `<Spec>Impl.java` rather than being settled where they are declared. A leaf or rename declaring type parameters of its own (`<R> ValidatedPrism<R, R> items()`) is refused at the declaration, since neither a field nor a stub has anywhere to declare them. A member whose type the spec's package cannot see, whether a package-private type handed over by a mix-in from another package or a private type nested in the spec's own enclosing class, is refused naming the type and the package, the same check `@GenerateIsos` and `@GeneratePathBridge` already make. And same-named members with covariantly differing returns, renames and wildcard-differing leaves alike, fold into one member returning the narrowest declaration, the only one of the declared returns that satisfies the rest; a group with no narrowest (a raw return beside incomparable parameterised ones, which the language admits through unchecked conversion) is refused naming every declaration. See [Diagnostics and limits](mapping/testing.md#diagnostics-and-limits).

  _Behaviour change, moving three javac errors to the declaration:_ each refused shape failed the build before, as `cannot find symbol` or an override error inside the generated file, and a covariant group could emit whichever member the scan reached first. Nothing that generated a compiling Impl changes.

- **`@ThroughField` auto-detects the container interfaces, and explains a narrower one** ([#773](https://github.com/higher-kinded-j/higher-kinded-j/issues/773)): auto-detection matches a field declared as `List`, `Set`, `Collection`, `Map`, `Optional` or an array, on the interface itself. A field declared as something narrower, a concrete container (`ArrayList<String>`, `HashSet<Tag>`, `TreeMap<K, V>`) or another interface (`Deque`, `SortedSet`), is refused at the declaration with the interface it implements and both remedies named, because the standard traversal promises no more than the interface type, which the field could not take back; an array of a primitive (`int[]`) is refused on the same ground, since the array traversal walks an `Object[]`. Detection reads the focus of the spec's own lens for the field, which is what the generated traversal composes with, so an accessor returning `ArrayList` behind a `Lens<S, List<String>>` is accepted, and a spec declaring no lens for the field is told so at the declaration rather than by a `cannot find symbol` in the generated file. See [`@ThroughField` auto-detection](optics/copy_strategies.md#throughfield-auto-detection) and [Compiler Errors](optics/compiler_errors.md#throughfield--reaches-field-items-which-is-declared-as-arrayliststring-rather-than-as-the-list-interface).

  _Behaviour change, replacing a path that threw:_ a spec whose `@ThroughField` reached an `ArrayList`, `HashSet`, `TreeSet` or `TreeMap` field compiled in 0.4.10 and threw `ClassCastException` on the first read or write through the generated traversal (a `HashMap` field survived by accident, since the map traversal rebuilds a `HashMap`). Such a spec is now refused; declare the field as the interface, or name a `traversal` that rebuilds the concrete type.

- **Generator priority decides everywhere** ([#774](https://github.com/higher-kinded-j/higher-kinded-j/issues/774)): when several `TraversableGenerator` SPI providers support one type, the highest `priority()` now wins on every route that chooses a generator. `@GenerateTraversals`, the Focus DSL's widening and `@ImportOptics` read their choice from one shared registry, so `PRIORITY_OVERRIDE` replaces a built-in generator and `PRIORITY_FALLBACK` yields to any default, wherever the `META-INF/services` entry lands. Two providers tied at the winning priority draw a compile-time warning naming both, on each of the three routes, and the first registered wins. See [How Plugin Discovery Works](tooling/generator_plugins.md#how-plugin-discovery-works) and the warning's entry in [Compiler Errors](optics/compiler_errors.md#multiple-traversablegenerator-spi-providers-with-equal-priority-n-support-type-x-a-warning).
...
- **Diagnostics render a type without its type-use annotations** ([#759](https://github.com/higher-kinded-j/higher-kinded-j/issues/759)): a type named in a compile-time message is now built from its element and resolved arguments rather than the mirror's string form, so a component declared `@Nullable Set<?>` is reported as `Set<?>`, the same rendering as the corrected declaration suggested beside it. A message no longer reads as advice to drop a nullness contract it never meant to touch. See [Compiler Errors](optics/compiler_errors.md).

- **`MaybePath`, `OptionalPath` and `AffinePath` take a supplier for the error** ([#794](https://github.com/higher-kinded-j/higher-kinded-j/issues/794)): all three now carry the deferred overload `Maybe.toEither` has had all along, so an error that is not free to build - one that formats a message, reads a `MessageSource`, or captures a stack trace - is built only on the branch that uses it. A lambda, a method reference, or a variable of a `Supplier` type picks the deferred overload; every other argument picks the eager one, an error whose own type is a functional interface included. The one shape to watch is such an error written *as a lambda*, which reads as a supplier and fails to infer; name the error type to pick the eager overload: `path.<MyError>toEitherPath(() -> "boom")`. `AffinePath.toEitherPath` gains the same overload, for the same reason. `toValidationPath` keeps only its eager form: its `Semigroup<E>` parameter pins the same `E` in both candidate signatures, so neither is more specific and the bare lambda - the idiomatic way to write a deferred error - is ambiguous. It compiles only with an explicit type witness or a hoisted `Supplier` variable, which is a worse method than a differently named one. Five book pages documented the deferred form before it existed; [Type Conversions](effect/conversions.md#maybepath--eitherpath) is the one that teaches it.

  _Behaviour change, for two call shapes that compiled before:_ `toEitherPath(null)` used to yield `Left(null)` and now selects the supplier overload and throws; cast the null to the error type (`path.toEitherPath((MyError) null)`) if a null left was meant. And an argument that was a `Supplier` *used as the error itself* now supplies the error rather than becoming it. Neither draws a compiler warning. Every other call site is untouched.

- **`Maybe` bridges to the JDK**: `toOptional()` completes the round-trip `fromOptional` started, and `stream()` renders a `Maybe` as a zero-or-one-element `Stream` with `Optional.stream()`'s contract, so a pipeline of lookups keeps its hits with `.flatMap(Maybe::stream)`. `Maybe` carries `stream()` deliberately alone among the optional-shaped types: an empty stream renders absence faithfully, where the same conversion on `Either` or `Try` would silently discard the error it holds; their discarding conversions stay named for what they drop, the way `EitherPath.toOptionalPath()` is. Four book snippets that already wrote `maybePath.run().toOptional()` now compile as written. See [Maybe Monad](monads/maybe_monad.md#interacting-with-maybe-values).

- **`@ThroughField` composes the auto-detected traversal without the cast** ([#779](https://github.com/higher-kinded-j/higher-kinded-j/issues/779)): over a lens focus whose own type arguments carry no wildcard, the generated composition no longer goes through a raw cast and carries no `@SuppressWarnings`, leaving nothing in the generated method for javac to take on trust: both halves of the composition are checked. The method's declared focus is held at the declaration to what the traversal hands back (the element type, a `Map`'s values, `Object` under a super-wildcard): a `Traversal<Bag, Integer>` over a `List<String>` field, which previously compiled and threw `ClassCastException` on first use, is refused naming the element type. An explicit `traversal` keeps the cast, as the author's undertaking that theirs rebuilds the declared type. See [`@ThroughField` auto-detection](optics/copy_strategies.md#throughfield-auto-detection) and the entry in [Compiler Errors](optics/compiler_errors.md#throughfield--declares-focus-integer-over-field-items-of-type-liststring-whose-elements-the-standard-traversal-hands-back-as-string).
...
- **`Kind` fields take a wildcard element** ([#787](https://github.com/higher-kinded-j/higher-kinded-j/issues/787)): a record component declared `Kind<ListKind.Witness, ? extends Role>` now widens to `TraversalPath<Holder, Role>`, the wildcard resolved to its bound the way `List<? extends Role>` already is, and an unbounded or super-bounded wildcard focuses `Object`. A wildcard inside a parameterised witness resolves the same way, so `Kind<EitherKind.Witness<? extends CharSequence>, Role>` reads `EitherTraverse.<CharSequence>instance()`. The traversal rebuilds the container rather than writing into it, so the component comes back holding a `Kind<ListKind.Witness, Role>`, which its declaration admits. A wildcard in the witness position itself, `Kind<?, Role>`, names no `Traverse` instance and keeps its plain `FocusPath`, with or without `@TraverseField`. See [Kind Field Support](optics/kind_field_support.md#convention-based-detection).

  _Behaviour change, replacing generated source that did not compile:_ in 0.4.10 a wildcard element under a recognised witness, a wildcard inside a parameterised witness's own type arguments, or a wildcard in either position under `@TraverseField` made the generated Focus class fail with `illegal start of type` at the `traverseOver` call, with no diagnostic at the declaration. One family of shapes that did compile changes signature: a witness bounded by a wildcard, `Kind<? extends ListKind.Witness, Role>` or `Kind<? extends EitherKind.Witness<String>, Role>`, was left a plain `FocusPath` because the wildcard witness went unrecognised, and now widens exactly as the plainly declared witness does, so code that assigned the old `FocusPath` or applied `traverseOver` to it by hand stops compiling; take the `TraversalPath` or `AffinePath` and drop the hand-applied step.

- **`@TraverseField` says when it is not applied** ([#789](https://github.com/higher-kinded-j/higher-kinded-j/issues/789)): the annotation names a `Traverse` for a `Kind<F, A>` component with a declared witness. Placed anywhere else, on a component that is not a `Kind`, on a raw `Kind`, or on a `Kind` whose witness is a bare or `? super` wildcard, it used to be passed over without a word, and the component quietly kept the path it would have had anyway. The processor now reports a note naming the component and what would make the annotation apply; the generated class is unchanged, and a note fails no build. The note a `Kind` field already drew for one of the library's own witnesses with no registered `Traverse` is written in the same form and once per component, where before it repeated for every navigator that reached the record. See the two notes' entries in [Compiler Errors](optics/compiler_errors.md#traversefield-the-annotation-on-record-component-xy-is-not-applied-a-note).

- **`@TraverseField` on a type-variable witness draws the same note** ([#790](https://github.com/higher-kinded-j/higher-kinded-j/issues/790)): a `Kind<F, A>` component whose witness is one of the record's own type variables, `Kind<F, String>` in a `Holder<F>`, cannot be traversed through a named `Traverse`, since a `Traverse` is written for one witness and `F` stands for any. With `@TraverseField` on it the processor used to write the annotation's expression into the generated class as it stood, which javac rejected inside generated output. The component now keeps its plain `FocusPath`, as it does without the annotation, and the note that says the annotation is not applied names the variable and what to declare instead. See [Compiler Errors](optics/compiler_errors.md#traversefield-the-annotation-on-record-component-xy-is-not-applied-a-note).

  _Behaviour change, replacing generated source that did not compile:_ since `@TraverseField` arrived, the shape has failed the build inside the generated file, with nothing at the declaration; it now compiles as a plain `FocusPath`, with a note at the declaration. If an expression forced the type with an unchecked cast, it is no longer applied either.

- **Nested generated types carry `@Generated`** ([#798](https://github.com/higher-kinded-j/higher-kinded-j/issues/798)): every nested type a processor writes now carries the marker itself: `XOps.Bound`, `XKind.Witness` and `XKindHelper.XHolder` from `@EffectAlgebra`, `XSupport.BoundSet` from `@ComposeEffects`, the `StageN` classes from `@GenerateAssembly`, the `ContextBuilder` from `@GenerateErrorEnvelope` and the navigator classes from `@GenerateFocus`. A nested type is a class file of its own, and JaCoCo reads the annotation from the class file it measures rather than from an enclosing one, so the outer marker alone left those types counted against a consuming build's coverage floor as though someone had written them. A build that excluded them by class name no longer needs to.

- **Traversals and folds are named nested classes** ([#817](https://github.com/higher-kinded-j/higher-kinded-j/issues/817)): the implementation `@GenerateTraversals`, `@ImportOptics` and `@GenerateFolds` write for each component is now a private nested class carrying `@Generated` (`PlaylistTraversals.SongTitlesTraversal`, `OrderFolds.ItemsFold`) rather than an anonymous class, which no annotation can reach and a coverage tool therefore measured. The public surface is unchanged: each factory still returns `Traversal<S, A>` or `Fold<S, A>`, and only the generated file and the class name in a stack trace differ. Two components that capitalise to the same word take a numeric suffix (`ItemsTraversal2`). In passing, a `@GenerateFolds` record whose own type parameter is named `M` now compiles: the monoid's type variable takes the first name the record leaves free (`M1`), as the effect variable of a traversal already did; and a fold over a `List<? extends X>` or `List<?>` component focuses the type the wildcard stands for, as the traversal over the same component already did, rather than writing the wildcard into a position it cannot occupy.
...
**Mapping at the Boundary** ([#670](https://github.com/higher-kinded-j/higher-kinded-j/issues/670), [#671](https://github.com/higher-kinded-j/higher-kinded-j/issues/671), [#672](https://github.com/higher-kinded-j/higher-kinded-j/issues/672), [#690](https://github.com/higher-kinded-j/higher-kinded-j/issues/690), [#752](https://github.com/higher-kinded-j/higher-kinded-j/issues/752))

- **A chapter of its own**: the record-mapping material becomes [Mapping at the Boundary](mapping/ch_intro.md), covering basics, codecs, structure, tiers, beans and sparse PATCH, generics, merge and envelopes, injection and testing. It opens with the response a client sees and closes with a compiled, law-checked capstone, [One 422, Every Bad Field](mapping/capstone.md). The old `optics/record_mapping.html` URL redirects. Two hands-on journeys sit beside it: [Batching & Coupled Updates](tutorials/optics/batching_journey.md) and [Boundary Mapping](tutorials/optics/boundary_mapping_journey.md).
- **Stock codec vocabulary**: `StandardCodecs` ships one `ValidatedPrism` per standard conversion family (`uuid`, `uri`, `localDate`, `instant`, `offsetDateTime`, `enumByName`, `bigDecimal`, the numeric-from-string trio, `booleanStrict`, `currency`, `locale`), so a typical DTO boundary maps with no hand-written leaves. Every codec accepts exactly the canonical form it renders, and every failure is a located `FieldError` that feeds the 422 leg unchanged. See [Standard codecs](mapping/codecs.md#standard-codecs).
- **`ValidatedPrism.canonical(message, parse, render)`** (plus a `FieldError` overload) wraps a throwing parser and a total render with the build-parse section law guarded per value: an accepted source must render back to itself, so a custom-canon codec is lawful without hand-writing the check. See [Laws](optics/validated_prism.md#laws).
- **Records of any width**: `parse`, the validated `patch` and fallible `@GenerateMerge` legs assemble records beyond 16 components through chunked `Validated.fields()` ladders, with the same located labels and declaration-order accumulation, law-checked at width. The only remaining bound is the JVM's constructor-slot limit on the record itself; the hand-written `fields()` ladder keeps its 16-field arity. See [Diagnostics and limits](mapping/testing.md#diagnostics-and-limits).
- **One leaf vocabulary across tiers**: the sparse `UpdateSpec` tier lifts element leaves over present `List`, `Optional` and `Map` properties exactly as the dense tiers do, every failing element located (`phones.1`), so one mix-in vocabulary serves a spec and its PATCH sibling. See [Sparse PATCH write-back](mapping/beans_patch.md#sparse-patch-write-back-updatespec).
- **Generic mix-ins**: a spec may extend a mix-in declaring type parameters, so `extends Emails<EmailAddress>` contributes `ValidatedPrism<String, EmailAddress>`, read under the spec's instantiation. A generic ancestor reached through a raw `extends` clause is still refused, naming the interface whose clause is raw. See [Generic Specs](mapping/generics.md#generic-mix-ins).

_Behaviour changes:_ a present same-typed identity container in a sparse PATCH now carries the dense tiers' null scan (`tags.1: must not be null` rather than a null reaching the domain); an `Optional`-typed PATCH property with a differing element type, previously rejected, patches through its element leaf; a spec extending a generic mix-in, previously refused, generates.

**Generated optics for generic types** ([#721](https://github.com/higher-kinded-j/higher-kinded-j/issues/721), [#723](https://github.com/higher-kinded-j/higher-kinded-j/issues/723), [#730](https://github.com/higher-kinded-j/higher-kinded-j/issues/730), [#733](https://github.com/higher-kinded-j/higher-kinded-j/issues/733), [#736](https://github.com/higher-kinded-j/higher-kinded-j/issues/736), [#740](https://github.com/higher-kinded-j/higher-kinded-j/issues/740), [#742](https://github.com/higher-kinded-j/higher-kinded-j/issues/742), [#746](https://github.com/higher-kinded-j/higher-kinded-j/issues/746), [#750](https://github.com/higher-kinded-j/higher-kinded-j/issues/750))
...
- **`@ImportOptics` declares the type parameters its signatures name**, in the spec's declaration order with bounds carried along: `BoxOpticsSpec<U> extends OpticsSpec<Box<U>>` generates `<U>`, and a concrete instantiation declares none. An optic method declaring parameters of its own is refused. `@GeneratePrisms` on a generic sealed hierarchy emits parameterised prisms. See [Generic spec interfaces](optics/optics_spec_interfaces.md#generic-spec-interfaces).
- **`@InstanceOf` narrows to what its test can check**: the generated `instanceof` is written under the type arguments the source type pins (`Circle<U>` from `Shape<U>`), an unbounded wildcard elsewhere, and an array target through its component, and the generated methods no longer need `@SuppressWarnings`. A focus asking for more is refused with both remedies, widening to the wildcard or narrowing through `@MatchWhen`. See [Parameterised targets](optics/optics_spec_interfaces.md#parameterised-targets).
- **`@GenerateIsos` answers a generic declaration at the declaration**: an iso naming a type variable, an instance method, one taking arguments, one the generated package cannot see, or a return type that is not a two-argument `Iso` is each refused where it is written, with the remedy, rather than reported by javac inside the generated file. See [Compiler Errors](optics/compiler_errors.md#generateisos-the-iso-returned-by-x-names-a-type-variable).
- **`@ViaCopyAndSet(copyConstructor = ...)` selects the constructor it names**: the attribute resolves to the supertype of `S` it stands for and is emitted as a cast on the argument, `new Config((BaseConfig) source)`, which is what picks between overloaded copy constructors. A name that does not resolve, is not a supertype, cannot be seen, or no constructor accepts is refused at the declaration. See [Copy Strategies](optics/copy_strategies.md#viacopyandset-legacy-types-with-a-copy-constructor-and-setters).
...
_Behaviour changes, source-breaking in narrow places:_ a generated `@ImportOptics` method's arity can fall (`OpticsSpec<Pair<A, String>>` now generates `<A>`, not `<A, B>`), so a call supplying explicit type witnesses stops compiling; drop them, the inferred result is unchanged. An `@InstanceOf` focus naming a type argument the source does not pin (`Prism<Shape, Circle<T>>` on `OpticsSpec<Shape>`), and a target that is a parameterised member of a generic type, are refused. A `copyConstructor` name that resolves now emits a cast, which can select a different constructor than ran in 0.4.9; check overloaded types with `LensLaws`. Generated signatures that now carry `@Nullable` can change what a nullness checker reports in the consuming build.

**The Focus DSL gives one answer** ([#711](https://github.com/higher-kinded-j/higher-kinded-j/issues/711), [#718](https://github.com/higher-kinded-j/higher-kinded-j/issues/718), [#719](https://github.com/higher-kinded-j/higher-kinded-j/issues/719), [#725](https://github.com/higher-kinded-j/higher-kinded-j/issues/725), [#756](https://github.com/higher-kinded-j/higher-kinded-j/issues/756))

- **One widening analysis**: `ShapesFocus.tags()` and `OuterFocus.shapes().tags()` come from one declaration and now report one path type. A navigation method composes the static Focus method for the field it reaches, so the two cannot drift, and a navigated path carries its field-name segments for `pathString()`. See [Path widening](optics/focus_navigation.md#path-widening).
- **`Set` and `Collection` components widen through the `Each` that rebuilds them**: a `Set` through `EachInstances.setEach()`, a `Collection` through the new `EachInstances.collectionEach()`, and `@GenerateTraversals` gains a `Collection` generator. Every route to either, whether Focus, `@GenerateTraversals`, `@ImportOptics` or `@ThroughField`, bottoms out in one rebuild policy in `Traversals`: source iteration order preserved, nulls carried through, an unmodifiable set for a set source and an unmodifiable list for anything else. See [Supported container types](optics/focus_containers.md#supported-container-types) and [Collection Components](optics/traversals.md#collection-components).
- **`@Nullable` detection reaches every recognised annotation**: JSpecify's (`TYPE_USE`), JetBrains', AndroidX's and SpotBugs' `@Nullable` now widen to an `AffinePath` alongside JSR-305's and Jakarta's, and a container decides its own widening before `@Nullable` is consulted. See [Focus DSL Reference](optics/focus_reference.md).
- **A container the widening cannot be written for is explained at the declaration**: an SPI container with a raw or wildcard type argument (`Either<String, ? extends Leaf>`, `Set<?>`) has no ground instantiation to infer its optic instance from, so the diagnostic names the component and the concrete alternative, and the generated file still compiles, leaving one message rather than two. See [Compiler Errors](optics/compiler_errors.md#generatefocus-record-component-xy-has-a-wildcard-type-argument-in-set-extends-t).
- **`@GenerateTraversals` says when it passes over a container**: a component that is a `Collection` or `Map` by erasure and reaches no generator (a `Deque`, a `SortedMap`, a raw `List`) draws a what/why/fix **note** where it is declared. A note rather than a warning, because the annotation has no per-component opt-out and a processor warning cannot be suppressed under `-Werror`. See [Compiler Errors](optics/compiler_errors.md#generatetraversals-no-traversal-was-generated-for-component-xy-of-type-dequet-a-note).

_Behaviour changes, each replacing a path that threw `ClassCastException` on first use:_ a raw or wildcard `Set` or `Collection` component under `@GenerateFocus` is rejected; name the argument, or keep `@GenerateLenses` and `@GenerateTraversals` alone. Three navigator return types move to what the static method reports: a `Collection` *subtype* such as `ArrayList` is a `FocusPath` over the container; an SPI container of non-navigable elements stops at the container until the declaring record sets `widenCollections = true`; a nested `List<Optional<String>>` composes to the leaf, so drop the trailing `.some()`. A component carrying one of the four newly recognised `@Nullable`s generates `AffinePath` where it generated `FocusPath`; read it with `getOptional`. `Traversals.forSet()` and `traverseSet` hand back an **unmodifiable** set (previously a mutable `LinkedHashSet`), and `EachInstances.setEach()` keeps source order across JVM runs.

**Traversals for every component shape** ([#721](https://github.com/higher-kinded-j/higher-kinded-j/issues/721))

- **`@GenerateTraversals` reads a wildcard type argument**: `List<? extends Leaf>` focuses `Leaf`, and `?` or `? super T` focuses `Object`, for `List`, `Map` and every HKJ and third-party container alike. Four more shapes now generate: an array of a type that cannot be created by name (`List<Leaf>[]`, `Leaf[][]`), a primitive array (boxed through, unboxed back), a generic record (`record Holder<T>(List<T> items)` gets `<T>` on the method), and a record naming its own `F`. Every container shape is compiled through the real processor by a sweep that also pins the element type each one focuses. See [Wildcard Element Types](optics/traversals.md#wildcard-element-types) and [Generic Records](optics/traversals.md#generic-records).
...
**Spec interfaces explain what they cannot generate** ([#712](https://github.com/higher-kinded-j/higher-kinded-j/issues/712), [#723](https://github.com/higher-kinded-j/higher-kinded-j/issues/723), [#755](https://github.com/higher-kinded-j/higher-kinded-j/issues/755))

- **A generated prism's focus is a variant of its source**: `@InstanceOf` and `@MatchWhen` are both held to the rule at the declaration, since a prism built back with identity needs a focus that is itself a source. The book's examples now focus a variant, and `Prism.of` is the route where the value type is the point. See [Spec Interfaces](optics/optics_spec_interfaces.md).
- **A `default` method on a spec interface is explained**: a method body cannot be read during annotation processing, so the diagnostic names the two alternatives, a `static` method on the interface or a utility class calling the generated statics, rather than generating a stub. See [Compiler Errors](optics/compiler_errors.md#xopticsspecfoo-is-a-default-method).
...
**`@GeneratePathBridge` emits a bridge the consuming build compiles** ([#746](https://github.com/higher-kinded-j/higher-kinded-j/issues/746), [#748](https://github.com/higher-kinded-j/higher-kinded-j/issues/748))

Members are read under the annotated interface's own instantiation and deduplicated across superinterfaces, with `throws` carried, varargs kept, a self-referential bound still inferring, and the bridge class declaring the interface's type parameters, so a generic service interface bridges cleanly under `-Werror`. `IO<T>` methods bridge through `Path.ioPath`, an inherited `@PathVia` is picked up, and the six shapes with no correct rendering are explained at the declaration. See [Compiler Errors](optics/compiler_errors.md#generatepathbridge-and-pathvia).

_Behaviour changes:_ a varargs delegate's bridge method is varargs, so a bare `null` argument now needs `(String[]) null`; a raw effect return type, a wildcard `Validated` error type, and a `static` or `private` `@PathVia` method are refused; an interface with no `@PathVia` method draws a processor warning.

**Effect handlers** ([#697](https://github.com/higher-kinded-j/higher-kinded-j/issues/697), [#699](https://github.com/higher-kinded-j/higher-kinded-j/issues/699))

- **`@ComposeEffects` generates typed composition support**: `injectX()` returns `Inject<XKind.Witness, …>`, `functor(...)` takes one `Functor` per effect and returns the composed `EitherFFunctor`, and `BoundSet`'s components are each algebra's `Bound`, with no casts and no suppression. See [Composing effects](effect/effect_handlers.md#composing-effects).
- **`@EffectAlgebra` accepts any name for the result type parameter**, so `RenamedOp<T>` generates as `Op<A>` does, and the three permit shapes it cannot generate for (dropping, pinning or bounding the algebra's parameter) are explained at the declaration. See [Defining effects](effect/effect_handlers.md#defining-effects).

_Behaviour changes / migration:_ the last effect in a composition is now injected at the depth its arity implies, which corrects the dispatch of programs that reach it through the generated `*Support`. Each `@ComposeEffects` field must be `Class<XOp<?>>` naming an `@EffectAlgebra`; `BoundSet<F>` becomes `BoundSet`; `functor()` and `BoundSet`'s accessors change erasure. A `*Support` is generated in your own build, so recompiling is the whole migration.

**Compile-time checks**

- **HKJ compiles under its own checker**: every module builds with `-Xplugin:HKJChecker` at zero findings. `discarded-effect` now gates on a sealed `Deferred` capability, so `Path.io(() -> 1).peek(log)` (a silent no-op) is reported while `Path.just(1).peek(log)` (already run) is not, and a `requireNonNull` guard that hands the effect back is read as the pass-through it is. The checker skips `@Generated` types, and a single declaration opts out of one check with `@SuppressWarnings("<check-id>")`, or of all of them with `@SuppressWarnings("hkj-checker")`. See [Compile-Time Checks](tooling/compile_checks.md#opting-out-one-declaration).

**Documentation**

- **The house style**: the mapping chapter established it (the payoff shown before the theory, the common case served before the fine print, mermaid for flows and decisions in a theme-safe palette, "Why this matters" beside the rule it justifies, Key Takeaways on every page), and it now runs through the [Resilience](resilience/ch_intro.md) chapter and every sub-chapter of [Optics](optics/ch_intro.md), the optics diagrams included. The [style guide](https://github.com/higher-kinded-j/higher-kinded-j/blob/main/docs/STYLE-GUIDE.md) records the conventions.
...
**The full mapper** ([#628](https://github.com/higher-kinded-j/higher-kinded-j/issues/628), [#645](https://github.com/higher-kinded-j/higher-kinded-j/issues/645), [#625](https://github.com/higher-kinded-j/higher-kinded-j/issues/625), [#624](https://github.com/higher-kinded-j/higher-kinded-j/issues/624), [#623](https://github.com/higher-kinded-j/higher-kinded-j/issues/623))
...
- **Sparse PATCH write-back**: a spec extending `UpdateSpec<D, W>` (bean wire only) generates `updateFrom(Wire) : Edits.Accumulated<Domain>`, the REST PATCH contract as a validated fold: null means keep, present values are set or parsed, every invalid present field reports at once, and shapes that cannot honour null-as-absent (record wires, primitives, `Optional` bridges, sealed hierarchies) are rejected. The example app gains a worked `PATCH /api/users/{id}` endpoint ([#647](https://github.com/higher-kinded-j/higher-kinded-j/pull/647)). See [Sparse PATCH write-back](mapping/beans_patch.md#sparse-patch-write-back-updatespec).
- **The validated `patch` tier**: a projection carrying fallible correspondences swaps `asLens()` for a dense `patch(domain, wire) : Validated<NonEmptyList<FieldError>, Domain>` write-back, validating every projected component at once while unprojected ones survive by construction. Deliberately the opposite of the sparse tier: `patch` treats a missing value as an error, `updateFrom` as keep-the-current-one. See [Leaf-carrying projections](mapping/tiers.md#leaf-carrying-projections-the-validated-patch).
- **Generic records, three ways, all nestable**: concrete instantiations (`MappingSpec<Page<User>, PageDto<UserDto>>`), threaded specs (one generic Impl behind `instance()`), and element-mapped specs (an abstract `ValidatedPrism<TDto, T>` leaf supplied through a generated `of(...)` factory), with use sites resolving by type-argument unification and composing `of(...)` in place. Record-to-record only; raw and wildcard uses are diagnosed. See [Generic records](mapping/generics.md).
- **Shared vocabulary**: specs may extend plain mix-in interfaces carrying renames, leaves and derived fields, collected with Java's own precedence and diagnosed with the declaring interface named. See [Shared vocabulary](mapping/codecs.md#shared-vocabulary-mix-in-interfaces).
...
_Behaviour changes against v0.4.8, all in the friendly direction but observable:_
...
- The generator is stricter about silent mistakes: a locally declared leaf naming no domain component is now a compile error with a nearest-name hint (`Did you mean 'email()'?`) instead of silently validating nothing; spec methods colliding with generated members are rejected ([#654](https://github.com/higher-kinded-j/higher-kinded-j/issues/654)); and a leaf on a projected component now takes effect (selecting the `patch` tier) instead of being silently ignored.

**Spring: the 422 leg** ([#627](https://github.com/higher-kinded-j/higher-kinded-j/issues/627))

A controller returning any all-located-`FieldError` `Invalid` (a mapper `parse`, `patch` or `updateFrom` result, a `Path.fields()` assembly, an `@GenerateAssembly` result) renders as a single **422 Unprocessable Content** response listing every bad field, each with its display `path`, lossless `segments` and `message`. The leg is chosen by payload shape alone; the status is configurable via `hkj.web.validation-field-error-status`. Paths use domain component names, renames included. See [The 422 leg](spring/spring_boot_integration.md#the-422-leg).

_Behaviour change:_ a pre-existing endpoint returning an all-`FieldError` payload moves from 400 to **422** on upgrade; set `hkj.web.validation-field-error-status: 400` to restore it. Tests and examples adopt Spring Framework 7's `HttpStatus.UNPROCESSABLE_CONTENT` (the RFC 9110 rename).
...
- [VResultPath](effect/path_vresult.md): A first-class railway for `VTask<Either<E, A>>` (async work that can fail with a typed domain error), speaking the family vocabulary (`map`/`via`/`then`, `mapError`/`recover`/`recoverWith`/`bimap`) with no `EitherT` bridges. Structured-concurrency combinators (`firstSuccess`, `allSucceed`/`allSucceedAccumulating`, `withTimeout`, `bracketOutcome`) keep typed failures in the value channel; defects stay on the `VTask` channel ([#606](https://github.com/higher-kinded-j/higher-kinded-j/issues/606))
- [Path-native resilience](resilience/ch_intro.md): `withRetry`/`withTimeout`/`withCircuitBreaker`/`withBulkhead` now form one `with*` vocabulary across the family: instance-chained on the lazy carriers (`IOPath`, `VTaskPath`, `VResultPath`), static (step-as-`Supplier`) on the eager `EitherPath`. Railway-aware: a business `Left` is a value (never retried, never trips the breaker), while typed overloads opt selected transient errors into retry and land timeouts/rejections as `Left`s ([#607](https://github.com/higher-kinded-j/higher-kinded-j/issues/607))

**Validation and Error Accumulation**

A non-empty error channel, an inclusive-or, and open-arity validated assembly.

- [NonEmptyList](monads/nonemptylist_monad.md): A list that encodes "at least one element" in the type, so `head`/`last`/`reduce` are total. The canonical companion to `Validated` (mirroring Cats' `NonEmptyList`/`ValidatedNel`): `Path.validNel`/`invalidNel` and `Validated.validNel`/`invalidNel` bake in its semigroup, dropping the manual `Semigroups.list()` argument. Full `Kind`/`Functor`/`Monad`/`Traverse` support, an `assertThatNonEmptyList` assertion, and Jackson support in `hkj-spring`. Purely additive ([#549](https://github.com/higher-kinded-j/higher-kinded-j/issues/549))
- [EitherOrBoth](monads/either_or_both_monad.md): The inclusive-or (`Ior`/`These`) for a success that also carries non-fatal warnings: sealed over `Left`/`Right`/`Both`, right-biased, with total `Maybe`-returning accessors. Accumulating `flatMap`, a `Kind`/`Kind2` with `Bifunctor`, an [EitherOrBothPath](effect/path_either_or_both.md) railway, plus `hkj-spring` Jackson support and a warnings-in-header return handler. Purely additive ([#551](https://github.com/higher-kinded-j/higher-kinded-j/issues/551), [#583](https://github.com/higher-kinded-j/higher-kinded-j/pull/583))
- [Open-arity accumulating assembly](monads/validated_assembly.md): `Validated.fields()`/`accumulate()` assemble a record from N validated fields (any arity to 16) with every error collected in declaration order, no `Semigroup` argument, and no `Kind` ceremony. Errors are located `FieldError`s with composable paths; one shape across three carriers: `Validated` (strict), `ValidationPath` (railway), and `EitherOrBoth` (tolerant) ([#581](https://github.com/higher-kinded-j/higher-kinded-j/issues/581))
- [@GenerateAssembly](monads/validated_assembly.md): The codegen layer for the above: annotate a record and get a same-package `UserAssembly.fields().name(v)…assemble()` companion with one order-enforcing method per component; a component typed as another annotated record accepts its sub-companion's result directly ([#586](https://github.com/higher-kinded-j/higher-kinded-j/issues/586))
- [Assembly arity ceiling](monads/validated_assembly.md): the shared `accumulate()`/`fields()` ladder, and the `@GenerateMapping`/`@GenerateMerge` `parse` built on it, locate up to **16 components**; the ladder generates its own `Tuple13..16` so the For-comprehension arity stays independent at 12 ([#626](https://github.com/higher-kinded-j/higher-kinded-j/issues/626))
- [PathOps first-success NonEmptyList overloads](effect/advanced_topics.md): The five race/first-success combinators gain total `NonEmptyList` overloads beside the throwing `List` ones, so a statically-known competitor set needs no empty guard ([#579](https://github.com/higher-kinded-j/higher-kinded-j/issues/579), [#585](https://github.com/higher-kinded-j/higher-kinded-j/pull/585))

**Record Mapping and Typed-Error Codegen**

Three annotation processors for the record↔DTO boundary.

- [@GenerateMapping](mapping/ch_intro.md): Annotate `interface UserMapping extends MappingSpec<User, UserDto>` and the processor generates a total `build` plus an accumulating `parse` returning `Validated<NonEmptyList<FieldError>, User>`. `@MapField` renames, nesting, `List`/`Optional`/`Map` container lifting (map values lift like list elements, keys are identity), derived wire fields (a `Getter`-returning `default` method fills a wire-only component on `build`), sealed-interface dispatch, and truthful emission tiers (`asIso` / `asLens` / accumulating `parse`). Every tier is law-checked against the published `hkj-test` harness ([#600](https://github.com/higher-kinded-j/higher-kinded-j/issues/600))
- [@GenerateMerge](mapping/ch_intro.md): The forward-only assembler: annotate an interface whose one method declares `DashboardDto assemble(User, Account, Settings)` and the processor fills each target component from the same-named source (identity or through a `ValidatedPrism` leaf). Ambiguous or unfilled components are what/why/fix compile errors; no inverse is generated ([#613](https://github.com/higher-kinded-j/higher-kinded-j/issues/613))
- [@GenerateErrorEnvelope](mapping/merge_envelopes.md#generating-error-envelopes-generateerrorenvelope): Each variant of a sealed error hierarchy declares only its domain fields plus one `ErrorEnvelope<C>` component, and the processor generates the `<Name>s` companion: per-variant factories, a typed context builder, and an `editContext` wither. Context is records-as-schema (`context.orderId()`, not `map.get(...)`); timestamps read from a `TimeSource` for deterministic tests ([#610](https://github.com/higher-kinded-j/higher-kinded-j/issues/610))

**Optics**

- [ValidatedPrism](optics/validated_prism.md): The smart-constructor optic for parse-don't-validate boundaries: `parse` returns `Validated<NonEmptyList<FieldError>, A>` (every failure located), `build` is total. Nested composition short-circuits while siblings accumulate; both round-trip laws ship as `ValidatedPrismLaws` in `hkj-test`. Purely additive ([#597](https://github.com/higher-kinded-j/higher-kinded-j/issues/597))
- [Published optic-law harness](tooling/test_assertions.md): `hkj-test` gains `org.higherkindedj.optics.laws` (`IsoLaws`, `LensLaws`, `PrismLaws`, `AffineLaws`, `TraversalLaws`), so users can law-test hand-written optics; failures name the violated law with the offending values. Purely additive ([#596](https://github.com/higher-kinded-j/higher-kinded-j/issues/596))
- [Optic-path labelling](optics/multi_edit.md): `@GenerateFocus` companions emit the component name as a path segment, and every path type surfaces `segments()`/`pathString()` (`"customer.address.zip"`), so `Edit.parseIfPresent` locates parse failures automatically. Purely additive ([#592](https://github.com/higher-kinded-j/higher-kinded-j/issues/592))
- [Edits](optics/multi_edit.md): Sparse, accumulating multi-edit over optics: `Edits.combine` folds pure edits into one `Update<S>`; `Edits.accumulate` adds the validated REST-`PATCH` shape, reporting every bad field at once and applying the writes only if all validated. `…IfPresent` treats `null` as absent. Purely additive ([#582](https://github.com/higher-kinded-j/higher-kinded-j/issues/582))
- [Update\<S\> and Monoids.update()](functional/semigroup_and_monoid.md): A named, composable update (`UnaryOperator<S>` with `identity`/`andThen`) and its monoid (the `Endo` monoid), the keystone under `Edits`. Purely additive ([#591](https://github.com/higher-kinded-j/higher-kinded-j/issues/591))

**Tooling, Build, and Internals**

- [TimeSource](monads/io_monad.md): `java.time.Clock` lifted into the effect world (`system()`/`of(clock)`/`fixed(instant)`, lazy `now()` / `nowAsync()`), deterministic in tests; deliberately not named `Clock`. `hkj-test` gains a `SteppableClock` so time-dependent code is exercised by moving the clock, not sleeping ([#609](https://github.com/higher-kinded-j/higher-kinded-j/issues/609))
- Processor diagnostics: what / why / fix. Annotation-processor errors now follow a shared three-part format (what is wrong, why, the exact fix), adopted across the `@GenerateFocus`/`@GenerateLenses`/`@ImportOptics` error paths and the standard for every future processor ([#601](https://github.com/higher-kinded-j/higher-kinded-j/issues/601))
- Codegen on-ramp: `-parameters` wired by both build plugins. The Gradle and Maven plugins now add `-parameters` automatically, completing the one-line codegen setup ([#602](https://github.com/higher-kinded-j/higher-kinded-j/issues/602))
- Annotation-processor hygiene: Every processor now reports `SourceVersion.latestSupported()` (no warnings on newer JDKs) and threads originating elements through the `Filer` for correct incremental processing; generated output is byte-identical ([#588](https://github.com/higher-kinded-j/higher-kinded-j/issues/588))
- `hkj-spring` deserializers resolve generic element types: The `Either`/`Validated`/`NonEmptyList` deserializers now implement Jackson 3.x contextual resolution, so nested custom types round-trip instead of yielding `LinkedHashMap` and a `ClassCastException` ([#578](https://github.com/higher-kinded-j/higher-kinded-j/pull/578))
