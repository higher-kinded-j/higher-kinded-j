 ...
 *
 * <ol>
 *   <li>A Semigroup: Provides only one operation: combine. It's for things that can be combined.
 *   <li>A Monoid: Provides combine and empty. It's for things that can be combined and also have a
 *       "zero" or "identity" value.
 * </ol>
 *
 * <p>By only having Monoid, you force every operation that needs to combine things to also require
 * an empty value, even when it makes no sense. This comes to the front in Validated. To accumulate
 * errors we only need a way to combine. We don't need an empty error. There are many useful types
 * that are Semigroups but cannot be Monoids.
 *
 * <p>A classic example is a non-empty list. You can always combine two non-empty lists to get a
 * new, larger non-empty list. However, there is no "empty" non-empty list, so it can't be a Monoid.
