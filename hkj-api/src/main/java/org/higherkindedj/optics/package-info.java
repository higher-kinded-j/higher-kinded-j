 ...
 *       (e.g., a sealed interface). A Prism can be seen as a "partial" Lens—it allows you to
 *       get an optional value (only if the variant matches) and to construct the whole from a part.
 *       For example, focusing on the {@code JsonString} case of a {@code Json} sealed interface.
 ...
 * together to focus deeply into nested data structures. For example, you could compose a Prism
 * and a Lens to focus on a field inside a specific variant of a sealed type.
 *
 * <pre>{@code
 ...
 * abstraction and integrate with type classes like {@link org.higherkindedj.hkt.Functor} and
 * {@link org.higherkindedj.hkt.Applicative}. This allows for powerful, generic operations like
 * traversing a structure and applying effectful modifications using an Applicative context (e.g.,
 * validating and modifying fields at the same time).
 */
@NullMarked
