...
public class ValidatedTraverseTest {
  ...
  void mapValid() {
    ...
    assertThat(VALIDATED.narrow(result)).isEqualTo(Validated.valid("Value: 10"));
  }
  ...
  void mapInvalid() {
    ...
    assertThat(VALIDATED.narrow(result)).isEqualTo(Validated.invalid("Error"));
  }
  ...
  void traverseValidSucceeds() {
    ...
    Kind<ValidatedKind.Witness<String>, Integer> input = VALIDATED.widen(validated);
    ...
    assertThat(VALIDATED.narrow(maybeResult.get())).isEqualTo(Validated.valid("Number is 123"));
  }
  ...
  void traverseValidFails() {
    ...
    Validated<String, Integer> validated = Validated.valid(-1);
    Kind<ValidatedKind.Witness<String>, Integer> input = VALIDATED.widen(validated);
  ...
  void traverseInvalid() {
    // Given an Invalid value
    Validated<String, Integer> validated = Validated.invalid("Initial Error");
    Kind<ValidatedKind.Witness<String>, Integer> input = VALIDATED.widen(validated);
    ...
    Maybe<Kind<ValidatedKind.Witness<String>, String>> maybeResult = MAYBE.narrow(result);
    assertThat(maybeResult.isJust()).isTrue();
    assertThat(VALIDATED.narrow(maybeResult.get())).isEqualTo(Validated.invalid("Initial Error"));
  }
  ...
  void sequenceValidSucceeds() {
    ...
    assertThat(maybeResult.isJust()).isTrue();
    assertThat(VALIDATED.narrow(maybeResult.get())).isEqualTo(Validated.valid(123));
  }
  ...
  void sequenceInvalid() {
    // Given an Invalid value
    final Validated<String, Kind<MaybeKind.Witness, Integer>> validated =
        Validated.invalid("Initial Error");
    final Kind<ValidatedKind.Witness<String>, Kind<MaybeKind.Witness, Integer>> input =
        VALIDATED.widen(validated);
