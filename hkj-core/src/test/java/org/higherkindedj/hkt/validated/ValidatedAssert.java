...
public class ValidatedAssert<E, A> {
  ...
  public ValidatedAssert<E, A> isValid() {
    assertTrue(
        validated.isValid(),
        String.format(
            "%s should be Valid but was Invalid with error: %s",
            description, validated.fold(Object::toString, v -> "N/A")));
    return this;
  }
  ...
  public ValidatedAssert<E, A> isInvalid() {
    assertTrue(
        validated.isInvalid(),
        String.format(
            "%s should be Invalid but was Valid with value: %s",
            description, validated.fold(e -> "N/A", Object::toString)));
    return this;
  }
  ...
  public ValidatedAssert<E, A> hasValueSatisfying(
    ...
    A actual =
        validated.fold(
            ...
            v -> v);
    assertTrue(
        predicate.test(actual),
        String.format(
            "%s value should satisfy: %s, but value was: %s",
  ...
  public ValidatedAssert<E, A> hasErrorSatisfying(
    ...
    assertTrue(
        predicate.test(actual),
        String.format(
            "%s error should satisfy: %s, but error was: %s",
            description, predicateDescription, actual));
    return this;
  }
   ...
   * @return this ValidatedAssert for chaining
   * @throws AssertionError if the Validated is Invalid or the value is not an instance of the
   *     expected class
   */
   ...
   * @param expectedClass the expected class
   * @return this ValidatedAssert for chaining
   * @throws AssertionError if the Validated is Valid or the error is not an instance of the
   *     expected class
   */
  ...
  public ValidatedAssert<E, A> isEqualTo(Validated<E, A> expected) {
    assertEquals(expected, validated, String.format("%s should equal %s", description, expected));
    return this;
  }
  ...
  public ValidatedAssert<E, A> isNotEqualTo(Validated<E, A> other) {
    assertNotEquals(other, validated, String.format("%s should not equal %s", description, other));
    return this;
  }
}
