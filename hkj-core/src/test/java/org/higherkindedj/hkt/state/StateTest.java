...
class StateTest extends TypeClassTestBase<StateKind.Witness<Integer>, Integer, String> {
    ...
    class EdgeCasesAndBoundaryConditions {
        ...
        void stateOperationsAreStackSafe() {
            ...
            // TODO: When Trampoline is available in https://github.com/higher-kinded-j/higher-kinded-j/issues/120
    ...
    class LazyEvaluationTests {
        ...
        void mapDefersExceptions() {
            ...
            Function<Integer, String> throwingMapper = i -> { throw testException; };
        ...
        void exceptionsPropagateThroughChains() {
            ...
            State<Integer, String> computation =
                    incrementState
                            .map(i -> i * 2)
                            .map(i -> { throw testException; })  // This step will fail
                            .map(String::valueOf);  // This never executes

            assertThatThrownBy(() -> computation.run(initialState))
                    .isSameAs(testException);
        }
    }
}