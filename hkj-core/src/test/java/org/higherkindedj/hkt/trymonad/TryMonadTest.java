...
class TryMonadTest extends TypeClassTestBase<TryKind.Witness, String, Integer> {
    ...
    void flatMap_shouldRejectNullResultFromFunction() {
      ...
      assertThatThrownBy(tryResult::get)
          .hasMessageContaining(
              "Function f in TryMonad.flatMap returned null when Kind expected, which is not"
                  + " allowed");
    }
  }
  ...
  class HandleErrorWithOperationTests {
    ...
    void handleErrorWith_shouldRejectNullResultFromHandler() {
      ...
      assertThatThrownBy(tryResult::get)
          .hasMessageContaining(
              "Function handler in TryMonad.handleErrorWith returned null when Kind expected, which"
                  + " is not allowed");
    }
  }
  ...
  class EdgeCaseTests {
    ...
    void handleErrorWith_shouldHandleDifferentExceptionTypes() {
      ...
      assertThatCode(() -> assertThat(tryResult2.get()).isEqualTo("Recovered from: IOException"))
          .doesNotThrowAnyException();
  ...
  class IntegrationTests {
    ...
    void combiningFlatMapAndHandleErrorWith() {
      ...
      Kind<TryKind.Witness, String> validInput = TryKindHelper.TRY.widen(Try.success("test"));
      ...
      assertThat(tryResult1.isSuccess()).isTrue();
      assertThatCode(() -> assertThat(tryResult1.get()).isEqualTo(4)).doesNotThrowAnyException();
      ...
      assertThat(tryResult2.isSuccess()).isTrue();
      assertThatCode(() -> assertThat(tryResult2.get()).isEqualTo(-1)).doesNotThrowAnyException();
    }
