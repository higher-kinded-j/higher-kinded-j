...
final class ValidatedSelectiveTestExecutor<E, A, B>
    ...
    protected void executeValidationTests() {
        ...
        Kind<ValidatedKind.Witness<E>, B> effectKind = VALIDATED.widen(Validated.valid(null));
    ...
    private void testWhenS() {
        ...
        Kind<ValidatedKind.Witness<E>, A> resultTrue = selective.whenS(trueKind, effectKind);
        Validated<E, A> validatedResultTrue = VALIDATED.narrow(resultTrue);
        assertThat(validatedResultTrue.isValid()).isTrue();

        // Test whenS with false - effect should not execute, returns Valid(null)
        Kind<ValidatedKind.Witness<E>, A> resultFalse = selective.whenS(falseKind, effectKind);
        ...
        Kind<ValidatedKind.Witness<E>, A> errorResult = selective.whenS(errorCondKind, effectKind);
        Validated<E, A> validatedErrorResult = VALIDATED.narrow(errorResult);
        assertThat(validatedErrorResult.isInvalid()).isTrue();

        // Test whenS with both Invalid - should accumulate errors
        Validated<E, A> errorEffect = Validated.invalid(invalidInstance.getError());
        Kind<ValidatedKind.Witness<E>, A> errorEffectKind = VALIDATED.widen(errorEffect);
    ...
    private void testIfS() {
        ...
        Validated<E, Boolean> errorCondition = Validated.invalid(invalidInstance.getError());
        Kind<ValidatedKind.Witness<E>, Boolean> errorCondKind = VALIDATED.widen(errorCondition);
        Kind<ValidatedKind.Witness<E>, A> errorResult = selective.ifS(errorCondKind, thenKind, elseKind);
        Validated<E, A> validatedErrorResult = VALIDATED.narrow(errorResult);
        assertThat(validatedErrorResult.isInvalid()).isTrue();
