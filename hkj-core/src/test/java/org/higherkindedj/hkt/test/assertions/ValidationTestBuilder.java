...
public final class ValidationTestBuilder {
  ...
  /** Failures accumulate by concatenation under {@link Monoids#list()} (empty = all passed). */
  private static final Monoid<List<AssertionError>> FAILURES = Monoids.list();
    ...
    List<AssertionError> failures =
        FAILURES.combineAll(
            IntStream.range(0, assertions.size())
                .mapToObj(i -> runOne(i + 1, assertions.get(i)))
                .toList());
  ...
  /** Runs one assertion, returning its failure as a singleton list or the empty Monoid identity. */
  private static List<AssertionError> runOne(int ordinal, ValidationAssertion assertion) {
    try {
      assertion.run();
      return List.of();
    } catch (AssertionError e) {
      return List.of(new AssertionError("Validation assertion " + ordinal + " failed", e));
    } catch (Exception e) {
      return List.of(
          new AssertionError(
              "Validation assertion " + ordinal + " failed with unexpected exception", e));
    }
  }
