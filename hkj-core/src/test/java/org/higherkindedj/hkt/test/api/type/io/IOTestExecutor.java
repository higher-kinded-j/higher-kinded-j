...
final class IOTestExecutor<A, B> {
    ...
    IOTestExecutor(
        ...
        this(contextClass, ioInstance, mapper,
                includeFactoryMethods, includeExecution, includeMap, includeFlatMap,
                includeValidations, includeEdgeCases, null);
    }
    ...
    private void testFlatMap() {
        ...
        IO<String> chainedIO = ioInstance
                .flatMap(flatMapper)
                .flatMap(b -> IO.delay(() -> b.toString()));
        ...
        Function<A, IO<B>> throwingFlatMapper = a -> {
            throw testException;
        };
    ...
    void testValidations() {
        ...
        if (validationStage != null && validationStage.getFlatMapContext() != null) {
            ...
            // Use the instance method
            builder.assertFlatMapperNull(() -> ioInstance.flatMap(null), flatMapContext, Operation.FLAT_MAP);
        }

        // Delay validation
        builder.assertFunctionNull(
                () -> IO.delay(null), "thunk", contextClass, Operation.DELAY);
    ...
    private void testEdgeCases() {
        ...
        IO<String> lazyWithSideEffect = IO.delay(() -> {
            sideEffect.set(true);
            return "executed";
        });
