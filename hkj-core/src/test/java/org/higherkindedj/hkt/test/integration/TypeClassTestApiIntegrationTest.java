 ...
 *
 * <p>Note: These tests focus on API flow and skip validations because the new API
 * doesn't yet support flexible validation contexts (map() is in EitherFunctor but we're
 * testing EitherMonad). Validation behavior is thoroughly tested in the individual
 * type class tests (EitherFunctorTest, EitherMonadTest, etc.).
 */
...
class TypeClassTestApiIntegrationTest {
    ...
    class ProgressiveDisclosureTests {
        ...
        void stageProgression() {
            ...
            assertThatCode(() -> stage4.withEqualityChecker(equalityChecker).testAll()).doesNotThrowAnyException();
        }
    }
