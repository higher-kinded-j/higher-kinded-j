...
/**
 ...
 <li>Specialized factory methods per type (Either, Maybe, etc.)</li>


 <li>Automatic helper adaptation - no manual work needed</li>


 <li>Type-safe fluent API with proper inference</li>


 <li>Consistent with TypeClassTest entry point</li>
...
// ============================================================================
    public static final class EitherKindHelperConfig<L, R>
        ...
        private static final EitherKindHelper EITHER =
...
// ============================================================================
    public static final class MaybeKindHelperConfig<A>
        ...
        private static final MaybeKindHelper MAYBE =
...
private MaybeKindHelperConfig(Maybe<A> instance) {
            super(
                    instance,
                    getMaybeClass(),
                    maybe -> MAYBE.widen(maybe),
                    kind -> MAYBE.narrow(kind));
        }
...
// ============================================================================
    public static class BaseKindHelperConfig<T, F, A> {
        ...
        public void test() {
            ...
            if (includeRoundTrip) {
                KindHelperTestPattern.testRoundTripWithHelper(
                        instance, helper);
            }
            if (includeValidations) {
                KindHelperTestPattern.testValidationsWithHelper(
                        targetClass, helper);
            }
            if (includeInvalidType) {
                KindHelperTestPattern.testInvalidTypeWithHelper(
                        targetClass, helper);
            }
            if (includeIdempotency) {
                KindHelperTestPattern.testIdempotencyWithHelper(
                        instance, helper);
            }
            if (includeEdgeCases) {
                KindHelperTestPattern.testEdgeCasesWithHelper(
                        instance, targetClass, helper);
            }
            if (includePerformance) {
                KindHelperTestPattern.testPerformance(
                        instance, widenFunc, narrowFunc);
            }
            if (includeConcurrency) {
                KindHelperTestPattern.testConcurrentAccess(
                        instance, widenFunc, narrowFunc);
            }
        }
        ...
        public void testWithValidation(Class<?> helperClass) {
            test();
            KindHelperTestPattern.validateImplementationStandards(
