...
class ContextTest {
  ...
  class EdgeCases {
    ...
    @Test
    @DisplayName("Stack safety: very deep flatMap chain should not overflow")
    void stackSafety_veryDeepFlatMapChain() {
      // This depth would cause StackOverflowError without trampolining
      final int depth = 10_000;

      Context<String, Integer> ctx = Context.succeed(0);
      for (int i = 0; i < depth; i++) {
        ctx = ctx.flatMap(n -> Context.succeed(n + 1));
      }
    ...
    @Test
    @DisplayName("Stack safety: deep recoverWith chain should not overflow")
    void stackSafety_deepRecoverWithChain() {
      // This depth would cause StackOverflowError without trampolining
      final int depth = 10_000;
    ...
    @Test
    @DisplayName("Stack safety: mixed flatMap and recoverWith chain should not overflow")
    void stackSafety_mixedChain() {
      final int depth = 5_000;
    ...
    @Test
    @DisplayName("Error should propagate through flatMap chain without recovery handlers")
    void errorPropagationThroughFlatMapChainWithoutRecovery() {
      // This tests the code path in evaluate() where an error occurs and no RecoveryHandler
      // is found on the stack (lines 541-553), specifically the case where we skip
      // continuations and eventually rethrow the error.
      RuntimeException originalError = new RuntimeException("original error");

      // Create a chain: succeed -> flatMap -> flatMap -> fail
      // The error should propagate through the continuations without recovery
    ...
    @Test
    @DisplayName("Error should propagate skipping continuations to reach recovery handler")
    void errorShouldSkipContinuationsToReachRecoveryHandler() {
      // This tests the code path where error occurs, we skip continuations (non-recovery items)
      // until we find a RecoveryHandler on the stack.
      // The recoverWith must WRAP the failing computation to catch the error.
      RuntimeException error = new RuntimeException("mid-chain error");

      // Chain: succeed -> flatMap -> flatMap -> fail, wrapped by recoverWith
      // The error should skip the flatMap continuations and be caught by recoverWith
      Context<String, Integer> ctx =
          Context.<String, Integer>succeed(1)
              .flatMap(n -> Context.succeed(n + 1))
              .flatMap(n -> Context.succeed(n + 1))
    ...
    @Test
    @DisplayName("ErrorMapped should transform error from inner computation")
    void errorMappedShouldTransformInnerError() {
      // This ensures ErrorMapped.run() catch block is executed (line 495)
      // by creating an ErrorMapped that wraps a computation that throws
      RuntimeException innerError = new RuntimeException("inner error");
      Context<String, String> innerCtx = Context.fail(innerError);

      // Wrap with ErrorMapped - the mapError path in evaluate() or direct run()
      Context<String, String> errorMapped =
          innerCtx.mapError(e -> new IllegalStateException("transformed: " + e.getMessage()));

      assertThatThrownBy(errorMapped::run)
          .isInstanceOf(IllegalStateException.class)
          .hasMessage("transformed: inner error");
    }

    @Test
    @DisplayName("ErrorMapped in flatMap chain should transform error")
    void errorMappedInFlatMapChainShouldTransformError() {
      // This tests ErrorMapped when it's part of a more complex chain
      RuntimeException error = new RuntimeException("chain error");
