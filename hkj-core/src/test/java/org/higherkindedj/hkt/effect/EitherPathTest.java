...
class EitherPathTest {
  ...
  class ResilienceStepCombinators {
    ...
    @Test
    @DisplayName("withCircuitBreaker: the typed overload passes a success through")
    void breakerTypedOverloadSuccess() {
      EitherPath<String, Integer> result =
          EitherPath.withCircuitBreaker(
              () -> Path.<String, Integer>right(5), CircuitBreaker.withDefaults(), open -> "shed");
      assertThat(result.run().getRight()).isEqualTo(5);
    }

    @Test
    @DisplayName("withCircuitBreaker: the typed overload passes an unrelated defect through")
    void breakerTypedOverloadPassesOtherDefectsThrough() {
      assertThatExceptionOfType(IllegalStateException.class)
          .isThrownBy(
              () ->
                  EitherPath.withCircuitBreaker(
                          () -> {
                            throw new IllegalStateException("step defect");
                          },
                          CircuitBreaker.withDefaults(),
                          open -> "shed")
                      .run())
          .withMessage("step defect");
    }
    ...
    @Test
    @DisplayName("withBulkhead: the typed overload passes a success through")
    void bulkheadTypedOverloadSuccess() {
      EitherPath<String, Integer> result =
          EitherPath.withBulkhead(
              () -> Path.<String, Integer>right(5),
              Bulkhead.withMaxConcurrent(1),
              full -> "no permit");
      assertThat(result.run().getRight()).isEqualTo(5);
    }

    @Test
    @DisplayName("withBulkhead: the typed overload passes an unrelated defect through")
