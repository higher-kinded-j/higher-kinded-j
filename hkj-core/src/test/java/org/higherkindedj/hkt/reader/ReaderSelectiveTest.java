  ...
  class PerformanceTests {
    ...
    void selectiveOperationsEfficientWithManyConditionals() {
      if (Boolean.parseBoolean(System.getProperty("test.performance", "false"))) {
        ...
        for (int i = 0; i < 100; i++) {
          final int index = i;
          Kind<ReaderKind.Witness<TestConfig>, Boolean> condition =
              selective.map(val -> val > index, result);
          Kind<ReaderKind.Witness<TestConfig>, Integer> thenValue =
        ...
        Reader<TestConfig, Integer> reader = READER.narrow(result);
        Integer value = reader.run(TEST_CONFIG);
        assertThat(value).isGreaterThan(DEFAULT_MAX_CONNECTIONS);
      }
    }

    @Test
    @DisplayName("Selective operations maintain lazy evaluation")
    void selectiveOperationsMaintainLazyEvaluation() {
      AtomicInteger evaluationCount = new AtomicInteger(0);

      Reader<TestConfig, Integer> trackingReader =
          Reader.of(
              cfg -> {
      ...
      Kind<ReaderKind.Witness<TestConfig>, Integer> trackingKind = READER.widen(trackingReader);

      // Create complex selective chain
      Kind<ReaderKind.Witness<TestConfig>, Boolean> condition =
          selective.map(i -> i > 0, trackingKind);
      Kind<ReaderKind.Witness<TestConfig>, Integer> result =
          selective.ifS(condition, trackingKind, trackingKind);

      // Should not evaluate until run is called
      assertThat(evaluationCount.get()).isEqualTo(0);

      // Now run and verify evaluation
      Reader<TestConfig, Integer> reader = READER.narrow(result);
      reader.run(TEST_CONFIG);

      assertThat(evaluationCount.get()).isGreaterThan(0);
    }
  }
}
