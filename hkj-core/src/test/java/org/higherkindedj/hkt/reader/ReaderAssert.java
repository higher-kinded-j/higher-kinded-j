...
public class ReaderAssert<R, A> extends AbstractAssert<ReaderAssert<R, A>, Reader<R, A>> {
    ...
    public ReaderAssert<R, A> isPureWhenRunWith(R environment) {
        ...
        if (!Objects.equals(firstResult, secondResult)) {
            failWithMessage(
                    "Expected Reader to be pure (produce same result on multiple runs) but got different results: <%s> and <%s>",
    ...
    public ReaderAssert<R, A> isConstantFor(R env1, R env2) {
        ...
        if (!Objects.equals(result1, result2)) {
            failWithMessage(
                    "Expected Reader to be constant (produce same result for all environments) but got <%s> for first environment and <%s> for second environment",
                    result1, result2);
        }
    ...
    public static class ReaderResultAssert<R, A> {
        ...
        public ReaderResultAssert<R, A> produces(A expected) {
            if (!Objects.equals(result, expected)) {
                throw new AssertionError(
                        String.format(
                                "Expected Reader to produce <%s> but produced <%s>", expected, result));
            }
            return this;
        }
         ...
         * @throws AssertionError if the predicate doesn't match
         */
        public ReaderResultAssert<R, A> matches(
                Predicate<? super A> predicate, String description) {
            if (!predicate.test(result)) {
                throw new AssertionError(
                        String.format("%s but <%s> did not match", description, result));
            }
            return this;
        }
