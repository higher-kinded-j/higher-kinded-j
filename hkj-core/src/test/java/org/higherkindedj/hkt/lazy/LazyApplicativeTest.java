...
class LazyApplicativeTest extends TypeClassTestBase<LazyKind.Witness, String, Integer> {
    ...
    class CompleteTestSuite {
        ...
        void runCompleteApplicativeTestsUsingStandardPattern() {
            ...
            TypeClassTest.<LazyKind.Witness>applicative(LazyMonad.class)
                    ...
                    .skipValidations() // Skip generic validations - tested in ValidationTests with correct expectations
                    .test();
        }
    ...
    class ApOperation {
        ...
        void apShouldApplyFunctionLazily() throws Throwable {
            ...
            Kind<LazyKind.Witness, Function<String, Integer>> funcKind = LAZY.widen(Lazy.defer(() -> {
                funcCounter.incrementAndGet();
                return String::length;
            }));

            Kind<LazyKind.Witness, String> valueKind = LAZY.widen(Lazy.defer(() -> {
        ...
        void apShouldPropagateExceptionsFromValueLazy() {
            ...
            Kind<LazyKind.Witness, Function<String, Integer>> funcKind = LAZY.widen(Lazy.now(String::length));
        ...
        void apShouldPropagateExceptionsFromFunctionApplication() {
            ...
            Function<String, Integer> throwingFunc = s -> {
                throw applyException;
            };

            Kind<LazyKind.Witness, Function<String, Integer>> funcKind = LAZY.widen(Lazy.now(throwingFunc));
            Kind<LazyKind.Witness, String> valueKind = LAZY.widen(Lazy.now("test"));
    ...
    class Map2Operation {
        ...
        void map2ShouldCombineValuesLazily() throws Throwable {
            ...
            Kind<LazyKind.Witness, String> kind1 = LAZY.widen(Lazy.defer(() -> {
                counter1.incrementAndGet();
                return "hello";
            }));

            Kind<LazyKind.Witness, String> kind2 = LAZY.widen(Lazy.defer(() -> {
        ...
        void map2ShouldPropagateExceptionsFromCombiningFunction() {
            ...
            BiFunction<String, String, Integer> throwingCombiner = (s1, s2) -> {
    ...
    class ValidationTests {
        ...
        void map2ShouldThrowNPEForNullCombiningFunction() {
            assertThatNullPointerException()
                    .isThrownBy(() -> applicative.map2(validKind, validKind2, (BiFunction<String, String, Integer>) null))
    ...
    class ApplicativeLaws {
        ...
        void interchangeLaw() throws Throwable {
            ...
            Kind<LazyKind.Witness, Function<Function<String, Integer>, Integer>> applyFunc = applicative.of(applyToValue);
    ...
    class IndividualComponents {
        ...
        void testValidationsOnly() {
            ...
            // Note: The generic validation test expects ALL operations (including inherited map from Functor)
        ...
        void testExceptionPropagationOnly() {
            ...
            assertThat(true)
                    .as("Exception propagation for Lazy is tested in ApOperation and Map2Operation nested classes")
                    .isTrue();
        }
