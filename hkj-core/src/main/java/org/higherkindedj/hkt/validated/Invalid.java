...
public record Invalid<E, A>(E error) implements Validated<E, A>, ValidatedKind<E, A> {
    ...
    public A orElseGet(Supplier<? extends A> otherSupplier) {
        FunctionValidator.requireFunction(otherSupplier, "otherSupplier", "Invalid.orElseGet");
    ...
    public <X extends Throwable> A orElseThrow(Supplier<? extends X> exceptionSupplier) throws X {
        FunctionValidator.requireFunction(exceptionSupplier, "exceptionSupplier", "Invalid.orElseThrow");
        X throwable = exceptionSupplier.get();
        Objects.requireNonNull(throwable, "orElseThrow 'exceptionSupplier' must not produce a null throwable for Invalid.");
        throw throwable;
    }

    @Override
    public void ifValid(Consumer<? super A> consumer) {
        FunctionValidator.requireFunction(consumer, "consumer", "Invalid.ifValid");
        // No action for Invalid
    }

    @Override
    public void ifInvalid(Consumer<? super E> consumer) {
        FunctionValidator.requireFunction(consumer, "consumer", "Invalid.ifInvalid");
    ...
    public <B> Validated<E, B> map(Function<? super A, ? extends B> fn) {
        FunctionValidator.requireMapper(fn, "Invalid.map");
    ...
    public <B> Validated<E, B> flatMap(Function<? super A, ? extends Validated<E, ? extends B>> fn) {
        FunctionValidator.requireFlatMapper(fn, "Invalid.flatMap");
        return (Validated<E, B>) this;
    }
