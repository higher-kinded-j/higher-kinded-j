...
public interface State<S, A> {
    ...
    static <S, A> State<S, A> of(Function<S, StateTuple<S, A>> runFunction) {
        FunctionValidator.requireFunction(runFunction, "runFunction", "State.of");
    ...
    default <B> State<S, B> map(Function<? super A, ? extends B> f) {
        FunctionValidator.requireMapper(f, "State.map");
        return State.of(
                initialState -> {
    ...
    default <B> State<S, B> flatMap(Function<? super A, ? extends State<S, ? extends B>> f) {
        FunctionValidator.requireFlatMapper(f, "State.flatMap");
        return State.of(
                initialState -> {
                    ...
                    Objects.requireNonNull(nextState, "State.flatMap function returned null State instance");
    ...
    static <S> State<S, Unit> modify(Function<S, S> f) {
        FunctionValidator.requireFunction(f, "state modification function", "State.modify");
    ...
    static <S, A> State<S, A> inspect(Function<S, @Nullable A> f) {
        FunctionValidator.requireFunction(f, "state inspection function", "State.inspect");
