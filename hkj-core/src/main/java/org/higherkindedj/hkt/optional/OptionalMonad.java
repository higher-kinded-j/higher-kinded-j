...
public class OptionalMonad extends OptionalFunctor
  ...
  /**
   * Recovers from an empty {@code OptionalKind} by replacing it with a pre-computed {@code
   * fallback}. If {@code ma} is present, it is returned unchanged and {@code fallback} is ignored.
   *
   ...
   * inherited {@link MonadError} default only reads {@code fallback} on the empty path, so a null
   * would be silently ignored for a present value and surface as a misleading {@code
   * handleErrorWith} failure for an empty one.
   *
   * @param <A> The type of the value.
   * @param ma The non-null {@code Kind<OptionalKind.Witness, A>} that might be empty.
   * @param fallback The non-null {@code OptionalKind} to use if {@code ma} is empty.
   ...
   */
  @Override
  public <A> Kind<OptionalKind.Witness, A> recoverWith(
      final Kind<OptionalKind.Witness, A> ma, final Kind<OptionalKind.Witness, A> fallback) {
  ...
  /**
   * Recovers from an empty {@code OptionalKind} with a pure fallback {@code value}, lifted via
   * {@link #of(Object)}. If {@code ma} is present, it is returned unchanged.
   *
   ...
   * since {@code of(null)} (and hence {@code recover(empty, null)}) yields an empty {@code
   * OptionalKind}.
   *
   * @param <A> The type of the value.
   * @param ma The non-null {@code Kind<OptionalKind.Witness, A>} that might be empty.
   ...
   */
  @Override
  public <A> Kind<OptionalKind.Witness, A> recover(
      final Kind<OptionalKind.Witness, A> ma, @Nullable A value) {
