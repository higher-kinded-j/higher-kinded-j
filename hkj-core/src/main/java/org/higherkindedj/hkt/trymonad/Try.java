...
public sealed interface Try<T> permits Try.Success, Try.Failure {
  ...
  static <T> Try<T> of(Supplier<? extends T> supplier) {
    FunctionValidator.requireFunction(supplier, "supplier", "Try.of");
    try {
  ...
  default <U> U fold(
    ...
    FunctionValidator.requireFunction(successMapper, "successMapper", "Try.fold");
    FunctionValidator.requireFunction(failureMapper, "failureMapper", "Try.fold");
  ...
  default <L> Either<L, T> toEither(Function<? super Throwable, ? extends L> failureToLeftMapper) {
    FunctionValidator.requireFunction(failureToLeftMapper, "failureToLeftMapper", "Try.toEither");
    return switch (this) {
  ...
  default void match(Consumer<? super T> successAction, Consumer<? super Throwable> failureAction) {
    FunctionValidator.requireFunction(successAction, "successAction", "Try.match");
    FunctionValidator.requireFunction(failureAction, "failureAction", "Try.match");
  ...
  record Success<T>(@Nullable T value) implements Try<T> {
    ...
    public @Nullable T orElseGet(Supplier<? extends T> supplier) {
      FunctionValidator.requireFunction(supplier, "supplier", "Try.Success.orElseGet");
      return value;
    }

    @Override
    public <U> Try<U> map(Function<? super T, ? extends U> mapper) {
      FunctionValidator.requireMapper(mapper, "Try.Success.map");
      try {
    ...
    public <U> Try<U> flatMap(Function<? super T, ? extends Try<? extends U>> mapper) {
      FunctionValidator.requireFlatMapper(mapper, "Try.Success.flatMap");
      ...
      FunctionValidator.requireNonNullResult(result, "Try.Success.flatMap", Try.class);
    ...
    public Try<T> recover(Function<? super Throwable, ? extends T> recoveryFunction) {
      FunctionValidator.requireFunction(
          recoveryFunction, "recoveryFunction", "Try.Success.recover");
      return this;
    }
    ...
    public Try<T> recoverWith(
      ...
      FunctionValidator.requireFunction(
          recoveryFunction, "recoveryFunction", "Try.Success.recoverWith");
      return this;
    }
  }
  ...
  record Failure<T>(Throwable cause) implements Try<T> {
    ...
    @Override
    public @Nullable T orElseGet(Supplier<? extends T> supplier) {
      FunctionValidator.requireFunction(supplier, "supplier", "Try.Failure.orElseGet");
      return supplier.get();
    }

    @Override
    public <U> Try<U> map(Function<? super T, ? extends U> mapper) {
      FunctionValidator.requireMapper(mapper, "Try.Failure.map");
      @SuppressWarnings("unchecked")
    ...
    @Override
    public <U> Try<U> flatMap(Function<? super T, ? extends Try<? extends U>> mapper) {
      FunctionValidator.requireFlatMapper(mapper, "Try.Failure.flatMap");
      @SuppressWarnings("unchecked")
      Try<U> self = (Try<U>) this;
      return self;
    }
    ...
    public Try<T> recover(Function<? super Throwable, ? extends T> recoveryFunction) {
      FunctionValidator.requireFunction(
          recoveryFunction, "recoveryFunction", "Try.Failure.recover");
      try {
    ...
    public Try<T> recoverWith(
        Function<? super Throwable, ? extends Try<? extends T>> recoveryFunction) {
      FunctionValidator.requireFunction(
          recoveryFunction, "recoveryFunction", "Try.Failure.recoverWith");
      ...
      FunctionValidator.requireNonNullResult(result, "Try.Failure.recoverWith", Try.class);
      @SuppressWarnings("unchecked")
