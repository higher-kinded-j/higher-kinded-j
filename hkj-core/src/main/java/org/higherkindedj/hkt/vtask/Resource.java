...
public final class Resource<A> {
   ...
   *
   * <p>The transformation is applied after acquire, and the original release is still used.
   *
  ...
  public <B> Resource<B> map(Function<? super A, ? extends B> f) {
    Objects.requireNonNull(f, "f must not be null");
    return new Resource<>(
        () -> f.apply(acquire.call()),
        b -> {
          // We can't easily reverse the mapping, so this is a limitation
          // In practice, map is used for non-resource values derived from the resource
        });
  }
  ...
  public <B> Resource<B> flatMap(Function<? super A, ? extends Resource<B>> f) {
    ...
    return new Resource<>(
        ...
        b -> {
          // Note: This simplified implementation doesn't have access to 'a' here
          // For proper nested release, use the 'use' method with explicit nesting
        });
  }
