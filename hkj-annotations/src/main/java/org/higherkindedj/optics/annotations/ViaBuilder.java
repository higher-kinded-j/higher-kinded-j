 ...
 * unboxed to its getter's type wherever that settles the call on the one taking exactly that type;
 * where another candidate takes a primitive too, unboxing could move the call, so the focus is
 * passed as it is.
 *
 * @see OpticsSpec
 * @see Wither
