...
class EffectAlgebraProcessorTest {
  ...
  class ValidationTests {
    ...
    void missingTypeParamShouldError() {
      ...
      assertThat(compilation.errors()).isNotEmpty();
      assertThat(compilation.errors().get(0).getMessage(null))
          .contains("exactly one type parameter");
    }
    ...
    @Test
    @DisplayName("Permit with extra type parameters should produce error")
    void extraTypeParamOnPermitShouldError() {
      var source =
          JavaFileObjects.forSourceString(
              "test.pkg.BadOp",
              """
              ...
              @EffectAlgebra
              public sealed interface BadOp<A> permits BadOp.Foo {
                  record Foo<A, B>() implements BadOp<A> {}
              }
              """);
      ...
      assertThat(compilation.errors()).isNotEmpty();
      assertThat(compilation.errors().get(0).getMessage(null))
          .contains("has extra type parameters beyond <A>");
    }

    @Test
    @DisplayName("Custom targetPackage should be used for generated files")
    void customTargetPackage() {
      var source =
          JavaFileObjects.forSourceString(
              ...
              public sealed interface MyOp<A> permits MyOp.DoIt {
                  record DoIt<A>() implements MyOp<A> {}
              }
              """);
      ...
      // Files should be in the custom target package
      assertThat(compilation.generatedSourceFile("test.gen.MyOpKind")).isPresent();
      assertThat(compilation.generatedSourceFile("test.gen.MyOpKindHelper")).isPresent();
      assertThat(compilation.generatedSourceFile("test.gen.MyOpFunctor")).isPresent();
      assertThat(compilation.generatedSourceFile("test.gen.MyOpOps")).isPresent();
      assertThat(compilation.generatedSourceFile("test.gen.MyOpInterpreter")).isPresent();
    }

    @Test
    @DisplayName("Algebra with consecutive uppercase produces correct singleton name")
    void consecutiveUppercaseInName() throws IOException {
      var source =
          JavaFileObjects.forSourceString(
              "test.pkg.IOOp",
              """
              ...
              @EffectAlgebra
              public sealed interface IOOp<A> permits IOOp.Run {
                  record Run<A>() implements IOOp<A> {}
              }
              """);
      ...
      String helperSource = getGeneratedSource(compilation, "test.pkg.IOOpKindHelper");
      // IOOp -> IOOP (consecutive uppercase chars don't get underscore between them)
      assertThat(helperSource).contains("IOOP;");
    }
  }
