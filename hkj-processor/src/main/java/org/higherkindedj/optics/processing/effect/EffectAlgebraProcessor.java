...
public class EffectAlgebraProcessor extends AbstractProcessor {
    ...
    for (TypeElement annotation : annotations) {
      for (Element element : roundEnv.getElementsAnnotatedWith(annotation)) {
        if (element.getKind() != ElementKind.INTERFACE) {
          error(
              "@EffectAlgebra can only annotate sealed interfaces, but found "
        ...
        if (!typeElement.getModifiers().contains(Modifier.SEALED)) {
          error(
              "@EffectAlgebra can only annotate sealed interfaces. "
                  + "Add the 'sealed' modifier to "
        ...
        if (typeElement.getTypeParameters().size() != 1) {
          error(
              "Effect algebra must have exactly one type parameter <A> (the result type), "
                  + "found "
  ...
  private void writeAlgebraClasses(
    ...
    try {
      ...
      generateKind(packageName, baseName, sourceClass, typeElement);
      generateKindHelper(packageName, baseName, sourceClass, typeElement);
      generateFunctor(packageName, baseName, sourceClass, typeElement);
      generateOps(packageName, baseName, sourceClass, typeElement, permits);
      generateInterpreter(packageName, baseName, sourceClass, typeElement, permits);
    } catch (Exception e) {
      error(
  ...
  private void generateKind(
      String packageName, String baseName, ClassName sourceClass, TypeElement sourceElement)
      throws IOException {
  ...
  private void generateKindHelper(
      String packageName, String baseName, ClassName sourceClass, TypeElement sourceElement)
      throws IOException {
    String kindName = baseName + "Kind";
  ...
  private void generateOps(
    ...
    opsBuilder.addJavadoc(
        ...
        baseName,
        baseName);
    opsBuilder.addOriginatingElement(sourceElement);
  ...
  private void generateInterpreter(
    ...
    interpreterBuilder.addJavadoc(
        ...
        baseName,
        baseName);
    interpreterBuilder.addOriginatingElement(sourceElement);
